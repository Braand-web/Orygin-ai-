...
const CHOOSER_PACKAGE = '@orygin-ai/dsh-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@orygin-ai/dsh-host-directory-picker-native',
  '@orygin-ai/dsh-host-directory-picker-browse',
  '@orygin-ai/dsh-client-ui-directory-picker-browse',
  '@orygin-ai/dsh-client-ui-directory-picker-native',
]
const errors: string[] = []
 ...
 *
 * Both have happened. `shell-env` in a preset realm left `ORYGIN_WEB_URL` reaching
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if (value.name !== '@orygin-ai/cordis-plugin-include') return
