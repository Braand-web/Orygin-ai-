...
describe('CI workflow', () => {
  ...
  it('keeps a required Wine Windows job, a non-blocking native Windows job with failover, and a master-only standby', () => {
    ...
    expect(windowsNative['runs-on']).toContain('ORYGIN_CI_FAILOVER_WINDOWS')
    expect(windowsNative['runs-on']).not.toContain('ORYGIN_CI_FAILOVER_LINUX')
    ...
    expect(windowsNative.env).toMatchObject({
      ORYGIN_COVERAGE_TEST_TIMEOUT_MS: '30000',
    })
    ...
    // and the verdict job resolve their pool through ORYGIN_CI_FAILOVER_LINUX,
    // never the Windows switch.
    for (const [jobName, job] of [['node-24', node24], ['node-24-coverage', node24Coverage], ['node-24-consumers', node24Consumers]] as const) {
      expect(typeof job['runs-on']).toBe('string')
      expect(job['runs-on'], `${jobName} runs-on must use the Linux failover switch`).toContain('ORYGIN_CI_FAILOVER_LINUX')
      expect(job['runs-on'], `${jobName} runs-on must not use the Windows failover switch`).not.toContain('ORYGIN_CI_FAILOVER_WINDOWS')
      expect(job['runs-on']).toContain('vm-backup')
    }
    expect(aggregate['runs-on']).toContain('ORYGIN_CI_FAILOVER_LINUX')
    expect(aggregate['runs-on']).not.toContain('ORYGIN_CI_FAILOVER_WINDOWS')
...
describe('Orygin e2e workflow', () => {
  it('prepares bubblewrap from the pinned payload without a package transaction', () => {
    const workflow = loadWorkflow('.github/workflows/e2e.yml')
    const e2e = workflowJob(workflow, 'e2e')
    if (!Array.isArray(e2e.steps)) throw new TypeError('Orygin e2e workflow must define steps')
  ...
  it('is manual-only and fails loud before running the focused live suite', () => {
    ...
    expect(e2b).toMatchObject({
      env: {
        ...
        ORYGIN_E2E_MAX_WORKERS: '1',
        ORYGIN_EXAMPLE_MODE: 'lib',
      },
    })
...
describe('Python release workflows', () => {
  it('keeps complete wheel validation separate from protected public publication', () => {
    ...
    expect(pythonCompatSteps).toContain('dist/orygin_harness_sdk-$VERSION-py3-none-any.whl')
    expect(pythonCompatSteps).toContain('dist/orygin_harness_runtime_bin-$VERSION-py3-none-manylinux_2_28_x86_64.whl')
  ...
  it('exposes the native wheel builder to the release caller with normalized versions', () => {
    ...
    expect(JSON.stringify(manylinuxSmoke)).toContain('-e ORYGIN_TELEMETRY_DISABLED')
  })
