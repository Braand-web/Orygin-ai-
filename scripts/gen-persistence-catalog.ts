...
const SESSION_PACKAGE = '@orygin-ai/dsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@orygin-ai/dsh-session/types'
 ...
 * top-level declaration (in `@orygin-ai/dsh-session`) and any declaration
 * merge inside a `declare module '@orygin-ai/dsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @orygin-ai/dsh-session/known-event-types',
    ' */',
    '',
    '/**',
