...
const vendoredPackages = new Set([
  '@orygin-ai/cordis',
  '@orygin-ai/cosmokit',
  '@orygin-ai/schemastery',
  '@orygin-ai/cordis-plugin-loader',
  '@orygin-ai/cordis-plugin-include',
  '@orygin-ai/cordis-plugin-group',
  '@orygin-ai/cordis-plugin-timer',
  '@orygin-ai/cordis-plugin-hmr',
  '@orygin-ai/cordis-plugin-logger-console',
])
const publicLandlockPackages = new Set([
  '@orygin-ai/node-addon-landlock-run',
  '@orygin-ai/node-addon-landlock-run-linux-arm64',
  '@orygin-ai/node-addon-landlock-run-linux-x64',
])
...
const publicationSourceAllowlist: Readonly<Record<string, readonly string[]>> = {
  '@orygin-ai/node-addon-landlock-run': ['src/main.c'],
}
const repositoryUrl = 'git+https://github.com/Braand-web/Orygin-ai-.git'
/**
 ...
 */
const publishedRepositoryUrl = 'git+https://github.com/Braand-web/Orygin-ai-.git'
...
const experimentalPackageNamePrefix = '@orygin-ai/dsh-experimental-'
/** Directories whose packages this repository publishes: one release member each. */
const releaseMemberDirectory = /^(?:packages\/(?!experimental\/)[^/]+\/[^/]+|apps\/[^/]+|vendor\/[^/]+)$/

const localArtifactDirs = new Set(['node_modules'])
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@orygin-ai/dsh': ['lib/*.js', 'config'],
  // The Web build emits sourcemaps for browser debugging; publishing them is
  // what the payload policy forbids, so the bundle ships without them.
  '@orygin-ai/dsh-web-frontend': ['dist', '!dist/**/*.map'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@orygin-ai/dsh-client-ui-primitives': ['lib/**/*.css'],
  '@orygin-ai/dsh-client-web': ['lib/**/*.css'],
  '@orygin-ai/dsh-client-ui-theme': ['lib/styles'],
  // The CPython side ships as source .py files, published as-is rather than built.
  '@orygin-ai/dsh-code-runtime-python': ['py/**/*.py'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@orygin-ai/dsh-sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@orygin-ai/dsh-sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  // SQLite loads every statement from immutable package resources at runtime.
  '@orygin-ai/dsh-session-persistence-sqlite': ['resources/sql/**/*.sql'],
  '@orygin-ai/dsh-skill-badge': ['assets'],
  '@orygin-ai/dsh-subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (manifest.name?.startsWith('@orygin-ai/')) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@orygin-ai/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@orygin-ai/dsh-')) {
    const peer = manifest.peerDependencies?.['@orygin-ai/cordis']
