...
import type { ToolSchema } from '@orygin-ai/dsh-llm'
import AgentRegistry from '@orygin-ai/dsh-agent'
import type { Agent } from '@orygin-ai/dsh-agent'
import { createScope } from '@orygin-ai/dsh-scope'
import SessionStore, { SessionId } from '@orygin-ai/dsh-session'
import SessionProjectionRegistry from '@orygin-ai/dsh-session-projection'
import SqliteSessionQueryEngine from '@orygin-ai/dsh-session-query-sqlite'
import GoalService from '@orygin-ai/dsh-goal'
import SystemPrompt from '@orygin-ai/dsh-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@orygin-ai/dsh-tools'
import LocalBashExecutor from '@orygin-ai/dsh-bash-local'
import * as BashEnvPlugin from '@orygin-ai/dsh-shell-env'
import { PwshLocalExecutor } from '@orygin-ai/dsh-pwsh-local'
import LocalSubprocessRuntime from '@orygin-ai/dsh-subprocess-local'
import LocalFileSystem from '@orygin-ai/dsh-fs-local'
import { AttachmentStore } from '@orygin-ai/dsh-attachment'
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@orygin-ai/dsh-attachment'
import UserQuestionService from '@orygin-ai/dsh-user-questions'
import PlanModeController from '@orygin-ai/dsh-plan-mode'
import WebRuntime from '@orygin-ai/dsh-web'
import * as WebSearchExa from '@orygin-ai/dsh-web-search-exa'
import * as WebFetchLocal from '@orygin-ai/dsh-web-fetch-http'
import SubagentRuntime from '@orygin-ai/dsh-subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@orygin-ai/dsh-subagent'
import * as ToolSubagentControl from '@orygin-ai/dsh-tool-subagent-control'
import * as ToolSubagentListAgents from '@orygin-ai/dsh-tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@orygin-ai/dsh-tool-subagent-report'
import SkillRegistry from '@orygin-ai/dsh-skill'
import * as SkillFileSystem from '@orygin-ai/dsh-skill-filesystem'
import LocalJobRegistry from '@orygin-ai/dsh-jobs-local'
import * as ToolAskUser from '@orygin-ai/dsh-tool-ask-user'
import * as ToolBash from '@orygin-ai/dsh-tool-bash'
import * as ToolPwsh from '@orygin-ai/dsh-tool-pwsh'
import * as ToolBashPersistent from '@orygin-ai/dsh-tool-bash-persistent'
import * as ToolPwshPersistent from '@orygin-ai/dsh-tool-pwsh-persistent'
import CordisHostRunner from '@orygin-ai/dsh-cordis-host-runner'
import * as ToolCordis from '@orygin-ai/dsh-tool-cordis'
import * as ToolFs from '@orygin-ai/dsh-tool-fs'
import * as ToolFsSearch from '@orygin-ai/dsh-tool-fs-search'
import * as ToolStrReplaceEditor from '@orygin-ai/dsh-tool-str-replace-editor'
import TerminalSessionService from '@orygin-ai/dsh-terminal'
import * as ToolPty from '@orygin-ai/dsh-tool-terminal'
import * as ToolGoal from '@orygin-ai/dsh-tool-goal'
import * as ToolSchedule from '@orygin-ai/dsh-schedule'
import Lsp from '@orygin-ai/dsh-lsp'
import * as ToolLsp from '@orygin-ai/dsh-tool-lsp'
import * as ToolSkill from '@orygin-ai/dsh-tool-skill'
import * as ToolSessionQuery from '@orygin-ai/dsh-tool-session-query'
import * as ToolTasks from '@orygin-ai/dsh-tool-jobs'
import type TeamService from '@orygin-ai/dsh-experimental-agent-team'
import * as ToolTeam from '@orygin-ai/dsh-experimental-tool-agent-team'
import * as ToolTodo from '@orygin-ai/dsh-tool-todo'
import * as ToolSubagent from '@orygin-ai/dsh-tool-subagent'
import * as ToolWeb from '@orygin-ai/dsh-tool-web'
import VmWorkflowEngine from '@orygin-ai/dsh-workflow-worker-thread'
import * as ToolRalph from '@orygin-ai/dsh-tool-ralph'
import * as ToolWorkflow from '@orygin-ai/dsh-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  {
    pkg: '@orygin-ai/dsh-tool-ask-user',
    dir: 'tool-ask-user',
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@orygin-ai/dsh-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@orygin-ai/dsh-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@orygin-ai/dsh-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `ORYGIN_*` environment comes from `@orygin-ai/dsh-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@orygin-ai/dsh-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@orygin-ai/dsh-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or ORYGIN restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@orygin-ai/dsh-tool-bash-persistent',
  ...
  {
    pkg: '@orygin-ai/dsh-tool-pwsh-persistent',
  ...
  {
    pkg: '@orygin-ai/dsh-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@orygin-ai/dsh-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. The image tool is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@orygin-ai/dsh-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  {
    pkg: '@orygin-ai/dsh-tool-terminal',
    dir: 'tool-terminal',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@orygin-ai/dsh-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@orygin-ai/dsh-tool-ralph',
    dir: 'tool-ralph',
  ...
  {
    pkg: '@orygin-ai/dsh-tool-session-query',
  ...
  {
    pkg: '@orygin-ai/dsh-tool-subagent',
    dir: 'tool-subagent',
  ...
  {
    pkg: '@orygin-ai/dsh-tool-subagent-control',
  ...
  {
    pkg: '@orygin-ai/dsh-tool-subagent-report',
  ...
  {
    pkg: '@orygin-ai/dsh-experimental-tool-agent-team',
    dir: 'tool-agent-team',
  ...
  {
    pkg: '@orygin-ai/dsh-tool-workflow',
    dir: 'tool-workflow',
