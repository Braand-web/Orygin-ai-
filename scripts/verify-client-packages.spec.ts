...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@orygin-ai/dsh-client-' + short,
...
describe('source package uses', () => {
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@orygin-ai/dsh-a/subpath'",
      "declare module '@orygin-ai/dsh-client-ui-slots' {}",
      "const load = () => import('@orygin-ai/dsh-b')",
...
describe('package modes', () => {
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@orygin-ai/dsh-client-runtime/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('dependency sections', () => {
  it('accepts dynamic peer plus dev relationships, static dev inputs, and private dependencies', () => {
    ...
    const runtime = pkg('runtime', {
      inject: ['@orygin-ai/dsh-client-feature'],
      sourceUses: {
        '@orygin-ai/dsh-agent': ['packages/client/runtime/src/index.ts'],
        '@orygin-ai/dsh-client-ui-slots': ['packages/client/runtime/src/client/slots.ts'],
        react: ['packages/client/runtime/src/client/view.tsx'],
      },
      dependencies: { immer: '^10.1.1' },
      peerDependencies: {
        [CORDIS]: 'workspace:^',
        '@orygin-ai/dsh-agent': 'workspace:^',
        '@orygin-ai/dsh-client-feature': 'workspace:^',
      },
      devDependencies: {
        [CORDIS]: 'workspace:^',
        '@orygin-ai/dsh-agent': 'workspace:^',
        '@orygin-ai/dsh-client-feature': 'workspace:^',
        '@orygin-ai/dsh-client-ui-slots': 'workspace:^',
        react: '^18.2.0',
      },
    })
  ...
  it('rejects internal dependencies, static peers, and mismatched peer development ranges', () => {
    ...
    const subject = pkg('feature', {
      sourceUses: {
        '@orygin-ai/dsh-agent': ['packages/client/feature/src/index.ts'],
        [slots.name]: ['packages/client/feature/src/view.tsx'],
      },
      dependencies: { '@orygin-ai/dsh-agent': 'workspace:^' },
    ...
    expect(found).toHaveLength(2)
    expect(found.join('\n')).toContain('peer-installed ORYGIN relationship')
    expect(found.join('\n')).toContain('static client input')
  })

  it('requires every peer to have the same development range', () => {
    const subject = pkg('feature', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@orygin-ai/cordis-plugin-loader': 'workspace:^' },
    })
    expect(collectClientPackageViolations(facts([subject]))).toEqual([
      'packages/client/feature/package.json: peerDependencies.@orygin-ai/cordis-plugin-loader'
      + ' is workspace:^, so devDependencies.@orygin-ai/cordis-plugin-loader must use the same range;'
  ...
  it('keeps the web shell runtime inputs development-only', () => {
    const web = pkg('web', {
      ...
      staticLinked: true,
      runtimeSourceUses: {
        '@orygin-ai/cordis-plugin-loader': ['packages/client/web/src/boot.ts'],
        react: ['packages/client/web/src/seed.ts'],
      },
      devDependencies: {
        [CORDIS]: 'workspace:^',
        '@orygin-ai/cordis-plugin-loader': 'workspace:^',
        react: '^18.2.0',
      },
    })
  ...
  it('allows npm dependency cycles', () => {
    const a = pkg('a', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@orygin-ai/dsh-client-b': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@orygin-ai/dsh-client-b': 'workspace:^' },
    })
    const b = pkg('b', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@orygin-ai/dsh-client-a': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@orygin-ai/dsh-client-a': 'workspace:^' },
    })
...
describe('module requests', () => {
  it('accepts a dynamic row supplier and its client subpath', () => {
    const ui = declaration('ui', { external: ['@orygin-ai/dsh-client-slots/client'] })
  ...
  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@orygin-ai/dsh-client-ui', '@orygin-ai/dsh-missing', '@orygin-ai/dsh-missing'],
      inject: ['', '@orygin-ai/dsh-a', '@orygin-ai/dsh-a'],
    })
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = declaration('a', {
      external: ['@orygin-ai/dsh-client-b'],
      inject: ['@orygin-ai/dsh-client-b'],
    })
    const b = declaration('b', {
      external: ['@orygin-ai/dsh-client-a'],
      inject: ['@orygin-ai/dsh-client-a'],
    })
...
describe('manifest declarations', () => {
  ...
  it('fixes unambiguous dependency sections and declaration entries', () => {
    ...
    roots.push(root)
    const subject = pkg('feature', {
      external: ['', 'react', '@orygin-ai/dsh-client-feature', '@orygin-ai/dsh-missing'],
      inject: ['', '@orygin-ai/dsh-agent', '@orygin-ai/dsh-agent'],
      sourceUses: {
        '@orygin-ai/dsh-agent': ['packages/client/feature/src/index.ts'],
        '@orygin-ai/dsh-client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@orygin-ai/dsh-agent': 'workspace:*',
    ...
    expect(fixed.dsh.client).toMatchObject({
      external: ['@orygin-ai/dsh-missing'],
      inject: ['@orygin-ai/dsh-agent'],
    })
