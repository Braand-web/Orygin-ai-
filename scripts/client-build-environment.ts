...
const CLIENT_BUILD_ENV_PREFIX = 'ORYGIN_CLIENT_'

/** Non-public selector used by build orchestration to request a named client profile. */
export const CLIENT_BUILD_PROFILE_SELECTOR = 'ORYGIN_BUILD_CLIENT_PROFILE'

/** Public client environment required by official ORYGIN artifacts. */
...
const CLIENT_COMMIT_HASH_VARIABLE = 'ORYGIN_CLIENT_COMMIT_HASH'
...
): Readonly<Record<`ORYGIN_CLIENT_${string}`, string>> {
  return {
    ORYGIN_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
 ...
 * @returns defined `ORYGIN_CLIENT_*` values only.
 */
...
export function resolveClientBuildEnvironment(
  ...
  if (profile === 'official') {
    ...
    return { ORYGIN_CLIENT_COMMIT_HASH: commitHash, ...OFFICIAL_CLIENT_BUILD_ENVIRONMENT }
  }
...
export function assertClientBuildEnvironment(
  ...
  expected: Readonly<Record<`ORYGIN_CLIENT_${string}`, string>>,
): void {
