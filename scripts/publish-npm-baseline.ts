...
const RELEASE_ENTRY_PACKAGE = '@orygin-ai/dsh'
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@orygin-ai/')) {
        throw new Error(`${manifestPath} must name an @orygin-ai package`)
      }
      if (name === '@orygin-ai/dsh-root') {
...
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@orygin-ai/dsh/lib/bin.js')
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@orygin-ai/') || name === '@orygin-ai/dsh-root')) {
...
function installedArtifactEnvironment(consumerRoot: string): NodeJS.ProcessEnv {
  ...
  environment.ORYGIN_HOME = resolve(consumerRoot, '.dsh')
  environment.ORYGIN_AGENTS_HOME = resolve(consumerRoot, '.agents')
  environment.ORYGIN_TELEMETRY_DISABLED = '1'
  environment.ORYGIN_API_KEY = 'keyless-installed-web-no-call'
