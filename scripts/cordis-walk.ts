...
const MERGE_HEAD = /declare module ['"](?:@orygin-ai\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@orygin-ai/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@orygin-ai/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@orygin-ai/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@orygin-ai/cordis'` Context merge
...
/**
 * Every event name a `declare module '@orygin-ai/cordis'` Events merge declares in one
