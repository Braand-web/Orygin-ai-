...
async function main(args: string[]): Promise<number> {
  ...
  const concurrencyOverride = process.env.ORYGIN_GATE_CONCURRENCY
  const maxConcurrency = concurrencyFromEnv('ORYGIN_GATE_CONCURRENCY', concurrencyDefault.workers)
  const concurrencySource = concurrencyOverride === undefined || concurrencyOverride === ''
    ? concurrencyDefault.source
    : '$ORYGIN_GATE_CONCURRENCY'
    ...
    case 'check-all':
      return [
        ...
        ...docSyncLeafGates({
          ...
          docTypecheckEnv: { ORYGIN_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
...
function ciSharedStaticGates(): Gate[] {
  return [
    ...
    pnpmScript('dsh-package-licenses', 'verify-dsh-package-licenses', { label: 'ORYGIN package licenses' }),
...
function nodeCompatGates(): Gate[] {
  const typecheck = flagEnabled('ORYGIN_NODE_COMPAT_SKIP_TYPECHECK')
    ? []
  ...
  if (options.cliSmoke) {
    gates.push(
      pnpmExec('cli-lazy-search-startup-smoke', [
        ...
        env: { ORYGIN_REQUIRE_BUILT_CLI_SMOKE: '1' },
        needs: ['build:web'],
      }),
    )
  }
...
function ciConsumerGates(): Gate[] {
  ...
  return [
    ...
    pnpmScript('doc-typecheck', 'doc-typecheck:contracts-ready', {
      needs: validatedBuild,
      env: { ORYGIN_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
    }),
    pnpmScript('node-next-types', 'verify-node-next-types', {
      label: 'node-next types',
      needs: validatedBuild,
...
function webSnapshotGate(needs: string[]): Gate {
  const workerRaw = process.env.ORYGIN_WEB_SNAPSHOT_WORKERS
  if (workerRaw !== undefined && workerRaw !== '') {
    ...
    if (!Number.isSafeInteger(workers) || workers < 2 || String(workers) !== workerRaw) {
      throw new Error(`run-gates: ORYGIN_WEB_SNAPSHOT_WORKERS must be an integer greater than 1, got ${JSON.stringify(workerRaw)}.`)
    }
    return pnpmScript('web-snapshot', 'test:web:ci', {
      label: 'web browser snapshot',
      displayCommand: `ORYGIN_SNAPSHOT=replay ORYGIN_WEB_SNAPSHOT_WORKERS=${workers} pnpm run test:web:ci`,
      env: { ORYGIN_SNAPSHOT: 'replay' },
      needs,
      streamOutput: true,
    })
  }
  return pnpmScript('web-snapshot', 'test:web:built', {
    label: 'web browser snapshot',
    displayCommand: 'ORYGIN_SNAPSHOT=replay pnpm run test:web:built',
    env: { ORYGIN_SNAPSHOT: 'replay' },
    needs,
  })
}
...
function lintGate(options: { needs?: string[] } = {}): Gate {
  const raw = process.env.ORYGIN_OXLINT_THREADS
  const script = 'lint:contracts-ready'
  return pnpmScript('lint', script, {
    ...raw === undefined || raw === ''
      ? {}
      : { displayCommand: `ORYGIN_OXLINT_THREADS=${raw} pnpm run ${script}` },
...
//
// ORYGIN_COVERAGE_MAX_WORKERS is the ordinary lane's worker budget, so the two
// parallel gates split it instead of each claiming it whole. When
// ORYGIN_COVERAGE_PARTITIONS is set, its single-worker processes replace the
...
// total of one (the serial reference jobs) also set ORYGIN_GATE_CONCURRENCY=1,
// which keeps the gates from overlapping at all.
// ORYGIN_COVERAGE_TEST_TIMEOUT_MS raises Vitest's per-test and expect.poll
...
function coverageWorkerArgs(): { instrumented: string[]; exempt: string[] } {
  const [flag] = positiveIntArg('ORYGIN_COVERAGE_MAX_WORKERS', '--maxWorkers')
...
function printResult(result: GateResult): void {
  const verbose = process.env.ORYGIN_GATE_VERBOSE === '1'
