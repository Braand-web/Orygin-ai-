...
const REQUESTING_PACKAGE = '@orygin-ai/dsh-client-ui-conversation'
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@orygin-ai/dsh-client-test', ['lib/types/index.js'])
    const development = bundle({ env: {} }).find(config => config.platform === 'browser')
    const artifact = bundle({ env: { ORYGIN_BUILD_FACE: 'client' } })
...
describe('client bundle purity gate', () => {
  ...
  it('leaves default externals and non-scoped specifiers alone', () => {
    expect(resolveId('@orygin-ai/dsh-client-ui-slots')).toBeNull()
    expect(resolveId('@orygin-ai/dsh-client-ui-primitives')).toBeNull()
    expect(resolveId('@orygin-ai/dsh-client-runtime/client')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects the retired web-react platform package', () => {
    expect(() => resolveId('@orygin-ai/dsh-client-web-react')).toThrow(/purity/)
    expect(() => resolveId('@orygin-ai/dsh-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('@orygin-ai/dsh-host-apiproxy/api')).toBeNull()
    expect(resolveId('@orygin-ai/dsh-session/surface')).toBeNull()
    expect(resolveId('@orygin-ai/dsh-brand')).toBeNull()
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@orygin-ai/dsh-goal/remote')).toBeNull()
    expect(() => resolveId('@orygin-ai/dsh-goal')).toThrow(/purity/)
    expect(() => resolveId('@orygin-ai/dsh-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@orygin-ai/dsh-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @orygin-ai leak', () => {
    expect(() => resolveId('@orygin-ai/dsh-agent')).toThrow(/purity/)
    expect(() => resolveId('@orygin-ai/dsh-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike', () => {
    expect(() => resolveId('@orygin-ai/dsh-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@orygin-ai/dsh-client-runtime')).toThrow(/purity/)
    expect(() => resolveId('@orygin-ai/dsh-client-ui-layout/client')).toThrow(/purity/)
  })

  it('admits the parser-preloaded runtime for every dynamic bundle', () => {
    expect(resolveId('@orygin-ai/dsh-client-runtime/client')).toBeNull()
    const withoutRequest = purityResolveId('@orygin-ai/dsh-client-ui-goal')
    expect(withoutRequest('@orygin-ai/dsh-client-runtime/client')).toBeNull()
  })
  ...
  it('externalizes the baseline independently of each package manifest', () => {
    ...
    const plain = clientConfigs('@orygin-ai/dsh-client-connection')[0]?.deps as {
    ...
    expect(plain.neverBundle('@orygin-ai/dsh-client-runtime/client')).toBe(true)
  })
})

describe('client bundle module requests', () => {
  it('requests what the declaration lists', () => {
    const requests = requestedExternals('@orygin-ai/dsh-client-fixture', {
      external: ['react', 'react/jsx-runtime', '@orygin-ai/dsh-client-ui-slots'],
    })

    expect([...requests].sort()).toEqual([
      '@orygin-ai/dsh-client-ui-slots', 'react', 'react/jsx-runtime',
    ])
  })

  it('requests nothing when the declaration is absent', () => {
    expect(requestedExternals('@orygin-ai/dsh-client-fixture', {}).size).toBe(0)
  })

  it('rejects a malformed declaration instead of reading past it', () => {
    expect(() => requestedExternals('@orygin-ai/dsh-client-fixture', { external: 'react' }))
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@orygin-ai/dsh-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://dsh.test/plugins/@orygin-ai/dsh-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@orygin-ai/dsh-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@orygin-ai/dsh-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://dsh.test/plugins/@orygin-ai/dsh-client-connection/client.js.map')
