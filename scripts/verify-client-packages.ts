...
const ORYGIN_PREFIX = '@orygin-ai/dsh-'
const CLIENT_WEB = '@orygin-ai/dsh-client-web'
  ...
  for (const pkg of [...facts.packages].sort((left, right) => left.manifest.localeCompare(right.manifest))) {
    ...
    for (const [name, rule] of [...expected].sort(([left], [right]) => left.localeCompare(right))) {
      ...
      violations.push(
        ...
        + ' is a peer-installed ORYGIN relationship; declare it in peerDependencies and devDependencies'
      ...
      for (const name of Object.keys(pkg[section]).sort()) {
        ...
        if (staticInputs.has(name)) {
          ...
          violations.push(
            ...
            + ' dynamic ORYGIN relationships are peer plus dev, and static client inputs are dev-only',
          )
        }
      }
    }
...
function isInternalDsh(name: string): boolean {
  return name === CORDIS || name.startsWith(ORYGIN_PREFIX)
}
