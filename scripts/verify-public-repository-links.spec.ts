...
describe('repository link policy', () => {
  it('rejects encoded and case-varied references to the unavailable repository', () => {
    const unavailableOwner = ['orygin', 'ai'].join('-')
    const unavailableName = ['orygin', 'harness', 'sdk'].join('-')
    ...
    const source = [
      'https://github.com/Braand-web/Orygin-ai-',
  ...
  it('preserves frozen archived Agent Notes', () => {
    const unavailableRepository = ['orygin-ai', 'orygin-ai-sdk'].join('/')
