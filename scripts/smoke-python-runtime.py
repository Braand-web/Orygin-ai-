...
if TYPE_CHECKING:
    from orygin_harness import RunResult
...
# (orygin-ai#2488), and the file must replay on both. Everything else is compared.
...
- id: sessions
  ...
  config:
    root: !!js process.env.ORYGIN_SESSION_ROOT
    compression: 'none'
- id: code-runtime
  name: '@orygin-ai/dsh-code-runtime-worker-thread'
- id: subagents
  name: '@orygin-ai/dsh-subagent'
- id: subagent-spawn-in-process
  name: '@orygin-ai/dsh-subagent-spawn-in-process'
  config:
    providerName: spawn
- id: subagent-tool
  ...
  config:
    provider: spawn
- id: workflow-engine
  name: '@orygin-ai/dsh-workflow-worker-thread'
  config:
    provider: spawn
- id: workflow-tool
  name: '@orygin-ai/dsh-tool-workflow'
- id: cordis-host-runner
  name: '@orygin-ai/dsh-cordis-host-runner'
- id: cordis-tool
  name: '@orygin-ai/dsh-tool-cordis'
"""
FS_SEARCH_CORDIS = """\
- id: sdk-jsonrpc-server
  name: '@orygin-ai/dsh-sdk-jsonrpc-server'
...
- id: fs-search
  name: '@orygin-ai/dsh-tool-fs-search'
  config:
...
def smoke_direct(base_url: str, executable: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="dsh-direct-") as temporary:
        ...
        environment = {
            **os.environ,
            "ORYGIN_CORDIS_CONFIG": str(cordis),
            "ORYGIN_SESSION_ROOT": str(sessions),
            "ORYGIN_CWD": str(root),
            "ORYGIN_API_KEY": "sk-keyless-smoke",
            "ORYGIN_BASE_URL": base_url,
        }
        peer = RuntimePeer([str(executable)], root, environment)
        try:
            peer.send({"jsonrpc": "2.0", "id": "initialize", "method": "initialize", "params": {"cwd": str(root), "provider": "orygin-official", "model": "smoke-model"}})
