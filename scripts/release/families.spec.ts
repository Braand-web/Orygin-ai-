...
describe('release families', () => {
  it('excludes private experimental packages from the dsh release', () => {
    ...
    expect(members.map(member => member.name)).not.toContain('@orygin-ai/dsh-experimental-agent-team')
  })
  ...
  it('bumps private dsh packages without adding release tags', () => {
    ...
    const published = member('packages/core/published', '@orygin-ai/dsh-published')
  ...
  it('names one tag for the whole dsh family and one per vendored package', () => {
    ...
    const cli = member('apps/cli', '@orygin-ai/dsh')
    const cordis = { ...member('vendor/cordis', '@orygin-ai/cordis'), version: '4.0.1' }
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const dsh = releaseFamily('dsh')
    const members = [member('apps/cli', '@orygin-ai/dsh'), { ...member('apps/web', '@orygin-ai/dsh-web-frontend'), version: '0.0.2' }]
  ...
  it('accepts independent vendored versions and rejects an unpublishable one', () => {
    const vendor = releaseFamily('vendor')
    const members = [
      { ...member('vendor/cordis', '@orygin-ai/cordis'), version: '4.0.1' },
      { ...member('vendor/cosmokit', '@orygin-ai/cosmokit'), version: '1.8.2' },
    ]
  ...
  it('requires a current official client build only for dsh artifacts', () => {
    ...
    vi.stubEnv('ORYGIN_CLIENT_COMMIT_HASH', officialEnvironment.ORYGIN_CLIENT_COMMIT_HASH)
    const official = buildFixture(officialEnvironment)
    const defaultBuild = buildFixture({})

    expect(() => { dsh.verifyBuildArtifacts(official) }).not.toThrow()
    expect(() => { dsh.verifyBuildArtifacts(defaultBuild) }).toThrow(/ORYGIN_CLIENT_TITLE/)
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@orygin-ai/dsh-consumer', { dependencies: { '@orygin-ai/dsh-library': 'workspace:^' } }),
      member('packages/a/library', '@orygin-ai/dsh-library'),
      member('packages/a/zebra', '@orygin-ai/dsh-zebra'),
    ]

    expect(dsh.publishOrder(members).order.map(entry => entry.name)).toEqual([
      '@orygin-ai/dsh-library',
      '@orygin-ai/dsh-consumer',
      '@orygin-ai/dsh-zebra',
    ])
  })
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@orygin-ai/dsh-left', { dependencies: { '@orygin-ai/dsh-right': 'workspace:^' } }),
      member('packages/a/right', '@orygin-ai/dsh-right', { dependencies: { '@orygin-ai/dsh-left': 'workspace:^' } }),
    ]
  ...
  it('publishes a peer before its consumer', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@orygin-ai/dsh-consumer', { peerDependencies: { '@orygin-ai/dsh-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@orygin-ai/dsh-zebra'),
    ]
  ...
  it('orders around a peer cycle rather than refusing to publish, and reports the edge it dropped', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@orygin-ai/dsh-left', { peerDependencies: { '@orygin-ai/dsh-right': 'workspace:^' } }),
      member('packages/a/right', '@orygin-ai/dsh-right', { peerDependencies: { '@orygin-ai/dsh-left': 'workspace:^' } }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@orygin-ai/dsh-right', peer: '@orygin-ai/dsh-left' },
    ])
  })
  ...
  it('honours an install edge even when a peer cycle surrounds it', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/base', '@orygin-ai/dsh-base', { peerDependencies: { '@orygin-ai/dsh-consumer': 'workspace:^' } }),
      member('packages/a/consumer', '@orygin-ai/dsh-consumer', {
        dependencies: { '@orygin-ai/dsh-base': 'workspace:^' },
        peerDependencies: { '@orygin-ai/dsh-base': 'workspace:^' },
      }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@orygin-ai/dsh-base', peer: '@orygin-ai/dsh-consumer' },
    ])
  })
  ...
  it('refuses an order that would publish a consumer before a dependency it installs', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@orygin-ai/dsh-alpha', { peerDependencies: { '@orygin-ai/dsh-bravo': 'workspace:^' } }),
      member('packages/a/bravo', '@orygin-ai/dsh-bravo', { peerDependencies: { '@orygin-ai/dsh-charlie': 'workspace:^' } }),
      member('packages/a/charlie', '@orygin-ai/dsh-charlie', { dependencies: { '@orygin-ai/dsh-alpha': 'workspace:^' } }),
    ]
    ...
    expect(() => { dsh.publishOrder(members) }).toThrow(/no publish order honours @orygin-ai\/dsh-charlie -> @orygin-ai\/dsh-alpha/)
  })

  it('ignores devDependencies when ordering', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@orygin-ai/dsh-alpha', { devDependencies: { '@orygin-ai/dsh-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@orygin-ai/dsh-zebra'),
    ]
  ...
  it('applies the harness payload policy to dsh and keeps upstream payloads for vendored packages', () => {
    const dsh = releaseFamily('dsh')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@orygin-ai/dsh-library')
    const vendored = member('vendor/cordis', '@orygin-ai/cordis')
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('dsh').installedEntry).toEqual({ packageName: '@orygin-ai/dsh', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  const sourceShipping = member('vendor/cosmokit', '@orygin-ai/cosmokit', {
    files: ['lib/index.js', 'lib/types/**/*.d.ts', 'src'],
  })
  const buildOutputOnly = member('vendor/cordis', '@orygin-ai/cordis', {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@orygin-ai/dsh-library', { files: ['lib/index.js'] }),
