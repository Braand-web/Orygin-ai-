...
const OWNERSHIP_MARKER_OWNER = 'orygin-ai worktree-local lefthook hooks'
...
const ALLOW_HOOKS_PATH_OVERRIDE = 'ORYGIN_LEFTHOOK_ALLOW_HOOKS_PATH_OVERRIDE'
const REPOSITORY_EXTENSION_PATTERN = '^extensions\\.'
const PAIRING_MERGE_DRIVER_CONFIG = [
  ['merge.dsh-translation-pairing.name', 'Orygin bilingual pairing records'],
  [
  ...
  while (true) {
    try {
      ...
      try {
        ...
        const writeDelay = Number(process.env.ORYGIN_TEST_LEFTHOOK_LOCK_WRITE_DELAY_MS ?? 0)
        if (writeDelay > 0) {
