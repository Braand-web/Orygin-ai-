...
const sharedTheme: Pick<DefaultTheme.Config, 'search' | 'socialLinks' | 'editLink'> = {
  ...
  socialLinks: [
    { icon: 'github', link: 'https://github.com/Braand-web/Orygin-ai-' },
  ],
  editLink: {
    pattern: ({ frontmatter }: PageData) => {
      ...
      return `https://github.com/Braand-web/Orygin-ai-/edit/master/${editSource}`
    },
    text: '在 GitHub 上编辑此页',
  },
}
...
/**
 * The Orygin wordmark, inlined so its `currentColor` fills follow the active
...
/**
 * Navigation-bar title: the Orygin wordmark and the release-stage tag.
