...
- A Orygin-compatible API endpoint and credential
...
Set the credential in the environment. Set `ORYGIN_BASE_URL` as well when the model is served by an OpenAI-compatible proxy rather than the default Orygin endpoint.

```sh
export ORYGIN_API_KEY=sk-your-key-here
# export ORYGIN_BASE_URL=http://127.0.0.1:8000/v1
# export ORYGIN_MODEL=orygin-v4-flash
# export ORYGIN_SYSTEM_PROMPT='You are a helpful software engineer assistant.'
```
...
| Session persistence | Uncompressed JSONL under `ORYGIN_SESSION_ROOT` |
