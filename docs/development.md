...
- Optional: a Orygin API key for the Web, headless, and ACP automation demos and real-API e2e tests.
...
tsdown --env.ORYGIN_BUILD_FACE host
tsc -b tsconfig.client.json
tsdown --env.ORYGIN_BUILD_FACE client
pnpm run build:web
```

Both tsdown passes use the same complete workspace match. They neither scan build artifacts to discover Client packages nor maintain a Host/Client package filter list. Package-local tsdown configs select entries for the current phase through `ORYGIN_BUILD_FACE`: an ordinary Client plugin produces both its Node loader and browser bundle during the Client phase; `api-remotes` uses `hostPhase: true` to produce its Host entry early and only its browser bundle during the Client phase. Tsdown consumes only the JavaScript emitted to `lib/types` by the preceding tsc phase.
...
The real Orygin adapter and key-backed agent demos read credentials from the environment or from a gitignored `.env` at the repo root:

```sh
ORYGIN_API_KEY=sk-...
ORYGIN_BASE_URL=https://... # optional
```

`ORYGIN_BASE_URL` is optional and defaults to the public API. Never commit real credentials. The real-API e2e suites self-skip when `ORYGIN_API_KEY` is not set.
...
The one-shot Headless coding agent needs `ORYGIN_API_KEY` in the environment or repo-root `.env`:
...
The ACP automation server exposes fresh agent sessions over JSON-RPC stdio and also needs `ORYGIN_API_KEY`:
