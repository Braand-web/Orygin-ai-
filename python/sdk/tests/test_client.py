...
from orygin_harness import OryginHarness, HarnessClient, HarnessConfig, Notification, SdkProtocolError
...
json.dump({
    "ORYGIN_API_KEY": os.environ.get("ORYGIN_API_KEY"),
    "ORYGIN_BASE_URL": os.environ.get("ORYGIN_BASE_URL"),
    "ORYGIN_CWD": os.environ.get("ORYGIN_CWD"),
    "ORYGIN_SESSION_ROOT": os.environ.get("ORYGIN_SESSION_ROOT"),
    "ORYGIN_CORDIS_CONFIG": os.environ.get("ORYGIN_CORDIS_CONFIG"),
}, open(env_dump, "w"))
...
""".strip()
    ...
    with OryginHarness(
        ...
        env={
            ...
            "ORYGIN_API_KEY": "env-key",
            "ORYGIN_BASE_URL": "http://127.0.0.1:4321",
        },
    ) as harness:
    ...
    assert dumped_env["ORYGIN_API_KEY"] == "env-key"
    assert dumped_env["ORYGIN_BASE_URL"] == "http://127.0.0.1:4321"
    assert dumped_env["ORYGIN_CWD"] == str(tmp_path)
    assert dumped_env["ORYGIN_SESSION_ROOT"] == str(tmp_path / "sessions")
    assert dumped_env["ORYGIN_CORDIS_CONFIG"] == str(tmp_path / "cordis.yml")
...
for line in sys.stdin:
    ...
    if msg.get("method") == "initialize":
        json.dump({"process": os.getcwd(), "environment": os.environ.get("ORYGIN_CWD"), "wire": msg["params"]["cwd"]}, open(os.environ["CAPTURE"], "w"))
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-runtime"}}}), flush=True)
...
""".strip()
    ...
    with OryginHarness(launch_args_override=(sys.executable, str(script)), cwd=str(tmp_path)) as harness:
...
""".strip()
    ...
    with HarnessClient(
        ...
        init = client.initialize(provider="orygin-official", cwd="/workspace", model="dsagent")
...
""".strip()
    ...
    with HarnessClient(HarnessConfig(launch_args_override=(sys.executable, str(script)))) as client:
        client.initialize(provider="orygin-official", cwd="/workspace", model="dsagent")
        with (
...
""".strip()
    ...
    with pytest.raises(Exception, match="bad initialize"):
        client.initialize(provider="orygin-official", cwd=".", model="dsagent")
...
def test_public_signatures_omit_unsupported_wire_parameters() -> None:
    from orygin_harness import OryginHarnessConfig, Session
    ...
    assert "profile" not in inspect.signature(OryginHarness.run).parameters
    assert "profile" not in inspect.signature(Session.run).parameters
    assert "system_prompt" not in OryginHarnessConfig.__dataclass_fields__
    assert "max_tokens" in OryginHarnessConfig.__dataclass_fields__
...
json.dump({"ORYGIN_CORDIS_CONFIG": os.environ.get("ORYGIN_CORDIS_CONFIG")}, open(os.environ["ENV_DUMP"], "w"))
for line in sys.stdin:
    msg = json.loads(line)
    if msg.get("method") == "initialize":
...
""".strip()
    ...
    module_dir = tmp_path / "orygin_harness_runtime"
    module_dir.mkdir()
...
""".strip()
    ...
    monkeypatch.delitem(sys.modules, "orygin_harness_runtime", raising=False)
...
def test_client_default_launch_uses_bundled_runtime_and_injects_default_config(
    ...
    if ambient_config is None:
        monkeypatch.delenv("ORYGIN_CORDIS_CONFIG", raising=False)
    else:
        monkeypatch.setenv("ORYGIN_CORDIS_CONFIG", ambient_config)

    with HarnessClient(HarnessConfig(env={"ENV_DUMP": str(env_dump)})) as client:
        init = client.initialize(provider="orygin-official", cwd="/workspace", model="orygin-v4-pro")

    assert init.serverInfo.name == "bundled-runtime"
    assert json.loads(env_dump.read_text())["ORYGIN_CORDIS_CONFIG"] == str(default_config)
...
def test_client_respects_explicit_config_over_bundled_default(
    ...
    with HarnessClient(
        HarnessConfig(env={"ENV_DUMP": str(env_dump), "ORYGIN_CORDIS_CONFIG": "./explicit.yml"})
    ) as client:
        client.initialize(provider="orygin-official", cwd="/workspace", model="orygin-v4-pro")

    assert json.loads(env_dump.read_text())["ORYGIN_CORDIS_CONFIG"] == "./explicit.yml"
...
def test_client_reports_missing_bundled_runtime_dependency(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="Install orygin-ai-runtime-bin"):
        HarnessClient().start()
