...
class OryginHarnessConfig:
    ...
    ORYGIN_API_KEY and ORYGIN_BASE_URL settings keep working. Use ``env`` to
    intentionally override or inject variables for a subprocess.
    """

    provider: str = "orygin-official"
    model: str = "orygin-v4-flash"
...
class OryginHarness:
    """Reusable synchronous SDK for running Orygin agent turns.
    ...
    def __init__(self, config: OryginHarnessConfig | None = None, **kwargs: object) -> None:
        if config is not None and kwargs:
            raise TypeError("pass either OryginHarnessConfig or keyword options, not both")
        self.config = config or OryginHarnessConfig(**kwargs)
        ...
        if self.config.session_root is not None:
            env["ORYGIN_SESSION_ROOT"] = self.config.session_root
        if self.config.cordis is not None:
            env["ORYGIN_CORDIS_CONFIG"] = self.config.cordis
        env["ORYGIN_CWD"] = cwd
        if self.config.base_url is not None:
            env["ORYGIN_BASE_URL"] = self.config.base_url
        if self.config.api_key is not None:
            env["ORYGIN_API_KEY"] = self.config.api_key
    ...
    def __enter__(self) -> "OryginHarness":
        self.start()
        return self
...
class Session:
    def __init__(self, harness: OryginHarness, session_id: str) -> None:
