...
class HarnessConfig:
    """Configuration for launching the local Orygin SDK runtime."""
...
class HarnessClient:
    """Synchronous JSON-RPC client for the Orygin SDK runtime over stdio."""
    ...
    def close(self) -> None:
        ...
        try:
            ...
            proc.wait()
        self._proc = None
        self._fail_waiters(self._runtime_closed_error("Orygin runtime closed"))
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    if remaining <= 0:
                        ...
                        raise TimeoutError(
                            f"{method} timed out waiting for Orygin runtime{suffix}"
                        )
    ...
    def _write_message(self, message: JsonObject) -> None:
        ...
        if proc is None or proc.stdin is None:
            raise TransportClosedError("Orygin runtime is not running")
        try:
            ...
            raise self._runtime_closed_error("Failed to write to Orygin runtime") from exc
    ...
    def _reader_loop(self) -> None:
        ...
        try:
            ...
            self._fail_waiters(self._runtime_closed_error("Orygin runtime stdout closed"))
    ...
    def _default_launch_args(self) -> tuple[str, ...]:
        ...
        try:
            ...
            raise FileNotFoundError(
                "Unable to locate the bundled Orygin SDK runtime. "
                "Install orygin-ai-runtime-bin or set HarnessConfig.runtime_bin."
            ) from exc
    ...
    def _inject_bundled_default_config(self, env: dict[str, str]) -> None:
        ...
        if not uses_bundled_runtime or env.get("ORYGIN_CORDIS_CONFIG"):
            return
        # _default_launch_args already imported the package or raised its install error.
        from orygin_harness_runtime import bundled_default_config_path

        env["ORYGIN_CORDIS_CONFIG"] = str(bundled_default_config_path())
