"""Locate the bundled Orygin SDK runtime shipped with this package.
...
- **node (dev-only)**: the full deploy closure under ``runtime/node/``
  ...
  runtime/node/node_modules/@orygin-ai/dsh-sdk-jsonrpc-demo/lib/packaged-bin.js`` on a
...
the client SDK injects via ``$ORYGIN_CORDIS_CONFIG`` for zero-config runs — the
...
PACKAGE_METADATA_FILENAME = "orygin-ai-runtime.json"

RUNTIME_MODE_ENV_VAR = "ORYGIN_RUNTIME_MODE"
...
_EXE_ACQUISITION_HINT = (
    ...
    "orygin-ai checkout, or install the matching `orygin-ai-runtime-bin` platform "
...
def bundled_package_dir() -> Path:
    ...
    if not metadata.is_file():
        raise FileNotFoundError(f"orygin-ai-runtime-bin is missing {metadata}")
    return root
...
def bundled_default_config_path() -> Path:
    ...
    The client SDK injects this path via ``$ORYGIN_CORDIS_CONFIG`` when the caller
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"orygin-ai-runtime-bin is missing the default runtime config at {path}"
        )
    return path
...
def bundled_runtime_path() -> Path:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"orygin-ai-runtime-bin is missing the runtime executable at {path}. "
            + _EXE_ACQUISITION_HINT
        )
    ripgrep = Path(f"{path}-rg")
    if not ripgrep.is_file():
        raise FileNotFoundError(
            f"orygin-ai-runtime-bin is missing the ripgrep sidecar at {ripgrep}. "
            + _EXE_ACQUISITION_HINT
        )
    if tag.startswith("macos-"):
        ...
        if not helper.is_file():
            raise FileNotFoundError(
                f"orygin-ai-runtime-bin is missing the node-pty spawn helper at {helper}. "
                + _EXE_ACQUISITION_HINT
            )
    return path
...
def resolve_bundled_launch_args(mode: str | None = None) -> tuple[str, ...]:
    ...
    ``ORYGIN_RUNTIME_MODE`` environment variable (``exe`` | ``node``), then
    ...
    raise ValueError(
        f"unsupported Orygin runtime mode {selected!r}: expected 'exe' or 'node' "
...
def _node_launch_args() -> tuple[str, str]:
    ...
    if not bin_js.is_file():
        raise FileNotFoundError(
            ...
            f"(no {bin_js}); run `scripts/build-exe-for-python-sdk.ts` in a orygin-ai "
