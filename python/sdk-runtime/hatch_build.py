...
def _host_platform_tag() -> str:
    ...
    try:
        ...
        raise RuntimeError(f"unsupported orygin-ai-runtime-bin build platform: {key}") from exc
...
class RuntimeBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict[str, object]) -> None:
        ...
        if self.target_name == "sdist":
            raise RuntimeError(
                "orygin-ai-runtime-bin is wheel-only; build and publish platform wheels only."
            )

        platform_tag = os.environ.get("ORYGIN_RUNTIME_PLATFORM_TAG") or _host_platform_tag()
        ...
        if len(matches) != 1:
            ...
            raise RuntimeError(
                f"unsupported ORYGIN_RUNTIME_PLATFORM_TAG {platform_tag!r}; expected one of {supported}"
            )
        expected_executable = matches[0][1]
        runtime_dir = Path(self.root) / "src" / "orygin_harness_runtime" / "runtime"
