...
Orygin is a plugin-based agent harness on vendored Cordis: **everything is a plugin**. Read [docs/architecture.md](docs/architecture.md) before changing `packages/`; follow [docs/AGENTS.md](docs/AGENTS.md) for documentation.
...
packages/    @orygin-ai/dsh-<pkg> workspaces at packages/<group>/<pkg>/
  core/        product API spine: session, system-prompt, tools, agent, agent-loop
  api/         Remote BFF assembly and Typert RPC gateway
  typert/      type graph generator, loader, and runtime registry
  llm/         LLM capability: Service Definition/Consumer + Orygin providers
...
native/      @orygin-ai/node-addon-landlock-run source of record (see native/README.md)
...
pnpm run test:e2e       # real-API tests; self-skip without ORYGIN_API_KEY
...
pnpm dsh --profile headless "task"  # run one task from source (needs ORYGIN_API_KEY)
pnpm run demo:cordis    # the agent modifies its own runtime (needs key)
pnpm run demo:acp       # ACP automation server (needs ORYGIN_API_KEY)
```
...
Real-API tests and demos read `ORYGIN_API_KEY`, optional `ORYGIN_BASE_URL`, and root `.env`. cordis.yml allows `!!js` (never `!js`) under plugin `config` and entry `disabled`; other metadata stays literal, so conditional composition also uses overlays ([primer](docs/cordis-primer.md#loader-configuration)). Never commit credentials. CI e2e skips without a key; [testing.md](docs/testing.md) owns key policy.

## Conventions

- Every npm package is `@orygin-ai/dsh-<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@orygin-ai/cordis` is a peerDependency (+ dev) of every harness package.
