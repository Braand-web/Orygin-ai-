...
async function harness(): Promise<{ ctx: Context; agent: Agent }> {
  ...
  await ctx.plugin(LlmOrygin, { models: [{ id: 'orygin-v4-flash' }] })
...
describe.skipIf(!process.env.ORYGIN_API_KEY)('workspace context e2e: real model sees AGENTS.md baseline', () => {
