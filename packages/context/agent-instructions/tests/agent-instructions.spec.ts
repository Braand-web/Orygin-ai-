...
import LlmRuntime, { createUserMessage, CallId, type Message, type StreamChunk } from '@orygin-ai/dsh-llm'
import SessionStore, { Session, SessionId, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@orygin-ai/dsh-session'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@orygin-ai/dsh-agent'
import AgentLoop from '@orygin-ai/dsh-agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@orygin-ai/dsh-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
describe('workspace context instruction discovery', () => {
  ...
  it('defaults dshHome and uses cwd itself as root when no project marker exists', async () => {
    ...
    // Isolate the default-home fallback: blank ORYGIN_HOME is treated as unset, and
  ...
  it('honors ORYGIN_HOME when dshHome is not configured explicitly', async () => {
    const root = await tempRepo()
    const envHome = await tempRepo()
    try {
      await write(join(envHome, 'AGENTS.md'), 'env global rule')
      vi.stubEnv('ORYGIN_HOME', envHome)

      const files = await discoverBaselineInstructionFiles({ cwd: root })

      expect(files).toEqual([{ absolutePath: join(envHome, 'AGENTS.md'), displayPath: '$ORYGIN_HOME/AGENTS.md' }])
    } finally {
      vi.unstubAllEnvs()
      await rm(root, { recursive: true, force: true })
  ...
  it('labels the default ORYGIN home as ~/.dsh when HOME points at the configured default', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.dsh/AGENTS.md'), 'global default rule')

      // A set ORYGIN_HOME would override the homedir default and relabel the home.
      vi.stubEnv('ORYGIN_HOME', '')
      vi.resetModules()
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@orygin-ai/dsh-agent-instructions')
  ...
  it('deduplicates user-global instructions when dshHome points at the project root', async () => {
    ...
    try {
      ...
      expect(files).toEqual([{ absolutePath: join(root, 'AGENTS.md'), displayPath: '$ORYGIN_HOME/AGENTS.md' }])
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
...
describe('workspace context request injection', () => {
  ...
  it('labels a custom dshHome as ORYGIN_HOME instead of pretending it is ~/.dsh', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      ...
      expect(files.map(file => file.displayPath)).toEqual(['$ORYGIN_HOME/AGENTS.md'])
    } finally {
      await rm(root, { recursive: true, force: true })
      await rm(home, { recursive: true, force: true })
    }
