...
describe('BootPage', () => {
  ...
  it('lists failed entries', () => {
    const { el, page } = mount()
    page.setState('@orygin-ai/dsh-client-ui-layout', 'failed')
    page.setState('ok', 'active')
    page.setState('@orygin-ai/dsh-client-ui-tool', 'failed')
    expect(el.textContent).toContain('@orygin-ai/dsh-client-ui-layout')
    expect(el.textContent).toContain('@orygin-ai/dsh-client-ui-tool')
