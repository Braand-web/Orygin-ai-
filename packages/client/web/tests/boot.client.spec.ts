...
import * as modulesClient from '@orygin-ai/dsh-client-modules/client'
import type {
...
const MODULES_ID = '@orygin-ai/dsh-client-modules'
...
describe('bootstrap failure rendering', () => {
  ...
  it('renders a malformed boot manifest', async () => {
    await expectBootFailure(() => {
      installFacade()
      delete win.__ORYGIN_BOOT__
    }, 'window.__ORYGIN_BOOT__ is missing or not an object')
  })
  ...
  it('renders a module-system construction failure', async () => {
    await expectBootFailure(() => {
      ...
      win.__ORYGIN_BOOT__ = { rev: 'graph', entries: [duplicate, duplicate] }
...
describe('plugin activation', () => {
  it('allows a modules-dependent row to be created before the modules row', async () => {
    ...
    win.__ORYGIN_BOOT__ = { rev: 'graph', entries }
