 ...
 * @module @orygin-ai/dsh-client-web/src/boot
 */
...
} from '@orygin-ai/dsh-client-modules/client'
import type {} from '@orygin-ai/dsh-client-ui-renderer/client'
...
export class AppWebEntry {
  ...
  async run(): Promise<void> {
    try {
      ...
      // is `ClientTransportHooks`, owned by @orygin-ai/dsh-client-connection;
      // this structural slice reads one optional member without adding a
      // package edge.
      const transport = (globalThis as {
        __ORYGIN_TRANSPORT__?: { loadBundle?: ClientModuleCreateOptions['loadBundle'] }
      }).__ORYGIN_TRANSPORT__
      this.modules = moduleLoader.create({
        boot: win.__ORYGIN_BOOT__,
  ...
  private async prefetchImmediateTier(): Promise<void> {
    ...
    const transport = (globalThis as {
      __ORYGIN_TRANSPORT__?: { loadBundle?: unknown }
    }).__ORYGIN_TRANSPORT__
