...
import type {} from '@orygin-ai/dsh-plan-mode/client'
// Type-only: the `goal` projection key merge (hint disambiguation).
import type {} from '@orygin-ai/dsh-goal/client'
...
import type { Translate } from '@orygin-ai/dsh-client-ui-slots'
...
export function InputBar({
  ...
  // Intake pre-check (Orygin Chat semantics): an addition that would break
  ...
  const intakeImages = useCallback((files: readonly File[]): void => {
    ...
    const rejected = ((): string | null => {
      if (imageLimits !== undefined) {
        // Format precedes limits (Orygin Chat's filter order): a batch with
