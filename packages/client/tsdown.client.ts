 ...
 * Everything else under @orygin-ai/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@orygin-ai\/dsh-(host-apiproxy|file-reference|session|llm|tools|brand)(\/|$)/
...
const GENERATED_REMOTE = /^@orygin-ai\/dsh-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
...
export function clientBundle(
  ...
  return ({ env }) => {
    const face = buildFace(env?.ORYGIN_BUILD_FACE)
...
export function clientOnly(configs: readonly UserConfig[]): BuildFaceConfig {
  return ({ env }) => buildFace(env?.ORYGIN_BUILD_FACE) === 'host'
...
function buildFace(value: unknown): BuildFace {
  ...
  throw new Error(`tsdown: --env.ORYGIN_BUILD_FACE must be host or client, received ${String(value)}`)
}
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @orygin-ai value import is a build error — a
      ...
      resolveId(source: string) {
        if (!source.startsWith('@orygin-ai/')) return null
