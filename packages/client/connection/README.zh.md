...
托管版 Orygin 外壳在该载体之上增加认证。登出状态下的读取使用固定的浏览器本地访客投影；受保护操作请求登录；已认证 WebSocket 会把 bearer token 交换为短时、单次使用的 ticket。若浏览器中保存的 bearer 已过期，安全启动读取与下行流会在请求重新登录后回退到访客投影，而不是让连接控制器陷入确定性的重试循环。
