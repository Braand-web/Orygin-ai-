...
import {
  ...
  createUserMessage,
  isTokenDelta,
} from '@orygin-ai/dsh-llm/message'
import { CallId } from '@orygin-ai/dsh-llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import type { AttachmentIdType, ImageAttachmentRef } from '@orygin-ai/dsh-attachment'
import type {
  SessionEvent,
  SessionId,
  TodoItem,
} from '@orygin-ai/dsh-session/types'
// Type-only: the brand constructor is host-side; the fixture casts at its
// wire-fabrication boundary (the schema layer's one-cast-point posture).
import type { CommandId } from '@orygin-ai/dsh-commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@orygin-ai/dsh-commands/types'
import { deriveEventMessage, foldSurface } from '@orygin-ai/dsh-session/surface'
import type {
  ApiProxy, ClientRequest, ClientResponse, HistoryEntry, HostFrame, MuxFrame, RpcReceipt,
  ModelProviderGroup, ModelSelection, RpcRequest, RpcResponse, RpcResult, ServerRequest, ServerResponse, SessionSummary,
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView,
} from './api.ts'
import type { RequestPayload, ResponseValue, RpcMethodMap } from '@orygin-ai/dsh-host-apiproxy/api'
...
const WEB_SEARCH_RESULT: Omit<Extract<ToolResultView, { card: 'web'; kind: 'search' }>, 'card' | 'kind'> = {
  answer: 'Orygin is a plugin-based agent harness on vendored Cordis where **every capability is a plugin**.',
  sources: [
    {
      url: 'https://github.com/Braand-web/Orygin-ai-',
      title: 'Orygin — plugin-based agent harness',
      snippet: 'Everything is a plugin: session, tools, agent-loop, and LLM adapters all mount on the same Cordis context.',
      publishedAt: '2026-07-01',
    },
    {
      url: 'https://www.orygin.fun/blog/harness-architecture',
      snippet: 'The capability-seam pattern splits each capability into interface, implementation, and consumer packages.',
    },
    {
      url: 'https://docs.orygin.fun/harness/plugins',
...
function buildAlphaLog(): SessionEvent[] {
  ...
  push({
    ...
    data: { provider: 'orygin-official', model: 'orygin-v4-flash', contextWindow: 128_000 },
  })
  ...
  toolTurn(70, 'web_search', '{"queries":["orygin harness architecture"]}', 'Search results for orygin harness architecture.')
  toolTurn(71, 'web_fetch', '{"url":"https://www.orygin.fun/blog/harness-architecture"}', '# Harness architecture\n\nEverything is a plugin.')
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const modelSelections = new Map<SessionId, ModelSelection>(sessions.map(session => [
    session.sessionId,
    { provider: 'orygin-official', model: 'orygin-v4-flash' },
  ]))
  ...
  const fixtureCredentials = new Map<string, true>([
    ...
    // Orygin route so unrelated GUI journeys do not enter first-run setup.
    ['ORYGIN_API_KEY', true],
  ])
  /**
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ['standard', { trust: 'system', content: "- id: tool-bash\n  name: '@orygin-ai/dsh-tool-bash'\n" }],
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@orygin-ai/dsh-tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@orygin-ai/dsh-tool-read'\n" }],
  ])
  ...
  const directoryTree = new Map<string, string[]>([
    ...
    [`${FIXTURE_HOME}/Documents`, [
      'project', 'orygin-iOS', 'orygin-android', 'orygin-platform',
      'orygin-web', 'orygin-ai', 'orygin-app', 'orygin-landing-blog',
    ]],
  ])
  ...
  const api: ApiProxy = {
    sessions: {
      ...
      create: async (request) => {
        ...
        modelSelections.set(created.sessionId, { provider: 'orygin-official', model: 'orygin-v4-flash' })
        attachedSessions += 1
      ...
      prompt: (request) => {
        ...
        const selection = modelSelections.get(id) ?? { provider: 'orygin', model: 'orygin-v4-flash' }
    ...
    settings: {
      // Only the resolved Orygin address needed by first-run readiness is
      ...
      describe: request => ok(request, {
        ...
        namespaces: [{
          ns: 'llm-orygin',
          schema: {},
          value: { apiKeyEnv: 'ORYGIN_API_KEY' },
          applies: 'live',
