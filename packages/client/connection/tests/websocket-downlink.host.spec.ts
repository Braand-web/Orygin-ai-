...
async function serve(downlinks: WebSocketDownlinks): Promise<{
  ...
  server.on('upgrade', (request, socket, head) => {
    ...
    if (pathname === MUX_EVENTS_PATH) downlinks.handleMux(request, socket, head, LOCAL_AUTH_PRINCIPAL)
    else if (pathname === HOST_EVENTS_PATH) downlinks.handleHost(request, socket, head, LOCAL_AUTH_PRINCIPAL)
    else socket.destroy()
  })
