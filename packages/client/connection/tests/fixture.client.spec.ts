...
describe('createFixtureApi', () => {
  ...
  it('serves grouped models and keeps a selection for later history and fixture requests', async () => {
    ...
    expect(catalog.result.value.groups.map(group => group.name)).toEqual(['Orygin', 'OpenAI'])
  ...
  it('serves configured Orygin readiness and keeps credential values write-only', async () => {
    const api = createFixtureApi()
    ...
    const initial = await api.credentials.describe(req({ refs: ['ORYGIN_API_KEY', 'TEST_API_KEY'] }))
    if (!initial.result.ok) throw new Error('credential describe failed')
    expect(initial.result.value.credentials).toEqual({
      ORYGIN_API_KEY: { configured: true, source: 'file', writable: true },
