/**
 * Package-owned invariant companion for `@orygin-ai/dsh-client-modules`.
 * @module @orygin-ai/dsh-client-modules/invariant
 */
...
const PACKAGE_NAME = '@orygin-ai/dsh-client-modules'
  ...
  ctx.on('internal/plugin', () => {
    ...
    for (const row of host.graph().entries) {
      if (host.clientPath(row.id) === undefined) {
        fail(`web plugin graph row "${row.id}" advertises ${row.url} but resolves no client bundle path — the served __ORYGIN_BOOT__ would 404 on fetch`)
      }
    }
  }, { global: true })
}
