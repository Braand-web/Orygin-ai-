...
- **自身不维护卸载记录**：样式移除与 fiber 拆卸顺序属于 HMR 驱动器（`@orygin-ai/dsh-client-hmr`）；loader 只在每条记录中登记其拥有的样式标签 id。
