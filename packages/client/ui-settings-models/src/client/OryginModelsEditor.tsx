/**
 * Curated editor for the direct Orygin adapter's advisory model catalog.
 ...
 * (rejected by {@link validateOryginModels} before any write).
 */
...
export interface OryginModelsValidationFailure {
...
export function modelDrafts(value: unknown): OryginModelDraft[] {
  if (!Array.isArray(value)) return []
  return value.map(entry =>
    typeof entry === 'object' && entry !== null && !Array.isArray(entry)
      ? entry as OryginModelDraft
      : {})
}
 ...
 */
export function validateOryginModels(value: unknown): OryginModelsValidationFailure | undefined {
...
/** Props of {@link OryginModelsEditor}. */
export interface OryginModelsEditorProps {
  /** Effective rows: inherited until the parent materializes an override. */
  models: readonly OryginModelDraft[]
  ...
  onChange: (models: OryginModelDraft[]) => void
  /** Remove the user-owned array and return to inheritance. */
  onReset: () => void
}

/**
 * Render the direct Orygin adapter's model catalog: id and display name on
 * each row, capacities behind the row's own disclosure.
 * @param props - effective rows plus the array-level override actions.
 * @returns the catalog editor.
 */
export function OryginModelsEditor(props: OryginModelsEditorProps): ReactNode {
  ...
  const capacityText = (model: OryginModelDraft, index: number, field: CapacityField): string => {
