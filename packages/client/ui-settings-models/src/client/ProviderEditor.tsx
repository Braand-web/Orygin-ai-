 ...
 * both families, Orygin's id/name/context-window model catalog, and the
...
import type { CredentialView, IApiClient, SettingsNamespaceView, SettingsPathOpView } from '@orygin-ai/dsh-api-remotes/client'
import {
  OryginModelsEditor, modelDrafts, validateOryginModels,
} from './OryginModelsEditor.tsx'
...
type EditorLayout = 'orygin' | 'pi-ai' | 'unknown'

/** The public Orygin endpoint shown as the orygin base-URL placeholder. */
const ORYGIN_PUBLIC_BASE_URL = 'https://api.orygin.fun'
...
function layoutOf(ns: string): EditorLayout {
  if (ns === 'llm-orygin') return 'orygin'
...
export function ProviderEditor(props: ProviderEditorProps): ReactNode {
  ...
  const modelFailure = validateOryginModels(schema.getPath(draft, ['models']))
  ...
  const applyOnce = async (): Promise<string | undefined> => {
    ...
    if (props.credentialOnly !== true) {
      ...
      const failure = validateOryginModels(schema.getPath(next, ['models']))
   ...
   */
  const curatedFields = (family: 'orygin' | 'pi-ai'): ReactNode => {
    // What a hand-declared route names for itself and nothing else can supply.
    // A whole-section `llm-orygin` profile is a composition fact with no
          ...
          <div className={styles['customizedBody']}>
            ...
            <div className={styles['field']}>
              ...
              <input
                ...
                placeholder={family === 'orygin'
                  ? ORYGIN_PUBLIC_BASE_URL
            ...
            {/* Both families edit the same rows through the same contract; only
                the extras differ — Orygin's inherited capacities, pi-ai's
