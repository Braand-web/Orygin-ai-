...
/** First-run Orygin prompt behavior over the shared Models join. */
...
import { OryginOnboardingDialog } from '../src/client/OryginOnboardingDialog.tsx'
import type { OryginOnboardingDialogProps } from '../src/client/OryginOnboardingDialog.tsx'
...
function oryginNamespace(apiKeyEnv: string | null): SettingsNamespaceView {
...
function harness(options: {
  ...
  const apiKeyEnv = options.apiKeyEnv === undefined ? 'ORYGIN_API_KEY' : options.apiKeyEnv
  const mutate = vi.fn(() => Promise.resolve(ok(oryginNamespace(apiKeyEnv))))
  ...
  const face = {
        ...
        return Promise.resolve(ok({
          providers: options.provider === false
            ...
            : [{
              ...
              displayName: 'Orygin',
              settingsNs: options.providerSettingsNs ?? 'llm-orygin',
              settingsPath: [],
    ...
    settings: {
      describe: () => Promise.resolve(ok({
        ...
        hasDocument: false,
        namespaces: options.settingsNamespace === false ? [] : [oryginNamespace(apiKeyEnv)],
      })),
      mutate,
    },
    credentials: {
  ...
  const props: OryginOnboardingDialogProps = {
    stepId: 'orygin-official',
    complete,
    openSection,
...
describe('OryginOnboardingDialog', () => {
  it('renders when the shell root is absent', async () => {
    const h = harness()
    document.getElementById('root')!.remove()
    render(<OryginOnboardingDialog {...h.props} />)
  ...
  it('cannot be dismissed implicitly and restores the previous inert state', async () => {
    ...
    appRoot.inert = true
    const view = render(<OryginOnboardingDialog {...h.props} />)
  ...
  it('does not block the product when Orygin setup is unavailable', async () => {
    for (const h of [
