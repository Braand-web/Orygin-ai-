...
import Schema from '@orygin-ai/schemastery'
import { bindSnapshotSelector } from '@orygin-ai/dsh-client-test-runtime'
import type { RpcResponse, SettingsNamespaceView } from '@orygin-ai/dsh-api-remotes/client'
import {
...
import {
  OryginModelsEditor, formatCapacity, modelDrafts, parseCapacity, validateOryginModels,
} from '../src/client/OryginModelsEditor.tsx'
import { apiKeyFailure } from '../src/client/apiKey.ts'
import { SettingsDescribeMirror } from '@orygin-ai/dsh-client-ui-settings/src/client/settings-mirror.ts'
...
const ORYGIN_TARGET = { provider: 'orygin-official', displayName: 'Orygin' }
const oryginCopy = (template: string): string => providerCopy(template, ORYGIN_TARGET)
...
const OryginConfig = Schema.object({
  apiKeyEnv: Schema.string().role('credential-ref'),
...
const DEFAULT_ORYGIN_MODELS = [
  ...
  { id: 'orygin-v4-pro', name: 'Orygin-V4-Pro', contextWindow: 1_000_000 },
]
...
function wireNamespaces(): SettingsNamespaceView[] {
  return [
    {
      ns: 'llm-orygin',
      schema: JSON.parse(JSON.stringify(OryginConfig.toJSON())) as unknown,
      value: {
        apiKeyEnv: 'ORYGIN_API_KEY',
        baseURL: 'https://base',
        defaultContextWindow: 1_000_000,
        maxTokens: 256_000,
        models: DEFAULT_ORYGIN_MODELS,
      },
      base: { defaultContextWindow: 1_000_000, maxTokens: 256_000, models: DEFAULT_ORYGIN_MODELS },
  ...
  const face = {
    llm: {
      providers: vi.fn(() => Promise.resolve(ok({
        providers: [
          { provider: 'orygin-official', displayName: 'Orygin', settingsNs: 'llm-orygin', settingsPath: [], active: true },
 ...
 * anywhere, so the whole-section Orygin route owns the first-run setup card.
 */
...
/**
 * Mount and open the Orygin editor. The shared fixture already has a usable
 * openai route, so Orygin is an ordinary row whose card opens through Edit
 * rather than by itself.
 */
async function mountOryginCard(overrides: Parameters<typeof scriptedFace>[0] = {}) {
  const mounted = await mountSection(overrides)
  fireEvent.click(screen.getByRole('button', { name: oryginCopy(en.editProvider) }))
  return mounted
}
...
describe('ModelsSection', () => {
  ...
  it('renders the unkeyed whole-section provider as an open setup card in the first-run posture', async () => {
    await mountFirstRun()
    // Nothing is reachable yet, and Orygin has no configured credential and
    // no stored apiKey → setup card.
    expect(screen.getByText('Orygin')).toBeTruthy()
  ...
  it('leaves the unkeyed provider a plain row once another provider is usable', async () => {
    ...
    expect(missing.closest('li')?.textContent).toContain('Orygin')
  ...
  it('decides setup need from the joined credential state and the first-run posture', () => {
    const entry = { provider: 'p', displayName: 'p', settingsNs: 'llm-orygin', settingsPath: [], active: true }
  ...
  it('uses one stable provider identity in action copy', () => {
    const target = { provider: 'orygin-official', displayName: 'Orygin' }
    expect(providerTargetLabel(target)).toBe('Orygin (orygin-official)')
    expect(providerCopy(en.deleteTitle, target)).toBe('Delete Orygin (orygin-official)?')
  ...
  it('stores a typed key write-only from the setup card without touching settings', async () => {
    ...
    await waitFor(() => { expect(set).toHaveBeenCalledWith({ ref: 'ORYGIN_API_KEY', value: 'sk-live' }) })
    ...
    expect((await screen.findByRole('status')).textContent).toBe(
      providerCopy(en.savedProvider, { provider: 'orygin-official', displayName: 'Orygin' }),
    )
  ...
  it('reuses the provider editor as a required credential-only onboarding form', async () => {
    ...
    expect(set).toHaveBeenCalledWith({ ref: 'ORYGIN_API_KEY', value: 'sk-onboarding' })
  ...
  it('applies customized orygin fields as path ops', async () => {
    const { mutate } = await mountOryginCard({
      mutate: vi.fn(() => Promise.resolve(ok(wireNamespaces()[0]))),
    })
    fireEvent.click(screen.getByText(en.customized))
    const baseURL = screen.getByLabelText<HTMLInputElement>(en.baseUrl)
    // The orygin placeholder is pinned to the public endpoint, not the
    // effective value (which may reflect a launch-environment override).
    expect(baseURL.placeholder).toBe('https://api.orygin.fun')
  ...
  it('materializes inherited models and adds an arbitrary Orygin id', async () => {
    const { mutate } = await mountOryginCard({
      mutate: vi.fn(() => Promise.resolve(ok(wireNamespaces()[0]))),
    })
    fireEvent.click(screen.getByText(en.customized))
    ...
    expect(screen.getAllByLabelText(new RegExp(en.modelId)).map(input => (input as HTMLInputElement).value))
      .toEqual(['orygin-v4-flash', 'orygin-v4-pro'])
  ...
  it('rejects duplicate Orygin model ids before writing', async () => {
    const { mutate } = await mountOryginCard()
    fireEvent.click(screen.getByText(en.customized))
    fireEvent.click(screen.getByText(en.addModel))
    const ids = screen.getAllByLabelText(new RegExp(en.modelId))
    fireEvent.change(ids[2] as HTMLInputElement, { target: { value: 'orygin-v4-flash' } })
    fireEvent.click(screen.getByText(en.apply))
  ...
  it('validates every adapter-owned model catalog invariant', () => {
    ...
    expect(validateOryginModels([{}])).toEqual({ index: 0, key: 'modelIdRequired' })
    expect(validateOryginModels([{ id: 'same' }, { id: 'same' }]))
      .toEqual({ index: 1, key: 'modelIdDuplicate' })
    expect(validateOryginModels([{ id: 'model', name: '' }]))
      .toEqual({ index: 0, key: 'modelNameInvalid' })
    expect(validateOryginModels([{ id: 'model', contextWindow: null }]))
      .toEqual({ index: 0, key: 'modelContextInvalid' })
    expect(validateOryginModels([{ id: 'model', contextWindow: 1.5 }]))
      .toEqual({ index: 0, key: 'modelContextInvalid' })
    expect(validateOryginModels([{ id: 'model', contextWindow: 0 }]))
      .toEqual({ index: 0, key: 'modelContextInvalid' })
    expect(validateOryginModels([{ id: 'model', contextWindow: 1 }])).toBeUndefined()
    expect(validateOryginModels([{ id: 'model', maxTokens: null }]))
      .toEqual({ index: 0, key: 'modelMaxTokensInvalid' })
    expect(validateOryginModels([{ id: 'model', maxTokens: 1.5 }]))
      .toEqual({ index: 0, key: 'modelMaxTokensInvalid' })
    expect(validateOryginModels([{ id: 'model', maxTokens: 0 }]))
      .toEqual({ index: 0, key: 'modelMaxTokensInvalid' })
    expect(validateOryginModels([{ id: 'model', maxTokens: 8192 }])).toBeUndefined()
  })
  ...
  it('accepts a suffixed context window and stores the plain count', async () => {
    ...
    expect(mutate.mock.calls[0]?.[0]).toEqual({
      ...
      ops: [{
        ...
        path: ['models'],
        value: [
          { ...DEFAULT_ORYGIN_MODELS[0], contextWindow: 1_000_000 },
          { ...DEFAULT_ORYGIN_MODELS[1], contextWindow: 256_000 },
        ],
      }],
      expectedRevision: 0,
    })
  ...
  it.each([
    ...
    expect(screen.getByText(en.modelsInherited)).toBeTruthy()
    expect(screen.getAllByLabelText(new RegExp(en.modelId)).map(input => (input as HTMLInputElement).value))
      .toEqual(base === undefined ? ['orygin-v4-flash', 'orygin-v4-pro'] : ['pinned-by-deployment'])
  })
  ...
  it('edits an output cap per model and carries its text across a removal', async () => {
    ...
    expect(mutate.mock.calls[0]?.[0]).toEqual({
      ...
      ops: [{
        op: 'set',
        path: ['models'],
        value: [{ ...DEFAULT_ORYGIN_MODELS[1], maxTokens: 64_000 }],
      }],
      expectedRevision: 0,
    })
  })
  ...
  it('settles a pasted id and refuses whitespace that would never match', async () => {
    ...
    fireEvent.change(ids[0] as HTMLInputElement, { target: { value: '  orygin-v4-flash  ' } })
    fireEvent.blur(ids[0] as HTMLInputElement)
    expect((ids[0] as HTMLInputElement).value).toBe('orygin-v4-flash')
    ...
    expect(validateOryginModels([{ id: '   ' }])).toEqual({ index: 0, key: 'modelIdRequired' })
    expect(validateOryginModels([{ id: 'model' }, { id: 'model ' }]))
      .toEqual({ index: 1, key: 'modelIdDuplicate' })
  })

  it('renders malformed draft fallbacks without inventing catalog values', () => {
    render(<OryginModelsEditor
      models={[{}]}
      overridden={false}
  ...
  it('can empty and reset the model override, then clear optional fields without dropping hidden data', async () => {
    ...
    expect(mutate.mock.calls[0]?.[0]).toEqual({
      ...
      ops: [{
        ...
        path: ['models'],
        value: [
          { id: 'orygin-v4-flash', description: 'Preserved hidden detail' },
          DEFAULT_ORYGIN_MODELS[1],
        ],
      }],
      expectedRevision: 0,
    })
  ...
  it('clears an inherited override with an unset op, never a whole-section replace', async () => {
    ...
    const { replace, update, mutate } = await mountOryginCard()
    fireEvent.click(screen.getByText(en.customized))
  ...
  it('pins the orygin placeholder and clears typed input back to inherited', async () => {
    const { face } = scriptedFace()
  ...
  it('rejects an invalid draft before writing', async () => {
    const { update } = await mountOryginCard()
    fireEvent.click(screen.getByText(en.customized))
  ...
  it('tells the user to reopen when another writer moved the namespace first', async () => {
    ...
    const { set } = await mountOryginCard({
  ...
  it('keeps the card usable when the write rejects instead of answering', async () => {
    ...
    await mountOryginCard({ mutate: vi.fn(() => Promise.reject(new Error('connection lost'))) })
    fireEvent.click(screen.getByText(en.customized))
  ...
  it('surfaces a shadowed credential write on the card', async () => {
    await mountFirstRun({
      set: vi.fn(() => Promise.resolve(fail('credentials: ORYGIN_API_KEY is shadowed by the read-only environment', 'credential-rejected'))),
    })
    const key = screen.getByLabelText<HTMLInputElement>(en.keyInput)
    fireEvent.change(key, { target: { value: 'sk-live' } })
    fireEvent.click(screen.getByText(en.apply))
  ...
  it('collapses the setup card on cancel without disturbing another open card', async () => {
    ...
    // …and Orygin collapsed to an ordinary row carrying the missing-key dot.
    expect(screen.getAllByLabelText(en.keyInput)).toHaveLength(1)
    expect(screen.getAllByRole('img', { name: en.credentialMissing })
      .some(dot => dot.closest('li')?.textContent?.includes('Orygin') === true)).toBe(true)
  ...
  it('loads on first render of an idle controller', async () => {
    ...
    render(<ModelsSection
      ...
      t={t}
    />)
    await screen.findByText('Orygin')
  })
...
describe('apiKeyFailure', () => {
  ...
  it.each([
    ['a pasted environment line', 'ORYGIN_API_KEY=sk-abc'],
