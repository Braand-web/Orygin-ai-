...
import type { RpcResponse } from '@orygin-ai/dsh-api-remotes/client'
...
const NAMESPACES = [
  {
    ns: 'llm-orygin',
    schema: {},
    value: { apiKeyEnv: 'ORYGIN_API_KEY', baseURL: 'https://base' },
...
describe('ModelsSettingsStore', () => {
  it('joins rows with configured, removable, and credential state', async () => {
    ...
    expect(seenRefs).toEqual([['ORYGIN_API_KEY', 'OPENAI_API_KEY']])
    const byProvider = new Map(state.rows.map(row => [row.entry.provider, row]))
    expect(byProvider.get('orygin-official')).toMatchObject({
      configured: true,
      removable: false,
