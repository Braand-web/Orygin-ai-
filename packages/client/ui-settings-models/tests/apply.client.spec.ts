...
import { resolveSlotLabel } from '@orygin-ai/dsh-client-ui-slots'
import { SlotRegistry } from '@orygin-ai/dsh-client-runtime/client'
import { LocaleRuntime } from '@orygin-ai/dsh-client-locale/client'
import { TestRemote } from '@orygin-ai/dsh-client-test-runtime'
import { apply as settingsApply, inject as settingsInject } from '@orygin-ai/dsh-client-ui-settings/client'
import { apply, inject, refreshIfLoaded } from '@orygin-ai/dsh-client-ui-settings-models/client'
import {
...
describe('ui-settings-models apply', () => {
  ...
  it('registers the models nav entry for declarations before or after apply', async () => {
    ...
    const orygin = onboarding.find(entry => entry.options.id === 'orygin-official')!
    expect(orygin.component).toBe(OryginOnboardingDialog)
    expect(orygin.options).toMatchObject({ id: 'orygin-official', order: 0 })
    const oryginInjected = (
      orygin.inject as unknown as () => import('../src/client/OryginOnboardingDialog.tsx').OryginOnboardingInjected
    )()
    expect(oryginInjected.hooks.models).toBe(injected.controller.store)
    expect(oryginInjected.api).toBeDefined()
...
describe('pushed invalidations', () => {
  ...
  it('routes pushed credential invalidation into the shared onboarding join', async () => {
    ...
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const entry = b.slots.entries('settings.onboarding')
      .find(candidate => candidate.options.id === 'orygin-official')!
    const injected = (
      entry.inject as unknown as
      () => import('../src/client/OryginOnboardingDialog.tsx').OryginOnboardingInjected
    )()
    injected.controller.store.update((state) => { state.status = 'ready' })
    const load = vi.spyOn(injected.controller, 'load').mockResolvedValue()
    b.ctx.remote.$dispatch('credentials/reference-updated', ['ORYGIN_API_KEY'])
