...
import SessionStore, { Session, SessionId } from '@orygin-ai/dsh-session'
...
import SandboxPolicyService, { setSandboxMode } from '@orygin-ai/dsh-sandbox-policy'
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@orygin-ai/dsh-terminal'
import type { TerminalSendRequest, TerminalWaitReason } from '@orygin-ai/dsh-terminal'
import { BashTerminalBackend, PWSH_PROMPT_SETUP } from '@orygin-ai/dsh-terminal-bash'
import { ENCODING_PREAMBLE } from '@orygin-ai/dsh-pwsh-local'
import * as ptyLocal from '@orygin-ai/dsh-terminal-bash'
import type { ResolvedConfig } from '@orygin-ai/dsh-terminal-bash/src/config.ts'
import type { LocalPtySession } from '@orygin-ai/dsh-terminal-bash/src/session.ts'
import { SubprocessRuntime } from '@orygin-ai/dsh-subprocess'
import type {
  SubprocessHandle,
  SubprocessSpawnSpec,
  SubprocessTerminalHandle,
  SubprocessTerminalSpawnSpec,
} from '@orygin-ai/dsh-subprocess'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      env: {
        ...
        ORYGIN_SHELL: '1', ORYGIN_SESSION_ID: 'agent', ORYGIN_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('bootstraps a pwsh dialect through the prompt function and scrubs bash-only env', async () => {
    ...
    expect(spawned?.env).toMatchObject({
      TERM: 'dumb', NO_COLOR: '1', ORYGIN_SHELL: '1', ORYGIN_SESSION_ID: 'agent', ORYGIN_PTY_SESSION_ID: 'pty-1',
    })
