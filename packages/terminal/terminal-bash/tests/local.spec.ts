...
import type { TerminalSendOperation } from '@orygin-ai/dsh-terminal'
...
describe.skipIf(process.platform === 'win32')('terminal-bash real shell', () => {
  it('persists cwd and environment across sends, scrubs secrets, and closes', async () => {
    const previous = process.env.ORYGIN_TEST_SECRET
    process.env.ORYGIN_TEST_SECRET = 'must-not-leak'
    try {
      ...
      if (previous === undefined) delete process.env.ORYGIN_TEST_SECRET
      else process.env.ORYGIN_TEST_SECRET = previous
    }
  }, 10_000)
  ...
  it('bootstraps a persistent pwsh, persists state, and scrubs secrets', async () => {
    ...
    try {
      ...
      const second = ctx.terminals.startSend(agent, created.sessionId, {
        text: 'Write-Output "keep=$env:KEEP secret=$env:ORYGIN_TEST_SECRET"',
        submit: true,
      })
