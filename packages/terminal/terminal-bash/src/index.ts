 ...
 * @module @orygin-ai/dsh-terminal-bash
 */
...
import { TerminalBackendCleanupError } from '@orygin-ai/dsh-terminal'
import type { TerminalBackend, TerminalBackendSpawnSpec } from '@orygin-ai/dsh-terminal'
import type { SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from '@orygin-ai/dsh-subprocess'
import type { SandboxExecutionPolicy } from '@orygin-ai/dsh-sandbox'
import { effectiveSandboxMode } from '@orygin-ai/dsh-sandbox-policy'
...
function childEnvironment(spec: TerminalBackendSpawnSpec, dialect: ShellDialect): Record<string, string> {
  ...
  const common = {
    ...
    GIT_PAGER: 'cat',
    ORYGIN_SHELL: '1',
    ORYGIN_SESSION_ID: spec.owner.id,
    ORYGIN_PTY_SESSION_ID: spec.sessionId,
  }
  if (dialect === 'pwsh') {
