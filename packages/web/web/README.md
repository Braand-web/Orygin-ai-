...
| Package | Role |
|---|---|
| `@orygin-ai/dsh-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@orygin-ai/dsh-web-search-exa` | Search provider: Exa |
| `@orygin-ai/dsh-web-search-perplexity` | Search provider: Perplexity |
| `@orygin-ai/dsh-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@orygin-ai/dsh-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
...
Selection never depends on registration, config, or HMR order. A capability has an explicit provider id (config `searchProvider`/`fetchProvider`, or env `$ORYGIN_WEB_SEARCH_PROVIDER`/`$ORYGIN_WEB_FETCH_PROVIDER` feeding the same fields), or auto-selects when exactly one usable provider is registered. `search()`/`fetch()` resolve the provider at execution time:
