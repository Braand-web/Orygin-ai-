...
export class WebRuntime extends Service {
   ...
   * `$ORYGIN_WEB_SEARCH_PROVIDER` / `$ORYGIN_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.ORYGIN_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.ORYGIN_WEB_FETCH_PROVIDER
  }
