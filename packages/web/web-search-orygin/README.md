...
A [Orygin](https://orygin.fun)-backed `WebSearchProvider` for the harness [web capability seam](../web/README.md) (`ctx.web`). It calls Orygin's **Anthropic-compatible Messages API** (`POST {baseURL}/messages`) with the native `web_search_20250305` server tool enabled, and maps the structured `web_search_tool_result` blocks Orygin returns into the seam's normalized `WebSearchResult`.

This is an **implementation** package: it registers a provider into `ctx.web`, resolves its credential for each search through the optional `ctx.credentials` seam, records the auxiliary request in the initiating Agent session when one exists, and does not register a model-facing tool. Like `@orygin-ai/dsh-llm-orygin`, it is a function/namespace plugin (`inject: ['web']`). The Anthropic wire shape is a provider-private detail — it does **not** make this provider depend on `ctx.llm`.

## How it differs from a dedicated search endpoint

Exa and Perplexity expose dedicated search endpoints; Orygin does not. Instead this provider issues a **full Messages model call** carrying the `web_search` server tool, so one search costs a complete model turn in latency and tokens — heavier than a pure retrieval endpoint. Orygin runs the search server-side and returns **structured** `web_search_tool_result` blocks; the provider parses those blocks and **never scrapes URLs out of model prose**.
...
It reuses the `ORYGIN_API_KEY` credential reference (no new secret) but **not** `$ORYGIN_BASE_URL`: the search endpoint is the Anthropic-compatible base (`https://api.orygin.fun/anthropic/v1`), distinct from the chat-completions base (`https://api.orygin.fun`) the LLM adapter uses. A mounted credentials service is authoritative; without one, the provider falls back to the launching process environment. The reference is resolved for each search, so a key stored or rotated by the Web Models page reaches the next call without a restart.

## Config

| Key | Default | Meaning |
|---|---|---|
| `apiKey` | omitted | Literal Orygin API key. Prefer `apiKeyEnv` so no secret enters configuration; a non-empty literal wins. |
| `apiKeyEnv` | `ORYGIN_API_KEY` | Credential reference resolved for each search through `ctx.credentials`, or from the process environment when that seam is absent. A missing value fails the call as `WEB_PROVIDER_CREDENTIAL_MISSING`. |
| `baseURL` | `https://api.orygin.fun/anthropic/v1` | Anthropic-compatible endpoint base; `/messages` is appended. Falls back to `$ORYGIN_SEARCH_BASE_URL` from any environment layer; do not reuse `$ORYGIN_BASE_URL`, which belongs to the chat-completions LLM adapter. An unparseable value makes the provider unavailable. |
| `model` | `orygin-v4-flash` | Anthropic-format model name. |
...
- id: web-search-orygin
  name: '@orygin-ai/dsh-web-search-orygin'
  config:
    apiKeyEnv: ORYGIN_API_KEY
    baseURL: https://gateway.internal/anthropic/v1
```

The entry above is the base layer of the `web-search-orygin` Settings section: a user layer over it reaches the NEXT search, because the provider projects the section per call rather than capturing it at registration. The seam's provider selection therefore never flickers when an endpoint or model changes. `apiKey` carries `role('secret')`, so it never rides a `describe()` response in any layer — a configuration surface learns only whether the credentials domain holds a value for the reference `apiKeyEnv` names, never whether a layer carries a literal key.

## Mapping

Orygin returns no provider-generated answer content this provider trusts as `content`, so `content` is omitted. `sources[]` comes from `web_search_result` items inside `web_search_tool_result` blocks: `url` ← `url`, `title` ← `title`, and `publishedAt` ← `page_age`. Snippets live separately as URL-keyed `cited_text` entries in a text block's `citations[]`; the provider joins them, leaving `snippet` absent when no excerpt exists.

Results are deduplicated by URL because one request may surface the same page across searches. Orygin exposes `maxUses`, not a result-count knob, so the seam enforces `maxResults` by truncating `sources[]` and setting `truncated`.
...
Immediately before dispatch, a search running under an initiating Agent appends the log-only `web/orygin-search-llm-request` session event. It contains the resolved endpoint, API version, and exact secret-free JSON body sent to Orygin; headers and credentials are excluded. Credential failures and cancellations before dispatch create no event, while later HTTP or response failures leave the attempted request durable. Direct programmatic provider calls outside an Agent have no initiating session to log.

## Model Experience

### Auxiliary Orygin search request

#### What the model sees

A separate Orygin model receives exactly `Perform a web search for the query: <query>` as its user text and one native `web_search` server-tool definition. This request is not part of the conversation model's context.
...
Through [`dsh-tool-web`](../tool-web/README.md), the conversation model sees deduplicated URLs, titles, dates, and citation snippets from structured search blocks; provider prose is not trusted as an answer. This provider's exact failures include the actionable missing-credential message, `Orygin search credential resolution failed: <error>`, `Orygin search aborted`, `Orygin search request failed: <error>`, `Orygin returned no web_search_tool_result blocks; the request may not have triggered native web search`, and `Orygin returned an unprocessable response body: <error>`; HTTP failures preserve the provider message. The consumer owns the error wrapper.
...
- **One search costs a full Messages model turn** — latency plus generated tokens, with up to `maxUses` server-side searches; Orygin exposes no dedicated retrieval endpoint.
