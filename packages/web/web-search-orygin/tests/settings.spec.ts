/** The `web-search-orygin` settings section layered over the composition entry. */

import { afterEach, describe, expect, it, vi } from 'vitest'
import { Context } from '@orygin-ai/cordis'
import type { Fiber } from '@orygin-ai/cordis'
import { SettingsProvider } from '@orygin-ai/dsh-settings'
...
import { WEB_SEARCH_ORYGIN_SETTINGS_NAMESPACE } from '@orygin-ai/dsh-web-search-orygin'
...
async function boot(): Promise<{ ctx: Context; settingsFiber: Fiber; pluginFiber: Fiber }> {
  ...
  const pluginFiber = ctx.plugin(oryginPlugin, { apiKey: 'ds-key', baseURL: 'https://search.entry.test/v1' })
...
describe('web-search-orygin settings section', () => {
  it('serves a stored endpoint to the next search without re-registering the provider', async () => {
    ...
    await bench.ctx.settings.update(WEB_SEARCH_ORYGIN_SETTINGS_NAMESPACE, {
  ...
  it('keeps the literal key out of every described layer', async () => {
    const bench = await boot()
    await bench.ctx.settings.update(WEB_SEARCH_ORYGIN_SETTINGS_NAMESPACE, { apiKey: 'ds-stored-secret' })

    const [descriptor] = bench.ctx.settings.describe({ redactSecrets: true })
      .filter(row => String(row.ns) === 'web-search-orygin')
  ...
  it('releases the namespace when the plugin unloads', async () => {
    const bench = await boot()
    expect(bench.ctx.settings.describe().map(row => String(row.ns))).toContain('web-search-orygin')

    await bench.pluginFiber.dispose()

    expect(bench.ctx.settings.describe().map(row => String(row.ns))).not.toContain('web-search-orygin')
    await bench.ctx.fiber.dispose()
  })
})
