...
import LocalCredentialProvider from '@orygin-ai/dsh-credentials-local'
import WebRuntime from '@orygin-ai/dsh-web'
import {
  OryginSearchProvider,
  ORYGIN_PROVIDER_ID,
} from '@orygin-ai/dsh-web-search-orygin'
import * as oryginPlugin from '@orygin-ai/dsh-web-search-orygin'
import { citationSnippets, mapAnthropicResponse } from '../src/provider.ts'
import type { AnthropicResponse } from '@orygin-ai/dsh-web-search-orygin/src/types.ts'
...
const options = {
  apiKey: 'ds-key',
  baseURL: 'https://api.orygin.test/anthropic/v1',
  model: 'orygin-chat',
...
describe('OryginSearchProvider availability', () => {
...
describe('OryginSearchProvider request mapping', () => {
  it('records and posts the same Anthropic Messages request with the web_search server tool', async () => {
    ...
    expect(url).toBe('https://api.orygin.test/anthropic/v1/messages')
...
describe('OryginSearchProvider settings changes mid-search', () => {
  it('serves one search from one section even when settings land during credential resolution', async () => {
    ...
    const provider = new OryginSearchProvider(() => ({ ...current, resolveApiKey }))
...
describe('OryginSearchProvider error handling', () => {
  ...
  it('maps a credential resolver rejection under an active signal to WEB_PROVIDER_ERROR', async () => {
    ...
    await expect(searchProvider({
      ...
      .rejects.toThrow(expect.objectContaining({
        ...
        message: 'Orygin search credential resolution failed: Error: credential backend failed',
      }))
  })
  ...
  it('uses the default credential reference when no resolver is configured', async () => {
    await expect(searchProvider({ ...options, apiKey: '' }).search({ query: 'q' }))
      .rejects.toThrow('Orygin search has no API key for "ORYGIN_API_KEY"')
  })
  ...
  it('keeps a status-line message when the error body is not JSON', async () => {
    ...
    await expect(searchProvider(options).search({ query: 'q' }))
      .rejects.toThrow(expect.objectContaining({ message: 'Orygin API error (HTTP 503)' }))
  })

  it('keeps the status-line message when the JSON error body carries no detail', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse({}, { status: 500 })))
    await expect(searchProvider(options).search({ query: 'q' }))
      .rejects.toThrow(expect.objectContaining({ message: 'Orygin API error (HTTP 500)' }))
  })
...
describe('web-search-orygin plugin registration', () => {
  it('registers the provider into ctx.web (HMR-safe)', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse(searchResponse())))
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: ORYGIN_PROVIDER_ID })
    const fiber = await ctx.plugin(oryginPlugin, { apiKey: 'ds-key' })
  ...
  it('rejects maxTokens: 0 at plugin construction', async () => {
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: ORYGIN_PROVIDER_ID })
    await expect(ctx.plugin(oryginPlugin, { apiKey: 'ds-key', maxTokens: 0 }))
  ...
  it('rejects maxUses: 0 at plugin construction', async () => {
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: ORYGIN_PROVIDER_ID })
    await expect(ctx.plugin(oryginPlugin, { apiKey: 'ds-key', maxUses: 0 }))
  ...
  it('rejects a fractional maxUses at plugin construction', async () => {
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: ORYGIN_PROVIDER_ID })
    await expect(ctx.plugin(oryginPlugin, { apiKey: 'ds-key', maxUses: 1.5 }))
      .rejects.toThrow(/maxUses expected number multiple of 1/)
  })

  it('has no default export (namespace plugin export shape)', () => {
    expect('default' in oryginPlugin).toBe(false)
  })
  ...
  it('survives the real Loader unwrapExports path keeping name/inject/Config', () => {
    ...
    const unwrapped = loader.unwrapExports(oryginPlugin) as Record<string, unknown>
    expect(unwrapped).toBe(oryginPlugin)
    expect(unwrapped.name).toBe('web-search-orygin')
  ...
  it('boots over ctx.web through the unwrapped module without an inject error', async () => {
    ...
    await ctx.plugin(WebRuntime, { searchProvider: ORYGIN_PROVIDER_ID })
    const loader = Object.create(Loader.prototype) as Loader
    const unwrapped = loader.unwrapExports(oryginPlugin) as Parameters<Context['plugin']>[0]
  ...
  it('falls back to the env key and defaults when config omits them', async () => {
    const prev = process.env.ORYGIN_API_KEY
    process.env.ORYGIN_API_KEY = 'env-key'
    try {
      const fetchMock = vi.fn(async () => jsonResponse(searchResponse()))
      vi.stubGlobal('fetch', fetchMock)
      const ctx = new Context()
      await ctx.plugin(WebRuntime, { searchProvider: ORYGIN_PROVIDER_ID })
      oryginPlugin.apply(ctx, {})
      await ctx.web.search({ query: 'q' })
      const [url, init] = fetchMock.mock.calls[0] as unknown as [string, RequestInit]
      expect(url).toBe('https://api.orygin.fun/anthropic/v1/messages')
      expect((init.headers as Record<string, string>)['x-api-key']).toBe('env-key')
      expect(JSON.parse(init.body as string)).toMatchObject({ model: 'orygin-v4-flash' })
      await ctx.fiber.dispose()
    } finally {
      if (prev === undefined) delete process.env.ORYGIN_API_KEY
      else process.env.ORYGIN_API_KEY = prev
    }
  })

  it('resolves the credential for each search so a stored or rotated key needs no restart', async () => {
    const previous = process.env.ORYGIN_API_KEY
    delete process.env.ORYGIN_API_KEY
    ...
    try {
      ...
      await ctx.plugin(oryginPlugin, { baseURL: 'https://api.orygin.test/anthropic/v1' })

      await expect(ctx.web.search({ query: 'missing' }))
        .rejects.toThrow(expect.objectContaining({ code: 'WEB_PROVIDER_CREDENTIAL_MISSING' }))

      const ref = credentialRef('ORYGIN_API_KEY')
      ...
      if (previous === undefined) delete process.env.ORYGIN_API_KEY
      else process.env.ORYGIN_API_KEY = previous
    }
  })
  ...
  it('reports an actionable credential error when neither config nor env supplies a key', async () => {
    ...
    try {
      ...
      if (prev !== undefined) process.env.ORYGIN_API_KEY = prev
    }
  })
})
