/**
 * Orygin search through an Anthropic-compatible Messages model call with the native
 ...
 * @module @orygin-ai/dsh-web-search-orygin/provider
 */

import { WebError } from '@orygin-ai/dsh-web'
import type {
  WebSearchProvider,
  WebSearchRequest,
  WebSearchResult,
  WebSearchSource,
} from '@orygin-ai/dsh-web'
...
export const ORYGIN_PROVIDER_ID = 'orygin-official'

/**
 * Default endpoint: Orygin's Anthropic-compatible API, `/v1` included
 * (`/messages` is appended). This is NOT the chat-completions base
 * (`https://api.orygin.fun`) `@orygin-ai/dsh-llm-orygin` uses, so this
 * provider does NOT reuse `$ORYGIN_BASE_URL` — only the API key is shared.
 */
export const ORYGIN_DEFAULT_BASE_URL = 'https://api.orygin.fun/anthropic/v1'

/** Default Anthropic-format model name (aligned with the repo's Orygin model vocabulary). */
export const ORYGIN_DEFAULT_MODEL = 'orygin-v4-flash'

/** Default `anthropic-version` header value. */
export const ORYGIN_DEFAULT_API_VERSION = '2023-06-01'

/** Default upper bound on generated tokens for the Messages request. */
export const ORYGIN_DEFAULT_MAX_TOKENS = 4096

/** Default maximum `web_search` server-tool uses per request. */
export const ORYGIN_DEFAULT_MAX_USES = 5

/** Attribution header sent on every request. Bump with the package version. */
const USER_AGENT = 'orygin-ai/0.0.1'

/**
 * Exact secret-free Orygin Messages request recorded immediately before one
 * auxiliary search dispatch.
 */
export interface OryginSearchLlmRequest {
...
declare module '@orygin-ai/dsh-session/types' {
  interface SessionEventMap {
    /** Secret-free auxiliary Orygin search request recorded before dispatch. */
    'web/orygin-search-llm-request': OryginSearchLlmRequest
  }
}

/** Resolved provider options (the plugin's `apply` supplies credential and constant defaults). */
export interface OryginSearchProviderOptions {
  /** Literal Orygin API key; when present it wins over {@link resolveApiKey}. */
  apiKey?: string
  /** Resolve the current Orygin API key for one search operation. */
   ...
   */
  recordRequest?: (request: OryginSearchLlmRequest) => void
}
...
/**
 * Map a Orygin Anthropic Messages response to a normalized search result. Walks
...
export function mapAnthropicResponse(response: AnthropicResponse): WebSearchResult {
  ...
  if (resultBlocks.length === 0) {
    throw new WebError(
      'Orygin returned no web_search_tool_result blocks; the request may not have triggered native web search',
      'WEB_PROVIDER_ERROR',
    )
  }
...
/** The Orygin-backed search provider; HTTP redirects fail as `WEB_PROVIDER_ERROR`. */
export class OryginSearchProvider implements WebSearchProvider {
  readonly id = ORYGIN_PROVIDER_ID
   ...
   */
  constructor(private readonly resolveOptions: () => OryginSearchProviderOptions) {}
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    const body: OryginSearchLlmRequest['body'] = {
      model: options.model,
    ...
    try {
      response = await fetch(endpoint, {
        ...
        redirect: 'error',
        headers: {
          // Official Orygin expects `x-api-key`; an Anthropic-compatible proxy
      ...
      throw new WebError(`Orygin search request failed: ${String(error)}`, 'WEB_PROVIDER_ERROR', { cause: error })
    }

    if (!response.ok) {
      const status = response.status
      let message = `Orygin API error (HTTP ${status})`
      try {
    ...
    try {
      ...
      throw new WebError(`Orygin returned an unprocessable response body: ${String(error)}`, 'WEB_PROVIDER_ERROR', { cause: error })
    }
  }
   ...
   */
  private async apiKey(options: OryginSearchProviderOptions, signal?: AbortSignal): Promise<string> {
    throwIfSearchAborted(signal)
    ...
    try {
      ...
      if (signal?.aborted === true || isAbortError(error)) throw searchAborted(signal, error)
      throw new WebError(
        `Orygin search credential resolution failed: ${String(error)}`,
        'WEB_PROVIDER_ERROR',
        { cause: error },
      )
    }
    if (resolved !== undefined && resolved.length > 0) return resolved
    const ref = options.apiKeyEnv ?? 'ORYGIN_API_KEY'
    throw new WebError(
      `Orygin search has no API key for "${ref}"; store it through the credentials service`
      + ' (the web Models page writes it), export it in the launching environment, or set a literal'
      + ' "apiKey" in the web-search-orygin config',
...
function searchAborted(signal?: AbortSignal, fallback?: unknown): WebError {
  return new WebError('Orygin search aborted', 'WEB_ABORTED', {
...
/** True for Orygin request limits that can be sent to the Messages API. */
