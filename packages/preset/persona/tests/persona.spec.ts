...
import { createScope, type ScopeKey } from '@orygin-ai/dsh-scope'
import { describe, expect, it } from 'vitest'
import * as Persona from '@orygin-ai/dsh-persona'
import { PERSONA_SECTION } from '@orygin-ai/dsh-persona'
...
describe('the persona row', () => {
  ...
  it('interpolates prompt variables strictly, like any other section', async () => {
    const ctx = await harness('')
    const key: ScopeKey = { agent: 'a1' }
    ctx.systemPrompt.variable('model', () => 'orygin-v4-pro')
    ...
    expect(renderPrompt(await ctx.systemPrompt.assemble({ scope: key })))
      .toContain('You run on orygin-v4-pro.')
  })
