...
import { Context, type FiberState } from '@orygin-ai/cordis'
import Loader, { type Entry, type EntryOptions } from '@orygin-ai/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@orygin-ai/cordis-plugin-include'
import Group from '@orygin-ai/cordis-plugin-group'
import { dshHomePath, resolveDshHome } from '@orygin-ai/dsh-home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@orygin-ai/dsh-launch-environment'
import type {} from '@orygin-ai/cordis-plugin-hmr'
// Side-effect type import: resolves `ctx.get('systemPrompt')` to the service.
import type {} from '@orygin-ai/dsh-system-prompt'
 ...
 * @param snapshotMode - the bin's `$ORYGIN_SNAPSHOT` value; only `'replay'` swaps the
 *   basename.
...
const BOOTSTRAP_NAMES = new Set([
  ...
  'ORYGIN_BASE_URL', 'ORYGIN_SEARCH_BASE_URL',
...
const BOOTSTRAP_PREFIXES = ['ORYGIN_', 'XDG_', 'DYLD_', 'BASH_FUNC_']
 ...
 * entries (`@orygin-ai/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@orygin-ai/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@orygin-ai/cordis-plugin-group`
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The Orygin implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend ORYGIN itself.`,
  })
}
