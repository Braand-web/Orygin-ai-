...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@orygin-ai/dsh-base'])
    ...
    expect(readProfileManifest('t', dir).dsh?.profile?.bundles).toEqual(['@orygin-ai/dsh-base'])
...
describe('loadProfile', () => {
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    // @orygin-ai/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web).toContain('@orygin-ai/dsh-base')
    try {
  ...
  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@orygin-ai/dsh-base': { patch: '[]\n' },
      '@orygin-ai/dsh-web-app': { patch: '[]\n' },
      '@orygin-ai/dsh-headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      '@orygin-ai/dsh-base', '@orygin-ai/dsh-web-app', '@orygin-ai/dsh-headless',
    ])
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).dsh?.profile?.bundles)
      .toEqual(['@orygin-ai/dsh-base', '@orygin-ai/dsh-headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      '@orygin-ai/dsh-base', '@orygin-ai/dsh-web-app', '@orygin-ai/dsh-headless', 'custom-bundle',
    ])
