...
describe('loadEnv', () => {
  it('loads variables from .env in the given dir', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'ORYGIN_APP_BOOT_SPEC_VAR=loaded\n')
    const warn = vi.fn()
    loadEnv(NAME, dir, warn)
    expect(process.env['ORYGIN_APP_BOOT_SPEC_VAR']).toBe('loaded')
    expect(warn).not.toHaveBeenCalled()
    delete process.env['ORYGIN_APP_BOOT_SPEC_VAR']
  })
  ...
  it('defaults dir to the process cwd and warn to a stderr write', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'ORYGIN_APP_BOOT_SPEC_DEFAULTS=yes\n')
    ...
    expect(process.env['ORYGIN_APP_BOOT_SPEC_DEFAULTS']).toBe('yes')
    delete process.env['ORYGIN_APP_BOOT_SPEC_DEFAULTS']
...
describe('loadLayeredEnv', () => {
  ...
  it.each([
    ['a harness switch', 'ORYGIN_PERMISSION_MODE=danger-full-access\n'],
    ['the executable search path', 'PATH=/tmp/evil\n'],
    ['a module preload', 'NODE_OPTIONS=--require /tmp/evil.js\n'],
    ['a skill root', 'ORYGIN_AGENTS_HOME=/tmp/injected\n'],
...
describe('boot', () => {
  ...
  it('can resolve bare plugins from the harness when the config project shadows their package name', async () => {
    ...
    const shadow = join(dir, 'node_modules', '@orygin-ai', 'dsh-system-prompt')
    const harnessPlugin = join(harness, 'node_modules', '@orygin-ai', 'dsh-system-prompt')
  ...
  it('exposes dshHomePath to Loader config expressions', async () => {
    ...
    vi.stubEnv('ORYGIN_HOME', dshHome)
...
describe('addHarnessSourceSection', () => {
  ...
  it('distinguishes the source path from the current workdir between identity and persona', async () => {
    ...
    try {
      ...
      const identityAt = rendered.indexOf('You are an AI agent powered by Orygin.')
