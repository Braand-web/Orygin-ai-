...
import type { SubprocessHandle, SubprocessOutputReader } from '@orygin-ai/dsh-subprocess'
...
function shellArgv(command: string): string[] {
  ...
  switch (command) {
    ...
    case 'echo "[${ORYGIN_STALE:-absent}|$ORYGIN_SHELL|$ORYGIN_SESSION_ID]"':
      return node('console.log("[" + [process.env.ORYGIN_STALE ?? "absent", process.env.ORYGIN_SHELL, process.env.ORYGIN_SESSION_ID].join("|") + "]")')
    case 'echo "[${ORYGIN_TEST_API_KEY:-absent}|${ORYGIN_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${ORYGIN_TEST_PLAIN:-absent}]"':
      return node('console.log("[" + [process.env.ORYGIN_TEST_API_KEY ?? "absent", process.env.ORYGIN_TEST_TOKEN ?? "absent", process.env.SUBPROCESS_TEST_PASSWORD ?? "absent", process.env.ORYGIN_TEST_PLAIN ?? "absent"].join("|") + "]")')
...
describe.skipIf(process.platform === 'win32')('tree-survivor escalation (terminate and bounded waits reach helpers the leader left behind)', () => {
  ...
  it('service teardown awaits tree survivors, not just handle settlement', async () => {
    const { Context } = await import('@orygin-ai/cordis')
    const { default: LocalSubprocessRuntime } = await import('@orygin-ai/dsh-subprocess-local')
...
describe('coverage seams', () => {
  ...
  it('childEnv keeps the POSIX spread on non-Windows hosts', () => {
    ...
    try {
      expect(childEnv({ ORYGIN_X: '1' }).ORYGIN_X).toBe('1')
    } finally {
...
describe('environment and spill-file hardening', () => {
  it('scrubs credential-shaped and ambient ORYGIN env vars from child processes', async () => {
    process.env.ORYGIN_TEST_API_KEY = 'super-secret'
    process.env.ORYGIN_TEST_TOKEN = 'also-secret'
    process.env.SUBPROCESS_TEST_PASSWORD = 'password-secret'
    process.env.ORYGIN_TEST_PLAIN = 'visible'
    try {
      const result = await finish(spawnSubprocess(spec(
        'echo "[${ORYGIN_TEST_API_KEY:-absent}|${ORYGIN_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${ORYGIN_TEST_PLAIN:-absent}]"',
      )))
      expect(result.stdout.text.trim()).toBe('[absent|absent|absent|absent]')
    } finally {
      delete process.env.ORYGIN_TEST_API_KEY
      delete process.env.ORYGIN_TEST_TOKEN
      delete process.env.SUBPROCESS_TEST_PASSWORD
      delete process.env.ORYGIN_TEST_PLAIN
    }
  })

  it('forwards explicit ORYGIN_* env entries while scrubbing ambient ones', async () => {
    // Both facts through one explicit map: the ambient ORYGIN_STALE is dropped by
    // the scrub, and the deliberately supplied current values merge after it.
    process.env.ORYGIN_STALE = 'old-value'
    try {
      const result = await finish(spawnSubprocess(spec('echo "[${ORYGIN_STALE:-absent}|$ORYGIN_SHELL|$ORYGIN_SESSION_ID]"', {
        env: { ORYGIN_SHELL: '1', ORYGIN_SESSION_ID: 'current-session' },
      })))
      expect(result.stdout.text.trim()).toBe('[absent|1|current-session]')
    } finally {
      delete process.env.ORYGIN_STALE
    }
  })
