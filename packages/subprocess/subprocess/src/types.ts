...
/** Namespace prefix reserved for Orygin-managed child environment facts. */
export const ORYGIN_ENV_PREFIX = 'ORYGIN_' as const

/** One environment key inside the managed {@link ORYGIN_ENV_PREFIX} namespace. */
export type DshEnvironmentKey = `${typeof ORYGIN_ENV_PREFIX}${string}`
 ...
 * to `TerminalSignal` in `@orygin-ai/dsh-terminal` without a cross-seam dependency;
