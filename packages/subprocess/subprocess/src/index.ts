 ...
 * `@orygin-ai/dsh-subprocess-local`.
 * @module @orygin-ai/dsh-subprocess
 */

import { Context, Service } from '@orygin-ai/cordis'
import { ORYGIN_ENV_PREFIX } from './types.ts'
import type { SubprocessHandle, SubprocessSpawnSpec } from './types.ts'
import type { SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from './types.ts'

export { ORYGIN_ENV_PREFIX } from './types.ts'
export type {
  CollectedOutput,
  DshEnvironment,
  DshEnvironmentKey,
 ...
 * harness's own `ORYGIN_API_KEY`/secrets must not leak into a spawned
 ...
 * `ORYGIN_*` names — the canonical base every harness child starts from. `PATH`,
 ...
 * credential or current `ORYGIN_*` fact goes through the spec's explicit `env`,
 ...
 * would otherwise survive and read back as `$env:ORYGIN_*` in the child;
...
export function scrubbedParentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (value !== undefined && !SENSITIVE_ENV_PATTERN.test(key) && !key.toUpperCase().startsWith(ORYGIN_ENV_PREFIX)) env[key] = value
  }
  return env
}
