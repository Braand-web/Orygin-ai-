...
import { scrubbedParentEnv, SubprocessRuntime } from '@orygin-ai/dsh-subprocess'
import type {
  SubprocessHandle,
...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and ORYGIN_ names (case-insensitively) but keeps PATH', () => {
    process.env.ORYGIN_SCRUB_PROBE = 'stale'
    ...
    try {
      ...
      expect(env.ORYGIN_SCRUB_PROBE).toBeUndefined()
      ...
      delete process.env.ORYGIN_SCRUB_PROBE
