...
import type { SubprocessTerminalSpawnSpec } from '@orygin-ai/dsh-subprocess'
...
class FakeTerminalSandbox {
  ...
  ambient = 'KEEP=visible\0UNICODE=你好\0NPM_TOKEN=secret\0ORYGIN_STALE=old\0BROKEN\0=bad\0'
  sessionId = '123\n'
  foreground = '456\n'
  groups = [123]
  zombieGroups: number[] = []
...
function spec(overrides: Partial<SubprocessTerminalSpawnSpec> = {}): SubprocessTerminalSpawnSpec {
  return {
    ...
    cols: 80,
    graceMs: 5,
    env: { TERM: 'dumb', ORYGIN_SESSION_ID: 'owner', TOKEN_EXPLICIT: 'kept' },
    ...overrides,
  }
}
...
describe('E2B terminal allocation', () => {
  it('hides bootstrap-shell bytes and preserves requested-shell bytes across the output boundary', async () => {
    ...
    expect(fake.writes.get('/runtime/terminal-one/environment')).not.toContain('ORYGIN_STALE')
  ...
  it('inherits only safe ambient values and limits the allocation signal to setup', async () => {
    ...
    expect(environment).not.toContain('ORYGIN_STALE')
