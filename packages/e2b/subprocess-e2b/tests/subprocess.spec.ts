...
import type E2BRuntime from '@orygin-ai/dsh-e2b'
import type { SubprocessSpawnSpec } from '@orygin-ai/dsh-subprocess'
...
class FakeSandbox {
  ...
  alive = true
  zombieOnly = false
  ambient = 'PATH=/ambient/bin\0KEEP=safe\0UNICODE=你好\0NPM_TOKEN=secret\0ORYGIN_STALE=old\0BROKEN\0=bad\0'
  ...
  it('starts asynchronously, keeps secrets out of the command, and supports deferred piped stdin/output', async () => {
    ...
    const handle = testHandle(runtime(fake), spec({
      ...
      env: {
        ...
        ORYGIN_API_KEY: 'explicit-secret',
        ORYGIN_MODE: 'test',
    ...
    expect(command).not.toContain('ORYGIN_API_KEY')
    expect(command).not.toContain('ORYGIN_MODE')
    ...
    expect(fake.writtenFileData.get('/workspace/.dsh-e2b/processes/one/environment')).toBe(
      'PATH=/bin\0UNICODE=你好\0HOME=/home/user\0FOO-BAR=hyphen-value\0--split-string=literal-value\0ORYGIN_API_KEY=explicit-secret\0ORYGIN_MODE=test\0',
    )
