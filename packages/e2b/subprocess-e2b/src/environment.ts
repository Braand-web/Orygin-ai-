...
import { e2bControlEnvs } from '@orygin-ai/dsh-e2b'
import type { Sandbox } from '@orygin-ai/dsh-e2b'
import { SENSITIVE_ENV_PATTERN } from '@orygin-ai/dsh-subprocess'
...
export function scrubRemoteEnvironment(raw: string): Map<string, string> {
  ...
  for (const [name, value] of remoteEnvironmentEntries(raw)) {
    if (name.startsWith('ORYGIN_') || SENSITIVE_ENV_PATTERN.test(name)) continue
...
export function bootstrapEnvironment(raw: string): Record<string, string> {
  ...
  for (const [name] of remoteEnvironmentEntries(raw)) {
    if (name.startsWith('ORYGIN_') || SENSITIVE_ENV_PATTERN.test(name)) environment[name] = ''
  }
  return environment
}
