...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    const sandbox = await Sandbox.create({
      apiKey,
      envs: { NPM_TOKEN: 'sentinel-secret', ORYGIN_STALE: 'sentinel-stale', KEEP: 'visible' },
      timeoutMs: 60_000,
      secure: true,
    ...
    try {
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> ORYGIN=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$ORYGIN_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> ORYGIN=<> KEEP=<visible>')
