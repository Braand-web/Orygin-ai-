/**
 * File-backed credentials provider over `$ORYGIN_HOME/.credentials.yaml`, layered
 ...
 * > $ORYGIN_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `ORYGIN_API_KEY=… dsh`, a CI
 ...
 * @module @orygin-ai/dsh-credentials-local
 */
...
import { CredentialProvider, credentialRef, parseCredentialKey } from '@orygin-ai/dsh-credentials'
import type {
  ApiKeyRecord,
  CredentialInfo,
  CredentialKey,
...
import type { LaunchEnvironmentEntry } from '@orygin-ai/dsh-launch-environment'
...
/** File-backed credentials provider (`$ORYGIN_HOME/.credentials.yaml`). */
