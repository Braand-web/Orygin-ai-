...
import { withFileLock } from '@orygin-ai/dsh-atomic-write'
...
const FLAT = [
  ...
  "ORYGIN_CRED_OTHER: 'quoted value'",
  'ORYGIN_CRED_BLOCK: |',
  '  first line',
  '  second line',
  'records: tricky',
...
describe('flat-layout boot migration', () => {
  it('upgrades the flat document in place, byte for byte, and serves its keys', async () => {
    ...
    expect(await ctx.credentials.resolve(credentialRef('ORYGIN_CRED_TEST'))).toEqual({ value: 'stored', source: 'file' })
    expect(await ctx.credentials.resolve(credentialRef('ORYGIN_CRED_OTHER')))
      .toEqual({ value: 'quoted value', source: 'file' })
    expect(await ctx.credentials.resolve(credentialRef('ORYGIN_CRED_BLOCK')))
  ...
  it('yields to a concurrent migrator under the writer lock', async () => {
    ...
    expect(await ctx.credentials.resolve(credentialRef('ORYGIN_CRED_TEST'))).toEqual({ value: 'winner', source: 'file' })
  })
  ...
  it('leaves an empty flow mapping alone', async () => {
    ...
    expect(await ctx.credentials.resolve(credentialRef('ORYGIN_CRED_TEST'))).toBeUndefined()
  })
  ...
  it('renders a final newline for a document that lacks one', () => {
    expect(renderFlatLayoutMigration('ORYGIN_CRED_TEST: bare')).toBe('version: 1\nrefs:\n  ORYGIN_CRED_TEST: bare\n')
  })
})
