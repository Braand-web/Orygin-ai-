...
vi.mock('@orygin-ai/dsh-atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@orygin-ai/dsh-atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@orygin-ai/dsh-atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
  mocked.__setGate(next)
}

const KEY = credentialRef('ORYGIN_CRED_DRAIN_A')
const OTHER = credentialRef('ORYGIN_CRED_DRAIN_B')
