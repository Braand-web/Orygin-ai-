...
import AgentRegistry, { agentEvents } from '@orygin-ai/dsh-agent'
import type { Agent } from '@orygin-ai/dsh-agent'
import AttachmentStore from '@orygin-ai/dsh-attachment'
import LlmRuntime, { LlmAdapter, ReasoningEffortId } from '@orygin-ai/dsh-llm'
import type {
...
import type { SessionId } from '@orygin-ai/dsh-session'
import SystemPrompt from '@orygin-ai/dsh-system-prompt'
import UserQuestionService from '@orygin-ai/dsh-user-questions'
import type { RpcRequest } from '@orygin-ai/dsh-host-apiproxy/api/rpc'
import { RpcId } from '@orygin-ai/dsh-host-apiproxy/api/rpc'
...
async function harness(logged?: {
  ...
  ctx.llm.registerAdapter(['orygin-official'], new CatalogAdapter('Orygin', [
    { provider: 'orygin-official', id: 'orygin-chat', name: 'Orygin Chat' },
    { provider: 'orygin-official', id: 'orygin-reasoner', name: 'Orygin Reasoner', description: 'Reasoning model' },
  ], REASONING))
...
describe('Web session model selection', () => {
  it('validates an ordered image batch before persisting any member', async () => {
    ...
    const api = createApiProxy(ctx, {
      defaultModelSelection: () => ({ provider: 'orygin-official', model: 'orygin-chat' }),
      cwd: '/tmp',
    })
  ...
  it('groups successful providers and leaves an unlisted current selection out of the catalog', async () => {
    ...
    const api = createApiProxy(ctx, { defaultModelSelection: () => ({ provider: 'orygin-official', model: 'orygin-chat' }), cwd: '/tmp' })
    ...
    expect(catalog.groups).toEqual([{
      ...
      name: 'Orygin',
      models: [
        { id: 'orygin-chat', name: 'Orygin Chat', reasoning: REASONING },
        {
          id: 'orygin-reasoner',
          name: 'Orygin Reasoner',
  ...
  it('accepts an advisory-unlisted model, rejects an unavailable provider, and switches only after the next assembly', async () => {
    ...
    expect(expectValue(await api.sessions.models(request({ sessionId }))).current)
      .toEqual({ provider: 'orygin-official', model: 'orygin-chat' })
    expect((await ctx.systemPrompt.assemble()).variables)
      .toMatchObject({ provider: 'orygin-official', model: 'orygin-chat' })
    ...
    )).resolves.toMatchObject({ provider: 'orygin-official', model: 'orygin-chat' })

    expect((await ctx.systemPrompt.assemble()).variables)
      .toMatchObject({ provider: 'orygin-official', model: 'private-preview' })
    await expect(agentEvents(ctx, agent).waterfall(
    ...
    expect(unsupported.result).toMatchObject({
      ...
      error: {
        ...
        message: 'provider "orygin-official" model "private-preview" does not support reasoning effort "medium"',
      },
    })
    ...
    expect(expectValue(await api.sessions.models(request({ sessionId }))).current)
      .toEqual({ provider: 'orygin-official', model: 'private-preview', reasoningEffort: 'max' })
    await ctx.fiber.dispose()
  })

  it('reads the Agent default live for a session whose log names no selection', async () => {
    const { ctx, sessionId } = await harness()
    let stored = { provider: 'orygin-official', model: 'orygin-chat' }
    const api = createApiProxy(ctx, {
    ...
    stored = { provider: 'orygin-official', model: 'orygin-reasoner' }
    expect(expectValue(await api.sessions.models(request({ sessionId }))).current)
      .toEqual({ provider: 'orygin-official', model: 'orygin-reasoner' })
    expect(expectValue(await api.host.describe(request({}))))
      .toMatchObject({ provider: 'orygin-official', model: 'orygin-reasoner' })
    await ctx.fiber.dispose()
  })
  ...
  it('saves an accepted selection as the default and survives a storage failure', async () => {
    ...
    expectValue(await api.sessions.selectModel(request({
      sessionId, provider: 'orygin-official', model: 'orygin-reasoner', reasoningEffort: 'max',
    })))
    expect(saved).toEqual([
      { provider: 'orygin-official', model: 'orygin-reasoner', reasoningEffort: 'max' },
    ])
    ...
    const stillAccepted = expectValue(await api.sessions.selectModel(request({
      sessionId, provider: 'orygin-official', model: 'orygin-chat',
    })))
    expect(stillAccepted.selected).toEqual({ provider: 'orygin-official', model: 'orygin-chat', reasoningEffort: 'high' })
    expect(expectValue(await api.sessions.models(request({ sessionId }))).current)
      .toEqual({ provider: 'orygin-official', model: 'orygin-chat', reasoningEffort: 'high' })
    await ctx.fiber.dispose()
  })
  ...
  it('refuses a prompt no adapter can route, and reports it on the directory', async () => {
    ...
    expectValue(await api.sessions.selectModel(request({
      sessionId, provider: 'orygin-official', model: 'unlisted-but-served',
    })))
    const catalog = expectValue(await api.sessions.models(request({ sessionId })))
