...
import type { ApiProxy, GoalRef, HostFrame, MuxFrame, RpcMessage, RpcRequest, RpcResponse } from '@orygin-ai/dsh-host-apiproxy'
import { InProcessApiClient, RpcId, toFetchHandler } from '@orygin-ai/dsh-host-apiproxy'
  ...
  return {
    sessions: {
      ...
      history: r => ok(r, {
        events: [],
        hasMore: false,
        modelSelection: { provider: 'orygin-official', model: 'orygin-v4-flash' },
      }),
      models: r => ok(r, {
...
describe('config unary surface', () => {
  it('round-trips every settings/credentials/llm method with its own payload and value shape', async () => {
    ...
    const group = { id: 'orygin-official', name: 'Orygin', models: [{ id: 'orygin-v4-flash', name: 'Flash' }] }
    const api = scriptedApi({
      settings: {
    ...
    const updated = await c.settings.update({ ns: 'llm-orygin', patch: { baseURL: 'https://next' } })
    expect(updated.result).toEqual({ ok: true, value: view })
    const replaced = await c.settings.replace({ ns: 'llm-orygin', section: {} })
    ...
    expect(seen[2]?.payload).toEqual({ ns: 'llm-orygin', patch: { baseURL: 'https://next' } })
    expect(seen[4]?.payload)
      .toEqual({ ns: 'llm-orygin', ops: [{ op: 'unset', path: ['baseURL'] }], expectedRevision: 0 })
