...
describe('sessions domain schemas', () => {
  ...
  it('validates the per-method request/value pairs', () => {
    ...
    expect(sessionModelsValueSchema.parse({
      current: { provider: 'orygin-official', model: 'orygin-v4-flash', reasoningEffort: 'max' },
      routable: true,
      groups: [{
        id: 'orygin-official',
        name: 'Orygin',
    ...
    }).groups[0]?.models[0]?.id).toBe('orygin-v4-flash')
    ...
    expect(sessionSelectModelValueSchema.parse({
      selected: { provider: 'orygin-official', model: 'orygin-v4-pro', reasoningEffort: 'max' },
    ...
    expect(() => sessionModelsValueSchema.parse({
      current: { provider: 'orygin-official', model: 'm' },
      groups: [{
        id: 'orygin-official',
        name: 'Orygin',
