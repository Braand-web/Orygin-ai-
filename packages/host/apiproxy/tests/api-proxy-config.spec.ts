...
import SessionStore from '@orygin-ai/dsh-session'
...
import LlmRuntime, { LlmAdapter } from '@orygin-ai/dsh-llm'
import type { GenerateOptions, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@orygin-ai/dsh-llm'
import { SettingsProvider, settingsNamespace } from '@orygin-ai/dsh-settings'
import type { SettingsNamespace } from '@orygin-ai/dsh-settings'
import { CredentialProvider } from '@orygin-ai/dsh-credentials'
import type {
  CredentialInfo,
  CredentialKey,
  CredentialRecord,
  ...
  CredentialRef,
  ResolvedCredential,
} from '@orygin-ai/dsh-credentials'
import type { HostFrame } from '../src/api/index.ts'
import type { RpcRequest, RpcResponse } from '../src/api/rpc.ts'
import { RpcId } from '../src/api/rpc.ts'
import { AGENT_DEFAULT_MODEL_SETTINGS_NAMESPACE } from '@orygin-ai/dsh-agent-default-model'
...
const AdapterConfig = z.object({
  ...
  apiKeyEnv: z.string().default('ORYGIN_API_KEY'),
  baseURL: z.string(),
})
...
async function harness(options?: {
  ...
  if (options?.configurableProviders !== false) {
    ctx.llm.registerConfigurableProviders([
      { provider: 'orygin-official', displayName: 'Orygin', settingsNs: 'llm-orygin', settingsPath: [] },
    ])
  }
...
describe('settings domain', () => {
  ...
  it('describes layered redacted namespaces with their secret slots', async () => {
    const ctx = await harness({ settings: {
      doc: { 'llm-orygin': { apiKey: 'user-secret', baseURL: 'https://user' } },
    ...
    expect(view.ns).toBe('llm-orygin')
    expect(view.applies).toBe('live')
    expect((view.schema as { refs?: unknown }).refs).toBeDefined()
    expect(view.value).toEqual({ apiKeyEnv: 'ORYGIN_API_KEY', baseURL: 'https://user' })
  ...
  it('serves every registered namespace, including one this repository never named', async () => {
    ...
    ctx.settings.register(settingsNamespace('web-search-orygin'), z.object({
      baseURL: z.string(),
    }))
    const api = createApiProxy(ctx, DEFAULTS)
    ...
    expect(value.namespaces.map(view => view.ns)).toEqual([
      'llm-orygin', 'some-other-plugin', 'permission', 'ui-theme', 'locale',
      'ui-conversation', 'shell', 'agent-loop', 'web-search-orygin',
    ])
  ...
  it('keeps serving a provider namespace whose directory entry is gone', async () => {
    ...
    expect(expectOk(await api.settings.describe(request({}))).namespaces.map(view => view.ns))
      .toEqual(['llm-orygin'])
    expect(expectOk(await api.settings.update(request({ ns: 'llm-orygin', patch: { baseURL: 'https://x' } }))).value)
  ...
  it('forwards a provider settings change for model-catalog consumers', async () => {
    ...
    const frames = await collectHost(api, ['host/remote-event'], 1, async () => {
      await api.settings.update(request({ ns: 'llm-orygin', patch: { baseURL: 'https://base' } }))
    })
    expect(frames).toEqual([forwardedSettings('llm-orygin')])
    ...
    expect(expectOk(await api.settings.describe(request({}))).namespaces[0]!.value)
      .toEqual({ apiKeyEnv: 'ORYGIN_API_KEY', baseURL: 'https://base' })
  })
  ...
  it('forwards an Agent-default settings change for model-catalog consumers', async () => {
    ...
    }), { base: { provider: 'orygin-official', model: 'orygin-v4-flash' } })
    const api = createApiProxy(ctx, DEFAULTS)
    ...
    const frames = await collectHost(api, ['host/remote-event'], 1, async () => {
      await defaultModel.replace({ provider: 'orygin-official', model: 'orygin-reasoner' })
    })
  ...
  it('maps a stale expectedRevision to settings-conflict carrying both revisions', async () => {
    ...
    expect(expectOk(await api.settings.update(request({ ns: 'llm-orygin', patch: { baseURL: 'https://first' }, expectedRevision: opened })))
      .revision).toBe(opened + 1)
    const error = expectErr(await api.settings.update(request({ ns: 'llm-orygin', patch: { baseURL: 'https://second' }, expectedRevision: opened })))
    expect(error.code).toBe('settings-conflict')
    expect(error.details).toEqual({ ns: 'llm-orygin', expected: opened, actual: opened + 1 })
  ...
  it('updates the user layer, answers with the new redacted view, and broadcasts the frame', async () => {
    ...
    const api = createApiProxy(ctx, DEFAULTS)
    const frames = await collectHost(api, ['host/remote-event'], 1, async () => {
      const view = expectOk(await api.settings.update(request({ ns: 'llm-orygin', patch: { apiKey: 'sk-new', baseURL: 'https://next' } })))
      expect(view.value).toEqual({ apiKeyEnv: 'ORYGIN_API_KEY', baseURL: 'https://next' })
  ...
  it('replace resets the user layer wholesale', async () => {
    const ctx = await harness({ settings: { doc: { 'llm-orygin': { baseURL: 'https://user' } } } })
    ctx.settings.register(NS, AdapterConfig)
    const api = createApiProxy(ctx, DEFAULTS)
    const view = expectOk(await api.settings.replace(request({ ns: 'llm-orygin', section: {} })))
    expect(view.value).toEqual({ apiKeyEnv: 'ORYGIN_API_KEY' })
    expect(view.user).toEqual({})
  })

  it.each([
    ['an invalid namespace name', 'Not A Namespace', {}],
    ['a schema-invalid patch', 'llm-orygin', { baseURL: 42 }],
  ...
  it('maps a read-only provider refusal onto the same rejection', async () => {
    ...
    const error = expectErr(await api.settings.update(request({ ns: 'llm-orygin', patch: {} })))
    expect(error.code).toBe('settings-rejected')
...
describe('credentials domain', () => {
  ...
  it('maps a shadowed write onto credential-rejected for set and unset alike', async () => {
    const ctx = await harness({ credentials: { shadowed: ['ORYGIN_API_KEY'] } })
    const api = createApiProxy(ctx, DEFAULTS)
    const described = expectOk(await api.credentials.describe(request({ refs: ['ORYGIN_API_KEY'] })))
    expect(described.credentials['ORYGIN_API_KEY']).toEqual({ configured: true, source: 'env', writable: false })
    const setError = expectErr(await api.credentials.set(request({ ref: 'ORYGIN_API_KEY', value: 'x' })))
    expect(setError.code).toBe('credential-rejected')
    expect(setError.details).toEqual({ ref: 'ORYGIN_API_KEY' })
    const unsetError = expectErr(await api.credentials.unset(request({ ref: 'ORYGIN_API_KEY' })))
...
describe('llm domain', () => {
  it('merges the configurable directory with live routes and appends undeclared ones', async () => {
    ...
    ctx.llm.registerAdapter(['orygin-official'], new CatalogAdapter('Orygin', ['orygin-v4-flash']))
  ...
  it('serves the host-scoped catalog with per-provider failures contained', async () => {
    const ctx = await harness()
    ctx.llm.registerAdapter(['orygin-official'], new CatalogAdapter('Orygin', ['orygin-v4-flash', 'orygin-v4-pro']))
    ...
    expect(value.groups).toEqual([{
      ...
      name: 'Orygin',
      models: [
        { id: 'orygin-v4-flash', name: 'orygin-v4-flash' },
        { id: 'orygin-v4-pro', name: 'orygin-v4-pro' },
      ],
    }])
  ...
  it('forwards llm/adapters-updated at every topology commit point', async () => {
    ...
    const api = createApiProxy(ctx, DEFAULTS)
    const frames = await collectHost(api, ['host/remote-event'], 2, async () => {
      const dispose = ctx.llm.registerAdapter(['orygin-official'], new CatalogAdapter('Orygin', []))
      dispose()
      return Promise.resolve()
    })
    expect(frames).toEqual([
...
describe('llm.discoverModels', () => {
  ...
  it('carries the route being edited so an adapter can answer from its own registry', async () => {
    ...
    expect(probe).toEqual({ provider: 'orygin' })
