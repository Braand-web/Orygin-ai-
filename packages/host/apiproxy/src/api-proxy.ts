...
import { installModelSelection } from '@orygin-ai/dsh-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@orygin-ai/dsh-agent'
import type {} from '@orygin-ai/dsh-agent-presets/types'
import { AttachmentError, admitEncodedImages } from '@orygin-ai/dsh-attachment'
import type { ImageAttachmentRef } from '@orygin-ai/dsh-attachment'
import { createUserMessage, freezeMessage, ReasoningEffortId } from '@orygin-ai/dsh-llm'
import { errorChain } from '@orygin-ai/dsh-llm'
import type { ContentBlock, MessageSource } from '@orygin-ai/dsh-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@orygin-ai/dsh-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@orygin-ai/dsh-session'
import type { SessionPersistence } from '@orygin-ai/dsh-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@orygin-ai/dsh-session-query'
import { SubagentError } from '@orygin-ai/dsh-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@orygin-ai/dsh-subagent'
import { isUserInvocable } from '@orygin-ai/dsh-skill'
import type { Workspace, WorkspaceRecord } from '@orygin-ai/dsh-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@orygin-ai/dsh-workspace'
// Type-only: brings the `ctx.tools` Context merge into this program (viewFor reads presenters).
import {
  InvalidPresetIdError, PresetExistsError, PresetMountError,
  PresetNotWritableError, resolveSessionPreset, UnknownPresetError,
} from '@orygin-ai/dsh-agent-presets'
import type { PresetBearingSession } from '@orygin-ai/dsh-agent-presets'
import type {} from '@orygin-ai/dsh-tools'
import type {
...
import type { SessionRawArtifact } from '@orygin-ai/dsh-session-persistence'
import {
...
import type {} from '@orygin-ai/dsh-session-projection'
// Type-only: resolves `ctx.get('tasks')` to the background job registry.
import type {} from '@orygin-ai/dsh-jobs'
import type { JobSnapshot } from '@orygin-ai/dsh-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@orygin-ai/dsh-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@orygin-ai/dsh-goal'
import type { GoalRef as CoreGoalRef } from '@orygin-ai/dsh-goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@orygin-ai/dsh-commands'
...
import type {} from '@orygin-ai/dsh-cordis-host-runner/types'
import type {} from '@orygin-ai/dsh-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@orygin-ai/dsh-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@orygin-ai/dsh-settings'
import { credentialRef } from '@orygin-ai/dsh-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@orygin-ai/dsh-session-title'
...
} from '@orygin-ai/dsh-user-questions'
import { UserQuestionError } from '@orygin-ai/dsh-user-questions'
import { DirectoryPickerError } from '@orygin-ai/dsh-host-directory-picker'
import {
...
} from '@orygin-ai/dsh-api-remotes'
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @orygin-ai/dsh-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @orygin-ai/dsh-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @orygin-ai/dsh-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @orygin-ai/dsh-credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @orygin-ai/dsh-session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @orygin-ai/dsh-skill', details: {} })
        }
