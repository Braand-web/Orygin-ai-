...
import { LspConnection } from '@orygin-ai/dsh-lsp-stdio'
import type { ConnectionWriter } from '@orygin-ai/dsh-lsp-stdio/src/connection.ts'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit ORYGIN_* env entries to the child', async () => {
    // A configured ORYGIN_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'ORYGIN_LSP_TEST_FACT', ORYGIN_LSP_TEST_FACT: 'managed' })
