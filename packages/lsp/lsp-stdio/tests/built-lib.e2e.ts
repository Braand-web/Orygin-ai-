...
/**
 * Keyless built-artifact smoke: plain Node imports `@orygin-ai/dsh-lsp` and
 * `@orygin-ai/dsh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@orygin-ai/dsh-lsp')
      const LspLocal = await import('@orygin-ai/dsh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@orygin-ai/dsh-fs-local')
