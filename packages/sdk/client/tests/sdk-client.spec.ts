...
function harnessWith(env: Record<string, string> = {}, extra: LaunchOverrides = {}): OryginHarness {
  const harness = new OryginHarness({ launch: fakeLaunch(env, extra) })
...
describe('OryginHarness', () => {
  ...
  it('supports await using disposal', async () => {
    let captured: OryginHarness
    {
      await using harness = new OryginHarness({ launch: fakeLaunch() })
      captured = harness
...
describe('HarnessClient', () => {
  ...
  it('delivers notifications to unfiltered and filtered subscriptions in wire order', async () => {
    ...
    expect(identity.serverInfo.name).toBe('orygin-ai-sdk-runtime')
