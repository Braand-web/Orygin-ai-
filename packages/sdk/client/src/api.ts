/**
 * High-level run API over {@link HarnessClient}: `OryginHarness` owns one
 ...
 * Mirrors the Python SDK's `OryginHarness`/`Session` pair.
 *
 * @module @orygin-ai/dsh-sdk-client/api
 */
...
import type { ContentBlock, OryginHarnessOptions, HarnessClientOptions, HarnessNotification, RunResult } from './types.ts'

/**
 * Reusable SDK for running Orygin agent turns in a runtime
 ...
 * child is reaped.
 */
export class OryginHarness implements AsyncDisposable {
  ...
  constructor(options: OryginHarnessOptions) {
    ...
    this.provider = options.provider ?? 'orygin-official'
    this.model = options.model ?? 'orygin-v4-flash'
...
export class HarnessSession {
   ...
   */
  constructor(readonly harness: OryginHarness, readonly id: string) {}
