...
describe('dsh-sdk-jsonrpc-server plugin apply', () => {
  it('serves initialize over the injected stdio pair', async () => {
    ...
    try {
      harness.send({ jsonrpc: '2.0', id: 'init-1', method: 'initialize', params: { cwd: storageDir, provider: 'orygin-official', model: 'apply-model' } })

      const response = await harness.waitForFrame(frame => frame.id === 'init-1', 'initialize response')
      expect(response).toEqual({
        jsonrpc: '2.0',
        id: 'init-1',
        result: { serverInfo: { name: 'orygin-ai-sdk-runtime', version: '0.0.1' } },
      })
  ...
  it('does not answer initialize until async sibling Loader entries settle', async () => {
    ...
    try {
      const initialize = {
        ...
        id: 'init-delayed',
        method: 'initialize',
        params: { cwd: storageDir, provider: 'orygin-official', model: 'apply-model' },
      }
      ...
      expect(response).toMatchObject({
        id: 'init-delayed',
        result: { serverInfo: { name: 'orygin-ai-sdk-runtime' } },
      })
    } finally {
      release()
  ...
  it('drives a session/prompt turn end-to-end and forwards session notifications as output frames', async () => {
    ...
    const harness = await mountPlugin(storageDir)
    try {
      harness.send({ jsonrpc: '2.0', id: 1, method: 'initialize', params: { cwd: storageDir, provider: 'orygin-official', model: 'dsagent-model' } })
  ...
  it('answers shutdown before exiting 0 exactly once, even against a racing second shutdown', async () => {
    ...
    try {
      ...
      harness.send({ jsonrpc: '2.0', id: 'after-exit', method: 'initialize', params: { cwd: storageDir, provider: 'orygin-official', model: 'x' } })
      await settle()
  ...
  it('still disposes and exits once when the flush callback fails', async () => {
    ...
    try {
      ...
      const before = harness.frames().length
      harness.send({ jsonrpc: '2.0', id: 'after-flush-failure', method: 'initialize', params: { cwd: storageDir, provider: 'orygin-official', model: 'x' } })
      await settle()
      expect(harness.frames().length).toBe(before)
    } finally {
      await harness.dispose()
  ...
  it('stops serving on a bare fiber dispose (HMR-style unload) without calling exit', async () => {
    ...
    try {
      ...
      expect(error.error).toMatchObject({
        code: -32603,
        message: 'unknown Orygin SDK runtime method: nope/unknown',
      })
      ...
      const before = harness.frames().length
      harness.send({ jsonrpc: '2.0', id: 'probe-2', method: 'initialize', params: { cwd: storageDir, provider: 'orygin-official', model: 'x' } })
      await settle()
      expect(harness.frames().length).toBe(before)
      expect(harness.exits()).toEqual([])
    } finally {
