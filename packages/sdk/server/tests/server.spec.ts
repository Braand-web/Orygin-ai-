...
import AgentRegistry, { type Agent, type AgentHandle } from '@orygin-ai/dsh-agent'

import SessionStore, { SessionId } from '@orygin-ai/dsh-session'
import * as agentCore from '@orygin-ai/dsh-agent-spine-demo'
...
import SubagentRuntime, { type SubagentResult, type SubagentRunEndInfo } from '@orygin-ai/dsh-subagent'
import type { JsonRpcTransportPeer } from '@orygin-ai/dsh-sdk-protocol'
...
describe('HarnessSdkJsonRpcServer', () => {
  it('creates a harness agent and calls the configured OpenAI-compatible endpoint', { timeout: 15_000 }, async () => {
    ...
    vi.stubEnv('ORYGIN_BASE_URL', llmServer.url)
    ...
    try {
      ...
      expect(init.serverInfo.name).toBe('orygin-ai-sdk-runtime')
      ...
      const orphanHandle = await ctx.agents.create({
        ...
        meta: { cwd: storageDir },
        agentOptions: { provider: 'orygin-official', model: 'dsagent-model' },
      })
  ...
  it('creates an SDK session without an optional system prompt', { timeout: 15_000 }, async () => {
    ...
    try {
      ...
      await server.initialize({ cwd: storageDir, provider: 'orygin-official', model: 'plain-model' })
      await server.prompt({
        sessionId: 'plain',
  ...
  it('notifies the host when a subagent run settles', async () => {
    ...
    try {
      ...
      const parentHandle = await ctx.agents.create({
        ...
        meta: { cwd: storageDir },
        agentOptions: { provider: 'orygin-official', model: 'orygin-official' },
      })
      ...
      const parentlessHandle = await parentHandle.agent.ctx.agents.create({
        ...
        meta: { cwd: storageDir },
        agentOptions: { model: 'orygin-official' },
      })
  ...
  it('does not re-register an LLM adapter whose provider already has an owner', async () => {
    ...
    try {
      ...
      expect(inspect.hasAdapterFor('orygin-official')).toBe(true)
      expect(inspect.hasAdapterFor('missing-provider')).toBe(false)
      await server.initialize({ cwd: storageDir, provider: 'orygin-official', model: 'preinstalled-model' })

      expect(ctx.get('llm')?.listProviders().filter(provider => provider.id === 'orygin-official')).toEqual([{ id: 'orygin-official', name: 'Orygin' }])
      await server.shutdown()
    } finally {
      await ctx.fiber.dispose()
      await rm(storageDir, { recursive: true, force: true })
  ...
  it('rejects a missing non-Orygin provider when an LLM service already exists', async () => {
    ...
    try {
      ...
      expect(ctx.get('llm')?.listProviders()).toEqual([{ id: 'orygin-official', name: 'Orygin' }])
      await server.shutdown()
    } finally {
      await ctx.fiber.dispose()
      await rm(storageDir, { recursive: true, force: true })
  ...
  it('rejects unknown JSON-RPC runtime methods', async () => {
    ...
    try {
      ...
      await expect(server.handleRequest('does/not/exist', {}))
        .rejects
        .toThrow('unknown Orygin SDK runtime method: does/not/exist')
