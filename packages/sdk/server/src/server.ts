 ...
 *
 * @module @orygin-ai/dsh-sdk-jsonrpc-server/server
 */

import type { Context } from '@orygin-ai/cordis'
import { resolve } from 'node:path'
import type { Agent, AgentHandle } from '@orygin-ai/dsh-agent'
import { createUserMessage } from '@orygin-ai/dsh-llm'
import { carrierKeyOf, type Scoped } from '@orygin-ai/dsh-scope'
import { SessionId } from '@orygin-ai/dsh-session'
import type SubagentRuntime from '@orygin-ai/dsh-subagent'
import type { SubagentRunEndInfo } from '@orygin-ai/dsh-subagent'
...
export class HarnessSdkJsonRpcServer {
  ...
  private provider = 'orygin-official'
  private model = 'orygin-official'
  ...
  /**
   * Configure the SDK route, mounting the Orygin fallback only when unowned.
  ...
  async initialize(params: InitializeParams): Promise<InitializeResult> {
    ...
    if (!this.hasAdapterFor(this.provider)) {
      if (this.provider !== 'orygin-official') throw new Error(`no adapter registered for provider "${this.provider}"`)
      this.llmFiber = await this.ctx.plugin(LlmOrygin, {})
    }
    return { serverInfo: { name: 'orygin-ai-sdk-runtime', version: '0.0.1' } }
  }
  ...
  async handleRequest(method: string, params: Record<string, unknown> | undefined): Promise<unknown> {
    switch (method) {
      ...
      default:
        throw new Error(`unknown Orygin SDK runtime method: ${method}`)
    }
  }
  ...
  private async createSession(sessionId: string): Promise<SessionRecord> {
    ...
    // (@orygin-ai/dsh-agent-presets README, "Composing a child agent").
