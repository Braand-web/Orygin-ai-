...
import {
  createLaunchEnvironmentSnapshot, ORYGIN_LAUNCH_ENVIRONMENT_KEY, launchEnvironmentOf,
} from '../src/index.ts'
...
describe('launchEnvironmentOf', () => {
  it('returns the launcher snapshot when the product CLI provided one', () => {
    ...
    ctx.provide(ORYGIN_LAUNCH_ENVIRONMENT_KEY, layered)
    expect(launchEnvironmentOf(ctx)).toBe(layered)
  })

  it('falls back to the inherited environment as the only layer', () => {
    vi.stubEnv('ORYGIN_ENV_SPEC_FALLBACK', 'ambient')
    try {
      const snapshot = launchEnvironmentOf(new Context())
      expect(snapshot.get('ORYGIN_ENV_SPEC_FALLBACK')).toEqual({ value: 'ambient', source: 'process' })
    } finally {
      vi.unstubAllEnvs()
    }
  })
