...
`resolveDshHome()` resolves the single-root Orygin home. Precedence, highest first: an explicit configured path, `$ORYGIN_HOME`, then `~/.dsh`. The harness keeps all user data under one root.
...
`dshHomeDisplay()` names an active root symbolically for user-facing paths: `~/.dsh` for the default home, `$ORYGIN_HOME` for any configured home. It never leaks an absolute machine path.

`ORYGIN_HOME_DIR_NAME` owns the default user-data directory name: `.dsh`.

`defaultDshHome()` returns the default Orygin home by joining the operating-system home directory with `.dsh`, using Node's platform path rules.
