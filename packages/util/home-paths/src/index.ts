/**
 * Shared filesystem path helpers for Orygin user data.
 *
 * @module @orygin-ai/dsh-home-paths
 */
...
/** Directory name for the default Orygin home under the OS home. */
export const ORYGIN_HOME_DIR_NAME = '.dsh'

/** Stable user-facing display form for the default Orygin home. */
export const DEFAULT_ORYGIN_HOME_DISPLAY = `~/${ORYGIN_HOME_DIR_NAME}`

/** Environment variable that overrides the default Orygin home. */
export const ORYGIN_HOME_ENV = 'ORYGIN_HOME'
...
/**
 * Resolve the default Orygin home using Node's platform path rules.
 * @returns the absolute default harness home path.
 */
export function defaultDshHome(): string {
  return join(homedir(), ORYGIN_HOME_DIR_NAME)
}
...
/**
 * Resolve the single-root Orygin home.
 *
 * Precedence, highest first: an explicit configured path, `$ORYGIN_HOME`, then
 * `~/.dsh`. The harness keeps all user data under one root. An empty or
 * whitespace-only `$ORYGIN_HOME` is treated as unset, so a blank override never
 * resolves the home to the current working directory.
 * @param configured - explicit harness-home override, which has highest precedence.
 * @param env - environment mapping used to read `ORYGIN_HOME`.
 * @returns the normalized absolute harness home path.
 */
export function resolveDshHome(configured?: string, env: Record<string, string | undefined> = process.env): string {
  const fromEnv = env[ORYGIN_HOME_ENV]
  const selected = configured ?? (fromEnv !== undefined && fromEnv.trim().length > 0 ? fromEnv : defaultDshHome())
  return resolve(expandHomePath(selected))
}

/**
 * Join path segments onto the resolved Orygin home.
 ...
 * `~/.dsh`, and any configured home is labelled `$ORYGIN_HOME`.
 * @param resolvedHome - the absolute path returned by {@link resolveDshHome}.
 * @returns `~/.dsh` for the default home, otherwise `$ORYGIN_HOME`.
 */
export function dshHomeDisplay(resolvedHome: string): string {
  return resolvedHome === resolve(defaultDshHome()) ? DEFAULT_ORYGIN_HOME_DISPLAY : `$${ORYGIN_HOME_ENV}`
}
