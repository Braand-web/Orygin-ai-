...
describe.skipIf(!process.env.ORYGIN_API_KEY)('spawn backend with-key smoke', () => {
  it('a parent delegates to a child that writes a file on disk', async () => {
    ...
    const parent = ctx.agentLoop.create(SessionId('e2e-parent'), { provider: 'orygin-official', model: 'orygin-v4-flash' })
