...
The standalone composition below shows the complete explicit capability. A Profile based on `@orygin-ai/dsh-base` keeps its existing Job rows, adds the product provider and tool rows, and does not mount duplicate Job services.

```yaml
- id: subagent-codex-safe
  name: '@orygin-ai/dsh-subagent-codex'
  config:
...
```yaml
- id: jobs
  name: '@orygin-ai/dsh-jobs-local'

- id: tool-jobs
  name: '@orygin-ai/dsh-tool-jobs'
...
- **Compatibility is pinned by development evidence** — upgrading from the verified 0.147.0 protocol baseline requires regenerating upstream schema evidence and rerunning handshake, answer-selection, approval, cancellation, keyless real-product, and credentialed Orygin nonce tests.
- **No human approval path** — known unattended approval requests are denied and unknown server requests fail closed; the three Profile modes never create a ORYGIN interaction channel or per-call allow policy.
