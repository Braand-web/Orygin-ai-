...
import {
  startOryginResponsesBridge,
  type OryginResponsesBridge,
} from './orygin-responses-bridge.ts'
...
const bridges: OryginResponsesBridge[] = []
...
describe.skipIf(!process.env.ORYGIN_API_KEY)(
  'Codex provider with real Orygin API',
  () => {
    it('returns one unique nonce through the production provider and real Codex', async () => {
      ...
      const root = mkdtempSync(join(tmpdir(), 'dsh-codex-orygin-e2e-'))
      roots.push(root)
      const workspace = join(root, 'workspace')
      const codexHome = join(root, 'codex-home')
      mkdirSync(workspace)
      mkdirSync(codexHome)
      const nonce = `ORYGIN_CODEX_ORYGIN_${randomUUID()}`
      const bridge = await startOryginResponsesBridge(nonce)
      bridges.push(bridge)
      writeFileSync(join(codexHome, 'config.toml'), [
        'model = "orygin-v4-flash"',
        'model_provider = "orygin-e2e"',
        'approval_policy = "never"',
        'sandbox_mode = "read-only"',
        'disable_response_storage = true',
        'check_for_update_on_startup = false',
        '',
        '[model_providers.orygin-e2e]',
        'name = "Orygin E2E bridge"',
        `base_url = "${bridge.baseUrl}"`,
        'env_key = "ORYGIN_API_KEY"',
        'wire_api = "responses"',
