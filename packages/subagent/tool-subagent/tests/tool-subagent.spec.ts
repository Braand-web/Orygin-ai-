...
import ToolRuntime, { TOOL_ABORTED_BEFORE_DISPATCH } from '@orygin-ai/dsh-tools'
import { assembleContextFor, type Agent } from '@orygin-ai/dsh-agent'
...
import type { SubagentStartRequest } from '@orygin-ai/dsh-subagent'
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @orygin-ai/dsh-jobs')
  })
