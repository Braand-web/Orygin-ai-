/**
 * Fresh-process SDK subagent client. Drives one child Orygin
 * runtime over stdio JSON-RPC through `@orygin-ai/dsh-sdk-client` and owns
 * cancellation and quiescent disposal. Structure mirrors the ACP backend
 * (`@orygin-ai/dsh-subagent-acp`): publish after the child handshake,
 ...
 *
 * @module @orygin-ai/dsh-subagent-dsh-sdk/run
 */

import { randomUUID } from 'node:crypto'
import { OryginHarness, type HarnessNotification } from '@orygin-ai/dsh-sdk-client'
import type { ContentBlock } from '@orygin-ai/dsh-llm'
import { SessionId, type SessionEvent, type TurnEndReason } from '@orygin-ai/dsh-session'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@orygin-ai/dsh-subagent'
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@orygin-ai/dsh-subagent'
...
export interface SdkRunSpec {
   ...
   * runtime's own `ORYGIN_API_KEY`, or `ORYGIN_CORDIS_CONFIG`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `ORYGIN_*` fact survives while ambient namesakes never leak.
   */
