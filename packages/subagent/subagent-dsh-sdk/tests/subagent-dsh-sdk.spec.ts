...
describe('dsh-subagent-dsh-sdk provider', () => {
  ...
  it('scrubs ambient credentials but forwards explicit config env', async () => {
    process.env.ORYGIN_TEST_AMBIENT_SECRET_KEY = 'leak-me-not'
    try {
      const ctx = await setup({
        FAKE_ECHO_ENV: 'ORYGIN_TEST_AMBIENT_SECRET_KEY,ORYGIN_API_KEY',
        ORYGIN_API_KEY: 'explicit-child-key',
        FAKE_TEXT: 'done',
      })
      const run = await ctx.subagents.start('dsh-sdk', request())
      const result = await run.result
      const answer = text(result.output)
      expect(answer).toContain('ORYGIN_TEST_AMBIENT_SECRET_KEY=\n')
      expect(answer).toContain('ORYGIN_API_KEY=explicit-child-key')
      await run.dispose()
      await ctx.fiber.dispose()
    } finally {
      delete process.env.ORYGIN_TEST_AMBIENT_SECRET_KEY
    }
  })
