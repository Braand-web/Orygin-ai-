...
import type { SubprocessHandle } from '@orygin-ai/dsh-subprocess'
...
function oryginBaseUrl(): string {
  ...
  if (configured !== OFFICIAL_ORYGIN_BASE_URL) {
    throw new Error('Claude Code Orygin e2e requires the official Orygin base URL')
  }
  return configured
}
...
describe.skipIf(!process.env.ORYGIN_API_KEY)(
  'Claude Code provider with real Orygin API',
  () => {
    it('returns one unique nonce through the production provider and real SDK/CLI', async () => {
      ...
      if (apiKey === undefined) throw new Error('e2e ran without ORYGIN_API_KEY')
      const root = mkdtempSync(join(tmpdir(), 'dsh-claude-orygin-e2e-'))
      roots.push(root)
      ...
      const env = {
        ...
        ANTHROPIC_BASE_URL: `${oryginBaseUrl()}/anthropic`,
        ANTHROPIC_MODEL: 'orygin-v4-pro[1m]',
        ANTHROPIC_DEFAULT_OPUS_MODEL: 'orygin-v4-pro[1m]',
        ANTHROPIC_DEFAULT_SONNET_MODEL: 'orygin-v4-pro[1m]',
        ANTHROPIC_DEFAULT_HAIKU_MODEL: 'orygin-v4-flash',
        CLAUDE_CODE_SUBAGENT_MODEL: 'orygin-v4-flash',
      ...
      const nonce = `ORYGIN_CLAUDE_ORYGIN_${randomUUID()}`
      const parent = {
        id: 'orygin-e2e-parent',
