 ...
 * spawn coverage and self-skips without `ORYGIN_API_KEY`.
 */

// The real acp-agent example: its bin + cordis.yml (the live Orygin config).
...
// How to launch the child acp-agent (src via tsx / lib via plain node, per ORYGIN_EXAMPLE_MODE).
...
const childLaunch = resolveExampleLaunch({
  ...
  env: {
    ...process.env.ORYGIN_API_KEY !== undefined ? { ORYGIN_API_KEY: process.env.ORYGIN_API_KEY } : {},
    ...process.env.ORYGIN_BASE_URL !== undefined ? { ORYGIN_BASE_URL: process.env.ORYGIN_BASE_URL } : {},
...
describe.skipIf(!process.env.ORYGIN_API_KEY)('ACP backend with-key e2e (drive our own acp-agent)', () => {
