...
import type { SubprocessOutcome } from '@orygin-ai/dsh-subprocess'
...
import { spawnSubprocess } from '@orygin-ai/dsh-subprocess-local/src/spawn.ts'
...
describe('child env layering (through the subprocess seam)', () => {
    ...
    try {
      ...
      const running = spawnSubprocess({
        argv: [
          ...
          '--input-type=module',
          '--eval',
          'process.stdout.write(JSON.stringify([process.env.ACP_TEST_AMBIENT_SECRET_TOKEN ?? "absent", process.env.ORYGIN_API_KEY]))',
        ],
        cwd: process.cwd(),
        stdio: { stdin: 'ignore', stdout: { maxBytes: 1000 }, stderr: { maxBytes: 1000 } },
        graceMs: 1000,
        env: { ORYGIN_API_KEY: 'explicit' },
      })
      await running.done
  ...
  it('forwards explicit ORYGIN_* config entries to the child', async () => {
    // A deployment sets child-harness facts like ORYGIN_PERMISSION_MODE in
    ...
    const ctx = await setup({ MOCK_ECHO_ENV: 'ORYGIN_ACP_TEST_FACT', ORYGIN_ACP_TEST_FACT: 'managed' })
...
describe('cwd resolution', () => {
  ...
  it('resolves a relative config cwd against the launch directory at load', async () => {
    ...
    // ABSOLUTE form — ORYGIN's own ACP server rejects a relative session cwd, and
