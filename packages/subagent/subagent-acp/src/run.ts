 ...
 * @module @orygin-ai/dsh-subagent-acp/run
 */
...
import { AssistantOutputFold } from '@orygin-ai/dsh-subagent'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@orygin-ai/dsh-subagent'
import type { SubprocessHandle, SubprocessSpawnSpec } from '@orygin-ai/dsh-subprocess'
...
export interface AcpRunSpec {
   ...
   * `ORYGIN_API_KEY`). Merged on top of the subprocess seam's scrubbed
   ...
   * Explicit `ORYGIN_*` entries are deployment-owned facts for the child harness
   * (e.g. `ORYGIN_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and ORYGIN_* names
