...
import { recordFeedback } from '@orygin-ai/dsh-command-feedback'
...
// pin ORYGIN_HOME to a temp dir so the suite never touches the ambient ~/.dsh.
let tempHome: string
let previousDshHome: string | undefined
beforeAll(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'dsh-otel-home-'))
  previousDshHome = process.env.ORYGIN_HOME
  process.env.ORYGIN_HOME = tempHome
})
afterAll(() => {
  if (previousDshHome === undefined) delete process.env.ORYGIN_HOME
  else process.env.ORYGIN_HOME = previousDshHome
...
describe('OpenTelemetrySessionBackend wire', () => {
  it('ships session records and the ops shutdown marker through the real SDK pipeline', async () => {
    ...
    expect(resource).toContainEqual({ key: 'service.name', value: { stringValue: 'orygin-ai' } })
    expect(resource).toContainEqual({ key: 'user.id', value: { stringValue: getOrCreateAnonymousUserId() } })

    const records = allRecords(captures)
    const ledger = records.filter(r => r.scope === '@orygin-ai/dsh-session-telemetry-otel')
    const ops = records.filter(r => r.scope === '@orygin-ai/dsh-session-telemetry-otel/ops')
  ...
  it('drains records enqueued after a timer export began: dispose during an in-flight batch', async () => {
    ...
    const ops = allRecords(captures).filter(r => r.scope === '@orygin-ai/dsh-session-telemetry-otel/ops')
    expect(ops).toHaveLength(1)
    expect(ops[0]!.record.attributes).toContainEqual({ key: 'telemetry.op', value: { stringValue: 'shutdown' } })
  })
