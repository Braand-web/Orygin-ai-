...
import SessionPersistenceSqlite, {
  ...
  SCHEMA_VERSION,
} from '@orygin-ai/dsh-session-persistence-sqlite'
import {
...
describe('SessionPersistenceSqlite physical packing', () => {
  it('loads from cordis.yml and packs through the assembled service', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@orygin-ai/dsh-session-persistence-sqlite'",
      '  config:',
    ...
    ctx.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@orygin-ai/dsh-session') return SessionStore
        if (specifier === '@orygin-ai/dsh-session-persistence-sqlite') {
