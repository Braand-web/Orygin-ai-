...
import * as FirstMessageTitleProvider from '@orygin-ai/dsh-session-title-first-prompt-llm'
...
describe.skipIf(!process.env.ORYGIN_API_KEY)('first-prompt title provider with real Orygin API', () => {
  it('replaces the fallback with a short model title', async () => {
    const ctx = new Context()
    contexts.push(ctx)
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmOrygin, { thinking: 'disabled' })
    ...
    expect(title).toMatchObject({
      ...
      source: {
        ...
        model: { provider: 'orygin-official', model: 'orygin-v4-flash' },
      },
    })
