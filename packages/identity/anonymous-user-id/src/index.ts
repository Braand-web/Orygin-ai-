 ...
 * harness home resolved by {@link resolveDshHome} (`$ORYGIN_HOME` > `~/.dsh`),
 ...
 * machine: every process sharing one `$ORYGIN_HOME` reports the same id, and
 ...
 *
 * @module @orygin-ai/dsh-anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `ORYGIN_HOME`; defaults to `process.env`. */
