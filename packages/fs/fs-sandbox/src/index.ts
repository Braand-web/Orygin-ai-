 ...
 * `@orygin-ai/dsh-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@orygin-ai/dsh-bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@orygin-ai/dsh-tool-fs`),
 * exactly as bash's does.
 *
 * @module @orygin-ai/dsh-fs-sandbox
 */

import { Context } from '@orygin-ai/cordis'
import { LocalFileSystem } from '@orygin-ai/dsh-fs-local'
import type { Config as LocalConfig } from '@orygin-ai/dsh-fs-local'
import { FsError } from '@orygin-ai/dsh-fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@orygin-ai/dsh-fs'
import { writableRoots } from '@orygin-ai/dsh-sandbox'
