...
describe.skipIf(!process.env.ORYGIN_API_KEY)('fs tools with-key smoke', () => {
  it('creates, reads, then edits a file — verified on disk', async () => {
    ...
    const agent = ctx.agentLoop.create(SessionId('fs-e2e'), { provider: 'orygin-official', model: 'orygin-v4-flash' })
