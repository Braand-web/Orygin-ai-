 ...
 * @module @orygin-ai/dsh-sandbox-local
 */
...
} from '@orygin-ai/node-addon-landlock-run'
import { Context } from '@orygin-ai/cordis'
import z from '@orygin-ai/schemastery'
import { assertNever } from '@orygin-ai/dsh-llm'
import { SandboxProvider, SandboxUnavailableError } from '@orygin-ai/dsh-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@orygin-ai/dsh-sandbox'
import type { SessionId } from '@orygin-ai/dsh-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@orygin-ai/dsh-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@orygin-ai/dsh-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@orygin-ai/dsh-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@orygin-ai/dsh-sandbox-windows-acl/src/runner.ts'))
