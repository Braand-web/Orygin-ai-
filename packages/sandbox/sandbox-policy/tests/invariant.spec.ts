...
import SessionStore, { type Session, type SessionEvent } from '@orygin-ai/dsh-session'
import InvariantRegistry, { InvariantError } from '@orygin-ai/dsh-invariants'
import * as SandboxPolicyInvariant from '@orygin-ai/dsh-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@orygin-ai/dsh-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => undefined)).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@orygin-ai/dsh-sandbox-policy',
    })
  })
})
