/**
 * @orygin-ai/dsh-web-app — the browser-surface bundle's runtime glue plugin
...
import { addHarnessSourceSection } from '@orygin-ai/dsh-app-boot'
import * as FrontendStatic from '@orygin-ai/dsh-host-frontend-static'
import { launchEnvironmentOf } from '@orygin-ai/dsh-launch-environment'
import { scrubbedParentEnv } from '@orygin-ai/dsh-subprocess'
import type {} from '@orygin-ai/cordis-plugin-loader'
...
const ORYGIN_WEB_URL = 'ORYGIN_WEB_URL' as const
...
function webSurfacePrompt(webUrl: string): string {
  ...
  return `You are interacting with the user through the Orygin Web GUI at ${webUrl}. `
    + 'When the user refers to "this page", "this GUI", or "this app" without naming another target, they mean this GUI. '
    + 'The browser provides no implicit DOM, route, or screenshot context. '
    + updateContract
    + 'Starting another server does not update this GUI. '
    + 'The apps/web Vite entry builds the shell but is not a standalone application because only dsh web injects window.__ORYGIN_BOOT__. '
...
function resolveDistIndex(): string {
  ...
  try {
    return require.resolve('@orygin-ai/dsh-web-frontend/dist/index.html')
  } catch {
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [ORYGIN_WEB_URL]: { description: 'Canonical local URL of the Orygin Web GUI serving this session.' },
        },
        resolve: () => ({ [ORYGIN_WEB_URL]: localWebUrl(runtimeCtx) }),
      })
    })
  }
