...
import type { WebServer } from '@orygin-ai/dsh-host-webserver'
...
describe('web-app runtime glue', () => {
  it('mounts dist serving, prompt section, bash variables, and publishes the URL with the LAN snapshot', async () => {
    ...
    ctx.provide(ORYGIN_LAUNCH_ENVIRONMENT_KEY, createLaunchEnvironmentSnapshot([
    ...
    expect(assembly.sections.find(entry => entry.name === 'harness:source')?.text).toContain('Orygin implementation checkout')
    ...
    expect(webRuntime?.resolve()).toEqual({ ORYGIN_WEB_URL: 'http://127.0.0.1:4567' })
  ...
  it('scrubs the helper environment and reports helper spawn or exit failures', async () => {
    vi.stubEnv('ORYGIN_API_KEY', 'must-not-reach-browser')
    vi.stubEnv('ORYGIN_HOME', '/must-not-reach-browser')
    ...
    expect(options?.env).not.toHaveProperty('ORYGIN_API_KEY')
    expect(options?.env).not.toHaveProperty('ORYGIN_HOME')
