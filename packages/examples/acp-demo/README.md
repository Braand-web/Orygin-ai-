...
ACP automation server app: the default agent spine, client-created agents through [`@orygin-ai/dsh-acp`](../../acp/acp/README.md), JSONL persistence, and semantic checkpointing behind one JSON-RPC stdio bin. Programmatic clients create fresh sessions; this package mounts no human UI.

## Composition

| Plugin | Role |
|---|---|
| `@orygin-ai/dsh-agent-spine-demo` | Providerless agent spine with no pre-created agents; `session/new` creates each agent. |
| `@orygin-ai/dsh-session-persistence-jsonl` | Durable session logs used by checkpointing, observability, and snapshot replay. |
| `@orygin-ai/dsh-session-checkpoint-policy` | Durability barriers before model calls and top-level tool effects, plus completed-step checkpoints. |
| `@orygin-ai/dsh-session-query-sqlite` | Derived exact/FTS session-query service, opened before the ACP transport so leaf consumers are ready for the first model request. |
| `@orygin-ai/dsh-acp` | Automation-only ACP transport over stdin/stdout. |
...
| `dshHome` | `$ORYGIN_HOME` or `~/.dsh` | Harness home shared by bash and local skill discovery. |
