...
import { agentEvents } from '@orygin-ai/dsh-agent'
import { TOOL_ORDER_REST } from '@orygin-ai/dsh-system-prompt'
...
/**
 * In-process unit coverage for the @orygin-ai/dsh-acp-demo composition:
...
async function withIsolatedSkillHomes<T>(run: () => Promise<T>): Promise<T> {
  ...
  process.env.ORYGIN_HOME = join(home, '.dsh')
  process.env.ORYGIN_AGENTS_HOME = join(home, '.agents')
  try {
    return await run()
  } finally {
