...
import { renderPrompt, TOOL_ORDER_REST } from '@orygin-ai/dsh-system-prompt'
import * as agentCore from '../src/index.ts'
import { agentEvents, type Agent } from '@orygin-ai/dsh-agent'
...
declare module '@orygin-ai/dsh-jobs' {
...
/**
 * Unit coverage for the @orygin-ai/dsh-agent-spine-demo bundle: mounting it brings
...
async function mount(config: agentCore.Config, withBash = false): Promise<Context> {
  const oldDshHome = process.env.ORYGIN_HOME
  const oldAgentsHome = process.env.ORYGIN_AGENTS_HOME
  process.env.ORYGIN_HOME = await mkdtemp(join(tmpdir(), 'dsh-agent-spine-demo-home-'))
  process.env.ORYGIN_AGENTS_HOME = await mkdtemp(join(tmpdir(), 'dsh-agent-spine-demo-agents-'))
  ...
  try {
    ...
    if (oldDshHome === undefined) {
      delete process.env.ORYGIN_HOME
    } else {
      process.env.ORYGIN_HOME = oldDshHome
    }
    if (oldAgentsHome === undefined) {
      delete process.env.ORYGIN_AGENTS_HOME
    } else {
      process.env.ORYGIN_AGENTS_HOME = oldAgentsHome
    }
  }
}
...
describe('dsh-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@orygin-ai/dsh-agent$'] },
      { package_blocklist: ['^@orygin-ai/dsh-session$'] },
    ]) {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    ...
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent powered by Orygin.')
  ...
  it('shares top-level dshHome between local skills and the managed bash environment', async () => {
    ...
    expect(ctx.shellEnv.collect(execution)).toMatchObject({ ORYGIN_HOME: home, ORYGIN_SHELL: '1' })
    await ctx.fiber.dispose()
  })

  it('rejects conflicting global and nested ORYGIN home directories', () => {
    expect(() => {
