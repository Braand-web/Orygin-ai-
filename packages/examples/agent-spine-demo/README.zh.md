...
```
@orygin-ai/cordis-plugin-timer  timer service (writes nothing to stdout)
@orygin-ai/dsh-llm              abstract LLM service + content-block vocabulary
@orygin-ai/dsh-session          event-sourced session log + store
@orygin-ai/dsh-session-title    log-backed title service + deterministic fallback
@orygin-ai/dsh-system-prompt    prompt-section + tool-schema assembly
@orygin-ai/dsh-tools            registry + guarded pre/around/post/final-result pipeline
@orygin-ai/dsh-skill            skill provider registry
@orygin-ai/dsh-skill-filesystem      local filesystem skill provider
@orygin-ai/dsh-agent            agent registry + initiator scope + agent/* events
@orygin-ai/dsh-goal             optional persisted same-session goal domain
@orygin-ai/dsh-tool-goal        optional model-facing goal controls
@orygin-ai/dsh-goal-round-driver     optional same-session goal-round driver
@orygin-ai/dsh-llm-retry        provider-routed request retry policy
@orygin-ai/dsh-jobs-local      generic background-job registry
@orygin-ai/dsh-invariants       configurable invariant registry service
@orygin-ai/dsh-session/invariant
@orygin-ai/dsh-agent/invariant
@orygin-ai/dsh-scope/invariant
@orygin-ai/dsh-agent-loop/invariant
                                  package-owned relational checks
@orygin-ai/dsh-tool-bash        the model-facing bash schema (unless toolBash=false)
@orygin-ai/dsh-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@orygin-ai/dsh-tool-skill       session-prefix skill catalog + model-facing loader schema
@orygin-ai/dsh-tool-jobs       job_output/job_list/job_kill schemas + completion notices
@orygin-ai/dsh-agent-loop       THE concrete loop (gets the forwarded `agents`)
...
- **LLM（大语言模型）适配器**：组合包交付抽象 `llm` 服务；叶节点在 `ctx.llm` 上注册具体适配器（`llm-orygin`、`llm-pi-ai`、`llm-replay`）。
...
```ts
import type { Config } from '@orygin-ai/dsh-agent-spine-demo'
...
例如，`{ invariants: { enabled: true, package_allowlist: ['^@orygin-ai/dsh-'], package_blocklist: ['agent-loop$'] } }` 会让包拥有的配套插件保持挂载，但抑制被阻止的拥有者。Blocklist 匹配优先于 allowlist 匹配；正则表达式与生命周期规则见 [`dsh-invariants`](../../runtime-diagnostics/invariants/README.zh.md)。
