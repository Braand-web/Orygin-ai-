...
import Storage from '@orygin-ai/dsh-storage'
import * as StorageDomain from '@orygin-ai/dsh-storage-domain'
import * as StorageJson from '@orygin-ai/dsh-storage-json'
import { remoteMethods } from '@orygin-ai/dsh-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@orygin-ai/dsh-session', SessionStore],
    ['@orygin-ai/dsh-session-persistence-jsonl', JsonlSessionPersistence],
    ['@orygin-ai/dsh-storage', Storage],
    ['@orygin-ai/dsh-storage-json', StorageJson],
    ['@orygin-ai/dsh-storage-domain', StorageDomain],
    ['@orygin-ai/dsh-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      "- name: '@orygin-ai/dsh-session'",
      "- name: '@orygin-ai/dsh-session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@orygin-ai/dsh-storage'",
      "- name: '@orygin-ai/dsh-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@orygin-ai/dsh-storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@orygin-ai/dsh-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
