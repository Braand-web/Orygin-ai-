...
import type { Agent, AgentStatus } from '@orygin-ai/dsh-agent'
import CommandRuntime from '@orygin-ai/dsh-commands'
import SessionStore, { SessionId } from '@orygin-ai/dsh-session'
import * as CommandFeedback from '@orygin-ai/dsh-command-feedback'
...
describe('/feedback real Loader composition through cordis.yml', () => {
  it('boots cordis.yml and records feedback without model-visible output', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@orygin-ai/dsh-commands'",
      "- name: '@orygin-ai/dsh-command-feedback'",
      '',
    ].join('\n'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@orygin-ai/dsh-commands', CommandRuntime],
      ['@orygin-ai/dsh-command-feedback', CommandFeedback],
    ])
    ...
    const userId = getOrCreateAnonymousUserId({ env: { ORYGIN_HOME: root } })
