...
Sandbox-consuming Service Provider for the [`@orygin-ai/dsh-shell`](../shell/) executor seam. Load it **instead of** `@orygin-ai/dsh-bash-local`, together with a [`ctx.sandbox`](../../sandbox/sandbox/) provider (e.g. [`@orygin-ai/dsh-sandbox-local`](../../sandbox/sandbox-local/)) and a [`ctx.sandboxPolicy`](../../sandbox/sandbox-policy/) (which owns the default mode + workspace root, shared with the sandboxed filesystem) — no alternate tool plugin is needed; `dsh-tool-bash` detects the executor's `sandboxMode` capability and adds the escalation fields.
...
```yaml
- id: sandbox
  name: '@orygin-ai/dsh-sandbox-local'
- id: sandbox-policy
...
- id: bash
  name: '@orygin-ai/dsh-bash-sandbox'
```
