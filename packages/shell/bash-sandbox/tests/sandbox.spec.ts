...
import type { ShellRunResult, CollectedOutput } from '@orygin-ai/dsh-shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@orygin-ai/dsh-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@orygin-ai/dsh-sandbox'
import { SandboxPolicyService } from '@orygin-ai/dsh-sandbox-policy'
import { SandboxBashExecutor } from '@orygin-ai/dsh-bash-sandbox'
...
import type { Config } from '@orygin-ai/dsh-bash-sandbox'
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'ORYGIN_WRAP=1', 'bash', '-c', 'printf "%s" "$ORYGIN_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$ORYGIN_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.ORYGIN_ORDER_FILE, "runner\\n");',
    ...
    try {
      const result = await bash.run(bash.resolve({
        command: 'true',
        env: { BASH_ENV: hook },
        dshEnv: { ORYGIN_ORDER_FILE: order },
      }))
