 ...
 * managed `ORYGIN_*` collection, abort translation, canonical result projection,
...
import SystemPrompt, { renderPrompt } from '@orygin-ai/dsh-system-prompt'
...
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from '@orygin-ai/dsh-shell'
...
import type { ShellProcessRead } from '@orygin-ai/dsh-shell'
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed ORYGIN_* environment', async () => {
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    expect(dshEnv?.['ORYGIN_SHELL']).toBe('1')
    expect(dshEnv?.['ORYGIN_HOME']).toEqual(expect.any(String))
    expect(dshEnv).not.toHaveProperty('ORYGIN_SESSION_ID')
  })
