 ...
 * `@orygin-ai/dsh-pwsh-local`) backs `ctx.shell`; the tool contract is
 ...
 * generic `ctx.jobs` runtime), the managed `ORYGIN_*` environment through the
 ...
 * exit-status parse from `@orygin-ai/dsh-shell`.
 *
 * @module @orygin-ai/dsh-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@orygin-ai/cordis'
import z from '@orygin-ai/schemastery'
import { defineTool, TOOL_ABORTED } from '@orygin-ai/dsh-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@orygin-ai/dsh-tools'
import { HarnessError } from '@orygin-ai/dsh-llm'
...
import type { SandboxExecutionPolicy, SandboxMode } from '@orygin-ai/dsh-sandbox'
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@orygin-ai/dsh-sandbox'
import type { SandboxPolicyService } from '@orygin-ai/dsh-sandbox-policy'
import type { ShellRunResult } from '@orygin-ai/dsh-shell'
import { parseExitStatus } from '@orygin-ai/dsh-shell'
...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a PowerShell command (`pwsh -Command`) and return its stdout/stderr. '
    ...
    + 'Current harness environment facts are exposed through managed `$env:ORYGIN_*` variables; inspect them when needed. '
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: PwshToolArgs, exec) {
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @orygin-ai/dsh-jobs and @orygin-ai/dsh-tool-jobs')
        }
