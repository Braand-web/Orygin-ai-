 ...
 * trusted, per-execution `ORYGIN_*` variables consumed by the model-facing shell
 ...
 *
 * @module @orygin-ai/dsh-shell-env
 */

import { Service, type Context } from '@orygin-ai/cordis'
import z from '@orygin-ai/schemastery'
import { ORYGIN_ENV_PREFIX } from '@orygin-ai/dsh-shell'
import type { DshEnvironment, DshEnvironmentKey } from '@orygin-ai/dsh-shell'
import { ORYGIN_HOME_ENV, resolveDshHome } from '@orygin-ai/dsh-home-paths'
import type { ToolExecution } from '@orygin-ai/dsh-tools'
import type {} from '@orygin-ai/dsh-session-persistence'
...
/** Model-visible metadata for one managed `ORYGIN_*` environment variable. */
...
export interface BashEnvContributor {
  ...
  name: string
  /** Complete set of `ORYGIN_*` keys this contributor may return. */
...
export interface BashEnvVariableInfo extends BashEnvVariable {
  ...
  contributor: string
  /** Declared `ORYGIN_*` environment variable name. */
  key: DshEnvironmentKey
}

const ORYGIN_SHELL_KEY = `${ORYGIN_ENV_PREFIX}SHELL` as const
const ORYGIN_SESSION_ID_KEY = `${ORYGIN_ENV_PREFIX}SESSION_ID` as const
const ORYGIN_SESSION_JSONL_KEY = `${ORYGIN_ENV_PREFIX}SESSION_JSONL` as const
const RESERVED_BASH_ENV_KEYS = new Set<DshEnvironmentKey>([
  ORYGIN_HOME_ENV,
  ORYGIN_SHELL_KEY,
  ORYGIN_SESSION_ID_KEY,
])
const BASH_ENV_KEY_SUFFIX = /^[A-Z][A-Z0-9_]*$/

/**
 * Registry (`ctx.shellEnv`) for trusted, per-execution `ORYGIN_*` variables.
 * The namespace is rebuilt for every model shell call: ambient `ORYGIN_*` values
...
export class ShellEnvRegistry extends Service {
  ...
  register(contributor: BashEnvContributor): () => void {
    const dispose = this.ctx.effect(function* (this: ShellEnvRegistry) {
      ...
      for (const [key, variable] of variables) {
        if (!key.startsWith(ORYGIN_ENV_PREFIX)
          || !BASH_ENV_KEY_SUFFIX.test(key.slice(ORYGIN_ENV_PREFIX.length))) {
  ...
  /**
   * Build the trusted `ORYGIN_*` snapshot for one shell tool execution.
   * @param execution - the current tool execution.
   * @returns an immutable environment overlay containing built-ins and current contributions.
   */
  collect(execution: ToolExecution): DshEnvironment {
    const values: Record<DshEnvironmentKey, string> = {
      [ORYGIN_HOME_ENV]: this.dshHome,
      [ORYGIN_SHELL_KEY]: '1',
    }
    if (execution.agent !== undefined) {
      values[ORYGIN_SESSION_ID_KEY] = execution.agent.session.header.id
    }
 ...
 * shell-agnostic persistence contributor (`ORYGIN_SESSION_JSONL`).
 * @param ctx - Cordis context that owns the service and registrations.
 * @param config - home-directory configuration for the built-in variables.
 */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  registry.register({
    ...
    resolve(execution) {
      ...
      return location?.kind === 'jsonl' ? { [ORYGIN_SESSION_JSONL_KEY]: location.path } : {}
    },
  })
}
