...
import { ShellExecutor } from '@orygin-ai/dsh-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult } from '@orygin-ai/dsh-shell'
import SystemPrompt from '@orygin-ai/dsh-system-prompt'
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@orygin-ai/dsh-tools'
...
import ApprovalService from '@orygin-ai/dsh-user-approval'
import type { ApprovalOutcome } from '@orygin-ai/dsh-user-approval'
import { LocalBashExecutor } from '@orygin-ai/dsh-bash-local'
import LocalSubprocessRuntime from '@orygin-ai/dsh-subprocess-local'
import SandboxPolicyService from '@orygin-ai/dsh-sandbox-policy'
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @orygin-ai/dsh-jobs and @orygin-ai/dsh-tool-jobs')
  })
...
describe('the model-facing bash tool builds its request from named args only (no {...args} forward)', () => {
  ...
  it('describes the managed harness environment namespace to the model', async () => {
    ...
    expect(description).toContain('$ORYGIN_*')
    expect(description).not.toContain('ORYGIN_SESSION_JSONL')
  })
  ...
  it('injects the session id and JSONL target path into a foreground request', async () => {
    ...
    expect(bash.requests[0]?.dshEnv).toEqual({
      ORYGIN_HOME: recordingDshHome,
      ORYGIN_SESSION_ID: 'request-fg',
      ORYGIN_SESSION_JSONL: path,
      ORYGIN_SHELL: '1',
    })
  })
  ...
  it('injects the same trusted variables into a background request without forwarding model env', async () => {
    ...
    await ctx.tools.execute({
      ...
      arguments: {
        ...
        env: { ORYGIN_SESSION_ID: 'spoofed', ORYGIN_SESSION_JSONL: '/tmp/spoofed' },
      },
      agent,
    })
  ...
  it('injects built-ins and the stable session id when no JSONL locator is available', async () => {
    ...
    const ambient = process.env.ORYGIN_SESSION_ID
    ...
    expect(bash.requests[0]?.dshEnv).toEqual({
      ORYGIN_HOME: recordingDshHome,
      ORYGIN_SESSION_ID: 'request-id-only',
      ORYGIN_SHELL: '1',
    })
    expect(process.env.ORYGIN_SESSION_ID).toBe(ambient)
  })
  ...
  it('keeps parent and child agent session environments isolated', async () => {
    ...
    expect(bash.requests.map(request => request.dshEnv)).toEqual([
      {
        ORYGIN_HOME: recordingDshHome,
        ORYGIN_SESSION_ID: 'request-parent',
        ORYGIN_SESSION_JSONL: ctx.sessionPersistence.locate(parent.session.header)?.path,
        ORYGIN_SHELL: '1',
      },
      {
        ORYGIN_HOME: recordingDshHome,
        ORYGIN_SESSION_ID: 'request-child',
        ORYGIN_SESSION_JSONL: ctx.sessionPersistence.locate(child.session.header)?.path,
        ORYGIN_SHELL: '1',
      },
    ])
    expect(bash.requests[0]?.dshEnv?.ORYGIN_SESSION_JSONL).not.toBe(bash.requests[1]?.dshEnv?.ORYGIN_SESSION_JSONL)
  })
