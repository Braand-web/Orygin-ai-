/**
 * Real-process tests for `@orygin-ai/dsh-pwsh-local`: the LOCAL subprocess
...
import { PwshLocalExecutor, ENCODING_PREAMBLE, candidatePwshPaths, resolvePwshPath } from '@orygin-ai/dsh-pwsh-local'
import LocalSubprocessRuntime from '@orygin-ai/dsh-subprocess-local'
import SubprocessRuntime from '@orygin-ai/dsh-subprocess'
import type { SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@orygin-ai/dsh-subprocess'
import { MAX_TIMER_DELAY_MS } from '@orygin-ai/dsh-timeout'
import type { ShellProcess } from '@orygin-ai/dsh-shell'
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.run', () => {
  ...
  it('resolve() carries stdin/env/dshEnv onto the spec, and run() threads them to the command', async () => {
    ...
    const spec = bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:SEAM_VAR][$env:ORYGIN_SEAM_VAR]"',
      stdin: 'piped\n',
      env: { SEAM_VAR: 'env-ok' },
      dshEnv: { ORYGIN_SEAM_VAR: 'dsh-ok' },
    })
    ...
    expect(spec.dshEnv).toEqual({ ORYGIN_SEAM_VAR: 'dsh-ok' })
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.start (background process handles)', () => {
  ...
  it('threads stdin and extra env into a background process', async () => {
    ...
    const proc = bash.start(bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:BG_VAR][$env:ORYGIN_BG_VAR]"',
      stdin: 'bg-stdin\n',
      env: { BG_VAR: 'bg-env' },
      dshEnv: { ORYGIN_BG_VAR: 'bg-dsh-env' },
    }))
