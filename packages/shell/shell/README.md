...
| Package | Role |
|---|---|
| `@orygin-ai/dsh-shell` (this) | Service Definition: abstract service + vocabulary types |
| `@orygin-ai/dsh-bash-local` | Service Provider: local subprocesses |
| `@orygin-ai/dsh-bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@orygin-ai/dsh-tool-bash` | the model-facing tool schemas over `ctx.shell` |
