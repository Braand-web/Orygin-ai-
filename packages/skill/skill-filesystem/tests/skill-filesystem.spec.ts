...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@orygin-ai/dsh-fs'
...
describe('FileSystemSkillProvider', () => {
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousDshHome = process.env.ORYGIN_HOME
    const previousAgentsHome = process.env.ORYGIN_AGENTS_HOME
    const previousBundledSkillDir = process.env.ORYGIN_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.ORYGIN_HOME = join(envHome, '.dsh')
      process.env.ORYGIN_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.ORYGIN_BUNDLED_SKILL_DIR = bundled
      ...
      process.env.ORYGIN_HOME = join(envHome, 'empty-dsh')
      delete process.env.ORYGIN_BUNDLED_SKILL_DIR
      process.env.ORYGIN_AGENTS_HOME = join(envHome, 'empty-agents')
      ...
      if (previousDshHome === undefined) {
        ...
        process.env.ORYGIN_HOME = previousDshHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.ORYGIN_AGENTS_HOME
      } else {
        process.env.ORYGIN_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.ORYGIN_BUNDLED_SKILL_DIR
      } else {
        process.env.ORYGIN_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
