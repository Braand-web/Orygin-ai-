 ...
 *
 * @module @orygin-ai/dsh-skill-filesystem
 */
...
import type Schema from '@orygin-ai/schemastery'
import { parse as parseYaml } from 'yaml'
import type { FileSystem, FsDirEntry, FsTarget } from '@orygin-ai/dsh-fs'
import { canonicalizeWatchPath, resolveDshHome } from '@orygin-ai/dsh-home-paths'
import {
  BUNDLED_SKILL_RANK,
  isSkillName,
  type SkillCandidate,
  ...
  type SkillSource,
} from '@orygin-ai/dsh-skill'

const PROJECT_ORYGIN_RANK = 100
const PROJECT_AGENTS_RANK = 200
const CUSTOM_RANK = 300
const USER_ORYGIN_RANK = 400
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  constructor(
    ...
    this.agentsHome = resolve(config.agentsHome ?? process.env.ORYGIN_AGENTS_HOME ?? join(homedir(), '.agents'))
    ...
    const bundledSkillDir = config.bundledSkillDir
      ?? (this.includeDefaultRoots ? process.env.ORYGIN_BUNDLED_SKILL_DIR : undefined)
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.dsh/skills'), source: 'project-dsh', rank: PROJECT_ORYGIN_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.dshHome, 'skills'), source: 'user-dsh', rank: USER_ORYGIN_RANK, skipSystem: true },
