 ...
 * process-local registry lives in `@orygin-ai/dsh-jobs-local`.
 * @module @orygin-ai/dsh-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@orygin-ai/dsh-jobs is the abstract job registry seam; load an implementation such as @orygin-ai/dsh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
