 ...
 * @module @orygin-ai/dsh-jobs-local
 */

import { Context } from '@orygin-ai/cordis'
import z from '@orygin-ai/schemastery'
import type { Agent } from '@orygin-ai/dsh-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@orygin-ai/dsh-scope'
import type { ScopeLayer } from '@orygin-ai/dsh-scope'
import { deadline, timeoutOf } from '@orygin-ai/dsh-timeout'
import { JobRegistry, JobId } from '@orygin-ai/dsh-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@orygin-ai/dsh-jobs'
 ...
 * `@orygin-ai/dsh-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @orygin-ai/dsh-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @orygin-ai/dsh-agent)')
    }
