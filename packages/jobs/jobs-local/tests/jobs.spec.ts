...
import { bindScopeParent, createScope, scopeOf } from '@orygin-ai/dsh-scope'
import type { ScopeKey } from '@orygin-ai/dsh-scope'
import { JobId } from '@orygin-ai/dsh-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@orygin-ai/dsh-jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @orygin-ai/dsh-tool-jobs in its composition)')
  })
