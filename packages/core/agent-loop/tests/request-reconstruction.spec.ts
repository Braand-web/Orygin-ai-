...
import LlmRuntime, { createUserMessage, LlmError, ReasoningEffortId  } from '@orygin-ai/dsh-llm'
import type { GenerateOptions, LlmModelReasoningInfo, LlmResolvedModelInfo, StreamChunk } from '@orygin-ai/dsh-llm'
import SessionStore, { Session, SessionId, foldRequestHeader } from '@orygin-ai/dsh-session'
...
describe('request stability across the loop', () => {
  ...
  it('rematerializes the selected adapter maxTokens default after a provider switch', async () => {
    const orygin = new MockAdapter([textResponse('orygin')], undefined, 256_000)
    ...
    expect(orygin.requests[0]?.maxTokens).toBe(256_000)
  ...
  it('preserves an explicit agent maxTokens cap across a provider switch', async () => {
    ...
    expect(orygin.requests[0]?.maxTokens).toBe(4_096)
