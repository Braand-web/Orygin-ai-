...
import { createUserMessage, CallId  } from '@orygin-ai/dsh-llm'
import { createScope } from '@orygin-ai/dsh-scope'
import type { Scope } from '@orygin-ai/dsh-scope'
import SystemPrompt from '@orygin-ai/dsh-system-prompt'
import { CodeRuntime } from '@orygin-ai/dsh-code-runtime'
import type { CodeRunRequest, CodeRunResult } from '@orygin-ai/dsh-code-runtime'
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@orygin-ai/dsh-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@orygin-ai/dsh-tools'
import type { Agent } from '@orygin-ai/dsh-agent'
import { Session, SessionId } from '@orygin-ai/dsh-session'
import type { JsonValue, SessionEventMap } from '@orygin-ai/dsh-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@orygin-ai/dsh-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
