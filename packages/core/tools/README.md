...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@orygin-ai/dsh-tool-call-timeout-policy` wrapper.
