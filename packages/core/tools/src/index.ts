 ...
 * execution pipeline.
 * @module @orygin-ai/dsh-tools
 */

import { Context, Service } from '@orygin-ai/cordis'
import z from '@orygin-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@orygin-ai/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@orygin-ai/dsh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@orygin-ai/dsh-llm'
import { assertNever, deepFreeze, HarnessError } from '@orygin-ai/dsh-llm'
import type { Agent } from '@orygin-ai/dsh-agent'
import { snapshotJsonValue } from '@orygin-ai/dsh-session'
import type { JsonValue, UserMessage } from '@orygin-ai/dsh-session'
import type { ToolProviderResult } from '@orygin-ai/dsh-system-prompt'
import type { CodeRuntime } from '@orygin-ai/dsh-code-runtime'
...
export type { JsonValue } from '@orygin-ai/dsh-session'
...
// lives in its own UI-facing module; re-export it so `@orygin-ai/dsh-tools`
...
declare module '@orygin-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@orygin-ai/dsh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@orygin-ai/dsh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@orygin-ai/dsh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@orygin-ai/dsh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@orygin-ai/dsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @orygin-ai/dsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
