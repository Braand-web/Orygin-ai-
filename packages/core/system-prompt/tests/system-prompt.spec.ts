...
import SystemPrompt, { AssembleContext, PromptAssembly, renderContextSnapshot, renderPrompt } from '@orygin-ai/dsh-system-prompt'
...
const IDENTITY = 'You are an AI agent powered by Orygin.'
...
describe('SystemPrompt', () => {
  describe('built-in sections', () => {
    it('registers the harness identity and the configured deployment persona', async () => {
      ...
      await ctx.plugin(SystemPrompt, { persona: 'You are Orygin.' })
  ...
  it('assembles sections in order with context-resolved text and collected tools', async () => {
    ...
    expect(assembly.sections.map(s => s.text)).toEqual([IDENTITY, 'You are Orygin.', 'Be precise.', 'cwd: /tmp'])
    expect(assembly.contexts).toEqual([
  ...
  describe('prompt variables', () => {
    ...
    it('interpolates {{name}} references in section text at render — the persona included', async () => {
      ...
      ctx.systemPrompt.variable('model', () => 'orygin-v4')
      ctx.systemPrompt.variable('cwd', () => '/work')

      expect(renderPrompt(await ctx.systemPrompt.assemble())).toBe(`${IDENTITY}\n\nYou run on orygin-v4 in /work.`)
    })
