 ...
 *
 * @module @orygin-ai/dsh-system-prompt
 */

import { Context, Service } from '@orygin-ai/cordis'
import z from '@orygin-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@orygin-ai/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@orygin-ai/dsh-scope'
import type { ContextSnapshotSection, ToolSchema } from '@orygin-ai/dsh-llm'
...
declare module '@orygin-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@orygin-ai/dsh-scope`): scoped listeners
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: 'You are an AI agent powered by Orygin.',
      })
    }
    this.section({
