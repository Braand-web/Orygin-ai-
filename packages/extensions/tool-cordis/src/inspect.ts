 ...
 * `name` adds.
 * @module @orygin-ai/dsh-tool-cordis/inspect
 */

import type { Context, Fiber } from '@orygin-ai/cordis'
...
import type {} from '@orygin-ai/dsh-cordis-host-runner'
...
export function describeDynamic(ctx: Context, agent?: Agent): string[] {
  ...
  if (rows.length === 0) {
    return ['No dynamic Plugins are defined in this session. Definitions live only in this process\'s memory, so a ORYGIN restart clears them.']
  }
