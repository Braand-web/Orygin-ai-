...
import type { SubagentCapabilities, SubagentProvider, SubagentResult, SubagentRun, SubagentStartRequest } from '@orygin-ai/dsh-subagent'
import type { WorkflowMeta, WorkflowResult, WorkflowResultInfo, WorkflowRun, WorkflowRunInfo } from '@orygin-ai/dsh-workflow'
...
describe('dsh-workflow-worker-thread', () => {
  describe('script execution over a real worker thread', () => {
    ...
    it('agent({schema, model}) forwards outputSchema and agentOptions to the provider across the thread', async () => {
      ...
      const result = await run(ctx, parent, scripted(`
        const found = await agent('list files', { model: 'orygin-v4-pro', schema: { type: 'object', properties: { files: { type: 'array', items: { type: 'string' } } }, required: ['files'] } })
      ...
      expect(provider.runs[0]!.request.agentOptions).toEqual({ model: 'orygin-v4-pro' })
    ...
    it('the worker spawns with a scrubbed environment: an escaped script finds no ambient credentials', async () => {
      ...
      // the escape below would read it back (exactly how ORYGIN_API_KEY
