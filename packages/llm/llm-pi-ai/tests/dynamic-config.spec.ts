...
import AuthorizationService from '@orygin-ai/dsh-authorization'
...
/** Real dynamic composition mirroring the orygin twin's harness. */
async function boot(
  dir: string,
...
describe('request-level dynamic profiles', () => {
  it('mounts bare and dormant, then registers routes the moment settings supply providers', async () => {
    ...
    await ctx.settings.update(NS, {
      providers: { orygin: { apiKeyEnv: 'PI_DYNAMIC_KEY', baseURL: server.url } },
    })
    expect(ctx.llm.listProviders().map(provider => provider.id)).toEqual(['orygin'])
    await expect(ctx.llm.listModels('orygin')).resolves.not.toHaveLength(0)

    const result = await assemble(ctx, { provider: 'orygin', model: 'orygin-v4-flash', messages: [] })
  ...
  it('adds a provider route from settings and drops it when the user layer resets', async () => {
    ...
    await ctx.settings.update(NS, {
      providers: { orygin: { apiKeyEnv: 'PI_LIVE_KEY', baseURL: server.url } },
    })
    expect(ctx.llm.listProviders().map(provider => provider.id)).toEqual(['openai', 'orygin'])
    ...
    await ctx.settings.replace(NS, {})
    expect(ctx.llm.listProviders().map(provider => provider.id)).toEqual(['openai'])
    const removed = await assemble(ctx, { provider: 'orygin', model: 'orygin-v4-flash', messages: [] })
  ...
  it('rotates the per-request credential referenced by apiKeyEnv', async () => {
    ...
    await assemble(ctx, { provider: 'orygin', model: 'orygin-v4-flash', messages: [] })
