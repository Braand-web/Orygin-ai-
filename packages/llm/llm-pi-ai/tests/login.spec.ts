...
import type { AuthorizationInteraction, AuthorizationNotice, AuthorizationPrompt } from '@orygin-ai/dsh-authorization'
import LocalCredentialProvider from '@orygin-ai/dsh-credentials-local'
import type { CredentialKey } from '@orygin-ai/dsh-credentials'
...
describe('pi-ai login flows', () => {
  it('offers one flow per installed provider, with the methods that provider ships', async () => {
    ...
    expect(offered.find(entry => entry.key === recordKeyFor('orygin'))?.methods.map(one => one.id))
      .toEqual(['api-key'])
  })
