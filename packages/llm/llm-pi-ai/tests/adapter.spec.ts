...
import { AttachmentId, AttachmentStore, ImageVariantId } from '@orygin-ai/dsh-attachment'
import type {
...
import LlmRuntime, { createUserMessage, CONTEXT_WINDOW_EXCEEDED_CODE, LlmError, ReasoningEffortId, userAgent } from '@orygin-ai/dsh-llm'
...
async function harness(baseURL: string, overrides: Record<string, unknown> = {}): Promise<Context> {
  ...
  await ctx.plugin(LlmPiAi, {
    providers: { orygin: { apiKeyEnv: 'PI_TEST_KEY', baseURL, ...overrides } },
  })
  return ctx
}
...
describe('PiAiAdapter provider routing', () => {
  ...
  it('keeps prepared model metadata and dispatch on one profile snapshot', async () => {
    ...
    let providers: Record<string, LlmPiAi.PiAiProviderProfile> = {
      orygin: { apiKeyEnv: 'PI_TEST_KEY', baseURL: first.url },
    }
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    ctx.llm.registerAdapter(['orygin'], new PiAiAdapter({
      profiles: () => resolveProfiles(providers),
      resolveApiKey: () => Promise.resolve('test-key'),
      auth: memoryAuth(),
    }))

    const prepared = await ctx.llm.prepareCall({ provider: 'orygin', model: 'orygin-v4-flash' })
    providers = { orygin: { apiKeyEnv: 'PI_TEST_KEY', baseURL: second.url } }
  ...
  it('preserves omitted profile options when constructing the adapter directly', async () => {
    ...
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    ctx.llm.registerAdapter(['orygin'], adapterOf({
      orygin: { apiKeyEnv: 'PI_TEST_KEY', baseURL: server.url },
    }))
  ...
  it('reports unsupported stop sequences rather than silently ignoring them', async () => {
    ...
    const ctx = await harness(server.url)
    const result = await assemble(ctx, { model: 'orygin-v4-flash', messages: [], stop: ['END'] })
  ...
  it('uses the resolved catalog context window for usage-based overflow detection', async () => {
    const model = getBuiltinModels('orygin').find(candidate => candidate.id === 'orygin-v4-flash')
...
describe('provider profile lifecycle', () => {
  ...
  it('exposes pi-ai model thinking levels verbatim without inventing a provider default', async () => {
    ...
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmPiAi, {
      providers: { orygin: {}, openai: {} },
    })

    await expect(ctx.llm.resolveModelInfo('orygin', 'orygin-v4-flash'))
  ...
  it('uses a supported profile reasoning value as the model default and rejects an unsupported one', async () => {
    ...
    await supported.plugin(LlmPiAi, {
      providers: { orygin: { reasoning: 'max' } },
    })
    await expect(supported.llm.resolveModelInfo('orygin', 'orygin-v4-flash'))
    ...
    await unsupported.plugin(LlmPiAi, {
      providers: { orygin: { reasoning: 'medium' } },
    })
    const described = await unsupported.llm.resolveModelInfo('orygin', 'orygin-v4-flash')
    ...
    await disabled.plugin(LlmPiAi, {
      providers: { orygin: { reasoning: 'off' } },
    })
    await expect(disabled.llm.resolveModelInfo('orygin', 'orygin-v4-flash'))
  ...
  it('fails a named-but-missing apiKeyEnv instead of using another ambient key', async () => {
    ...
    expect(second.finish.failure.message).toMatch(/provider route "orygin".*PI_CUSTOM_REF_KEY/s)
  ...
  it('rejects unsupported or unresolved image input before provider I/O', async () => {
    const adapter = adapterOf({ openai: {}, orygin: {} })
...
describe('abort wiring', () => {
  it('preserves an unknown pre-dispatch adapter Error exactly', async () => {
    ...
    const adapter = adapterOf({ orygin: {} })
  ...
  it('honors a pre-aborted caller signal', async () => {
    ...
    const controller = new AbortController()
    controller.abort('already stopped')
    const result = await assemble(ctx, { model: 'orygin-v4-flash', messages: [], signal: controller.signal })
  ...
  it('forwards an abort that arrives while provider streaming is active', async () => {
    ...
    const resultPromise = assemble(ctx, {
      model: 'orygin-v4-flash', messages: [], signal: controller.signal,
    })
  ...
  it('aborts upstream when a consumer stops early', async () => {
    const server = await mockServer([{ events: textEvents, delayMs: 30 }])
    const ctx = await harness(server.url)
    for await (const chunk of ctx.llm.stream({ provider: 'orygin', model: 'orygin-v4-flash', messages: [] })) {
