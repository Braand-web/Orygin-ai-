...
import type { AttachmentStore, ImageAttachmentRef, ImageRequestPolicy, RequestImageAttachment } from '@orygin-ai/dsh-attachment'
import { createUserMessage, CallId, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, createMessage } from '@orygin-ai/dsh-llm'
import type { ContentBlock, StreamChunk } from '@orygin-ai/dsh-llm'
...
describe('toPiContext', () => {
  ...
  it('omits empty tools and absent system prompt', () => {
    const context = toPiContext({ provider: 'orygin', model: 'm', messages: [], tools: [] })
  ...
  it('preserves provider and model for foreign assistant messages without replay state', () => {
    const context = toPiContext({
      ...
      messages: [createMessage({
        ...
        source: { kind: 'model', provider: 'orygin', model: 'old-model' },
      })],
    })
  ...
  it('replays all native block kinds when optional metadata is absent', () => {
    ...
    const context = toPiContext({
      ...
      messages: [createMessage({
        ...
        source: {
          kind: 'model',
          ...{ provider: 'orygin', model: 'orygin-v4-flash', replayState: state },
        },
      })],
    })
  ...
  function expectDegraded(replayState: unknown, message: string): void {
    ...
    const context = toPiContext({
      ...
      messages: [createMessage({
        ...
        source: {
          kind: 'model',
          ...{ provider: 'orygin', model: 'orygin-v4-flash', replayState },
        },
      })],
    }, undefined, onDegrade)
    expect(context.messages[0]).toMatchObject({
  ...
  it.each([
    ...
    ['model', { ...validReplay, response: { ...validResponse, model: 'orygin-v4-pro' } }],
...
describe('mapStopReason / mapUsage', () => {
  ...
  it('classifies a completed stop with no content as an EMPTY_RESPONSE error', () => {
    expect(mapStopReason(assistant({ stopReason: 'stop' }))).toEqual({
      kind: 'error',
      failure: {
        message: 'model "orygin-v4-flash" returned a completed response with no content',
  ...
  it('uses the resolved context window for silent and length-stop overflows', () => {
    ...
    expect(mapStopReason(silent, 100)).toEqual({
      kind: 'error',
      failure: {
        message: 'pi-ai detected context overflow for model "orygin-v4-flash"',
