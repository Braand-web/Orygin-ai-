...
import LlmRuntime, { LlmAdapter, LlmError } from '@orygin-ai/dsh-llm'
import type { GenerateOptions, LlmConfigurableProvider, StreamChunk } from '@orygin-ai/dsh-llm'
...
describe('llm/adapters-updated', () => {
  ...
  it('replaces a route set in one event, never publishing an empty registry between the two', async () => {
    // The retry-policy swap in llm-orygin: disposing and re-registering
...
describe('configurable-provider directory', () => {
  it('registers entries, lists detached copies in order, and fires the topology event', async () => {
    ...
    ctx.llm.registerConfigurableProviders([
      entry({ provider: 'orygin-official', displayName: 'Orygin', settingsNs: 'llm-orygin', settingsPath: [] }),
      entry(),
    ])
    ...
    expect(ctx.llm.listConfigurableProviders()[0]!.displayName).toBe('Orygin')
