...
describe('OryginUploadIndex', () => {
  it('normalizes trailing endpoint slashes in the credential scope', () => {
    expect(oryginFileScope('https://api.orygin.fun///', 'key'))
      .toBe(oryginFileScope('https://api.orygin.fun', 'key'))
  })
  ...
  it('isolates API-key namespaces and reuses only records above the refresh margin', async () => {
    ...
    const first = oryginFileScope('https://api.orygin.fun', 'first-key')
    const second = oryginFileScope('https://api.orygin.fun', 'second-key')
    const record = {
      scope: first,
      attachmentId: ATTACHMENT,
      variantId: VARIANT,
      fileId: OryginFileId('file-api-one'),
      bytes: 3,
      createdAt: 1_000,
      expiresAt: 10_000,
    }
  ...
  it('keeps a reusable cross-process winner and removes only an exact generation', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'dsh-upload-index-'))
    const index = new OryginUploadIndex(join(dir, 'index.json'))
    const scope = oryginFileScope('https://api.orygin.fun', 'key')
    const first = {
      scope, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: OryginFileId('file-api-first'), bytes: 3, createdAt: 1, expiresAt: 10_000,
    }
    const duplicate = { ...first, fileId: OryginFileId('file-api-duplicate') }
  ...
  it('treats a corrupt upload cache as empty and repairs it on the next commit', async () => {
    ...
    const index = new OryginUploadIndex(path)
    const scope = oryginFileScope('https://api.orygin.fun', 'key')
    const record = {
      scope,
      attachmentId: ATTACHMENT,
      variantId: VARIANT,
      fileId: OryginFileId('file-api-repaired'),
      bytes: 3,
      createdAt: 1,
      expiresAt: 10_000,
    }
  ...
  it.each([
    ...
    await expect(index.get(
      oryginFileScope('https://api.orygin.fun', 'key'), VARIANT, 1, 1,
  ...
  it('rejects duplicate persisted mappings as a corrupt cache', async () => {
    ...
    const record = {
      scope, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: OryginFileId('file-api-one'), bytes: 3, createdAt: 1, expiresAt: 10_000,
    }
  ...
  it('drops expired records on commit and clears only the selected namespace', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'dsh-upload-index-'))
    const index = new OryginUploadIndex(join(dir, 'index.json'))
    const first = oryginFileScope('https://api.orygin.fun', 'first')
    const second = oryginFileScope('https://api.orygin.fun', 'second')
    const expired = {
      scope: first, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: OryginFileId('file-api-expired'), bytes: 3, createdAt: 1, expiresAt: 2,
    }
    const live = {
      ...expired, scope: second, fileId: OryginFileId('file-api-live'), expiresAt: 10_000,
    }
