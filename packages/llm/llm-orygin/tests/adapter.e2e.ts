...
import LlmRuntime, { createUserMessage, CallId, ReasoningEffortId, createMessage } from '@orygin-ai/dsh-llm'
...
import type { Config } from '@orygin-ai/dsh-llm-orygin'
 ...
 * without $ORYGIN_API_KEY; the pre-release vision smoke additionally
 * requires $ORYGIN_VISION_E2E=1 (see vitest.e2e.config.ts).
 */

const FLASH = 'orygin-v4-flash'
const PRO = 'orygin-v4-pro'
const VISION = 'orygin-v4-flash-vision-exp'
const VISION_E2E_ENABLED = process.env.ORYGIN_VISION_E2E === '1'
...
beforeEach(async () => {
  ...
  vi.stubEnv('ORYGIN_HOME', identityHome)
})
...
describe.skipIf(!process.env.ORYGIN_API_KEY)('llm-orygin e2e (real API)', () => {
  it.skipIf(!VISION_E2E_ENABLED)('uses the built-in official route to upload, reference, and delete one image', async () => {
    const key = process.env.ORYGIN_API_KEY
    if (key === undefined) throw new Error('e2e ran without ORYGIN_API_KEY')
    const baseURL = process.env.ORYGIN_BASE_URL ?? LlmOrygin.PUBLIC_BASE_URL
    const ctx = await harness(VISION, { baseURL })
    await ctx.plugin(E2eAttachmentStore)
    const attachments = ctx.attachments as E2eAttachmentStore
    let uploadedFile: LlmOrygin.OryginFileIdType | undefined
    ...
    const observedFetch: typeof fetch = async (input, init) => {
      ...
      if (method === 'POST' && url.pathname.endsWith('/files') && response.ok) {
        ...
        if (typeof value.id === 'string') uploadedFile = LlmOrygin.OryginFileId(value.id)
      }
      return response
    }
    vi.stubGlobal('fetch', observedFetch)
    const files = new LlmOrygin.OryginFilesClient({ baseURL, apiKey: key })
    ...
    try {
      ...
      expect(
        result.finish.kind,
        `Orygin vision result: ${JSON.stringify(result.finish)}`,
      ).toBe('stop')
