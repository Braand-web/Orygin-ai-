...
import { OryginFileStore, MAX_CHAT_IMAGE_BYTES } from '../src/file-store.ts'
import { OryginFileId } from '../src/file-id.ts'
import { oryginFileScope, OryginUploadIndex } from '../src/upload-index.ts'
...
const CONNECTION = { baseURL: 'https://api.orygin.fun', apiKey: 'key' }
...
describe('OryginFileStore', () => {
  it('singleflights the first upload and reuses the durable mapping across store instances', async () => {
    ...
    const index = new OryginUploadIndex(join(dir, 'index.json'))
    const remote = uploadFetch()
    const first = new OryginFileStore({ index, now: () => NOW, fetch: remote.fetchImpl })
    ...
    const resumed = new OryginFileStore({ index, now: () => NOW, fetch: remote.fetchImpl })
  ...
  it('keeps a shared upload alive while another waiter remains', async () => {
    ...
    const store = new OryginFileStore({ index, now: () => NOW, fetch: fetchImpl })
  ...
  it('normalizes a non-Error cancellation reason', async () => {
    ...
    const store = new OryginFileStore({
      index: new OryginUploadIndex(join(dir, 'index.json')),
      now: () => NOW,
      fetch: fetchImpl,
    })
    const controller = new AbortController()
    ...
    await expect(upload).rejects.toMatchObject({
      message: 'Orygin file upload cancelled with a non-Error reason.',
      cause: 'cancelled',
    })
  })
  ...
  it('rejects a request version above the chat per-image limit before transport', async () => {
    ...
    const store = new OryginFileStore({ now: () => NOW, fetch: fetchImpl })
  ...
  it.each([
    ...
    const remote = uploadFetch()
    const store = new OryginFileStore({
      index: new OryginUploadIndex(join(dir, `${extension}.json`)),
      now: () => NOW,
  ...
  it('normalizes a non-Error failure from the durable upload index', async () => {
    ...
    const store = new OryginFileStore({ index, now: () => NOW, fetch: vi.fn() as typeof fetch })

    await expect(store.ensureUploaded(VERSION, CONNECTION, POLICY)).rejects.toMatchObject({
      message: 'Orygin file upload failed with a non-Error reason.',
  ...
  it('reuses local expires_at above the refresh margin and uploads again at the margin', async () => {
    ...
    const store = new OryginFileStore({ index, now: () => now, fetch: remote.fetchImpl })
  ...
  it('removes a losing upload and keeps the winning durable mapping when duplicate cleanup fails', async () => {
    ...
    vi.spyOn(index, 'commit').mockResolvedValue({
      accepted: false,
      record: {
        scope: oryginFileScope(CONNECTION.baseURL, CONNECTION.apiKey),
        attachmentId: VERSION.attachment.attachmentId,
        variantId: VERSION.variantId,
        fileId: OryginFileId('file-api-winner'),
        bytes: 3,
        createdAt: NOW,
  ...
  it('stops pagination when a page omits or repeats its cursor', async () => {
    ...
    for (const mode of ['missing', 'repeated'] as const) {
      ...
      const store = new OryginFileStore({
        index: new OryginUploadIndex(join(dir, `${mode}.json`)),
        now: () => NOW,
        fetch: fetchImpl,
      })
