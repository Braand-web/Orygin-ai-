...
import { BlockAssembler } from '@orygin-ai/dsh-llm'
import type { Context } from '@orygin-ai/cordis'
import type { FinishReason, GenerateOptions, Message, TokenUsage } from '@orygin-ai/dsh-llm'
...
export async function assemble(ctx: Context, options: Omit<GenerateOptions, 'provider'> & { provider?: string }): Promise<AssembledResult> {
  ...
  const request = { provider: 'orygin-official', ...options }
