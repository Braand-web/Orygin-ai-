...
import { userAgent } from '@orygin-ai/dsh-llm'
import { OryginFileId } from '../src/file-id.ts'
import {
  OryginFilesClient,
  OryginFilesError,
  isFilesQuotaError,
...
describe('OryginFilesClient', () => {
  it('uploads multipart bytes with the required purpose and explicit expiry', async () => {
    ...
    const client = new OryginFilesClient({ baseURL: 'https://api.orygin.fun/', apiKey: 'key', fetch: fetchImpl })
  ...
  it('validates list, retrieve, and delete responses', async () => {
    ...
    await expect(client.list({ after: OryginFileId('file-api-before'), limit: 20, order: 'desc' })).resolves.toMatchObject({
      data: [{ id: 'file-api-one' }], firstId: 'file-api-one', lastId: 'file-api-one', hasMore: false,
    })
    await expect(client.retrieve(OryginFileId('file-api-one'))).resolves.toMatchObject({ id: 'file-api-one' })
    await expect(client.delete(OryginFileId('file-api-one'))).resolves.toBeUndefined()
  })
  ...
  it.each([
    ...
    const client = new OryginFilesClient({
      baseURL: 'https://api.orygin.fun',
      apiKey: 'key',
      fetch: vi.fn(() => Promise.resolve(new Response('not-json', { status }))),
    })
    await expect(client.retrieve(OryginFileId('missing'))).rejects.toMatchObject({
      name: 'OryginFilesError',
      code,
      detail: '',
    })
  })
  ...
  it.each([
    ...
    const error = await client.retrieve(OryginFileId('missing')).catch((caught: unknown) => caught)
    expect(error).toBeInstanceOf(OryginFilesError)
    expect(error).toMatchObject({ message: 'Orygin Files API error (HTTP 400)', detail: '' })
  })
  ...
  it('wraps transport failures but preserves an aborted request reason', async () => {
    ...
    await expect(client.retrieve(OryginFileId('one'))).rejects.toMatchObject({
      code: 'TRANSPORT',
      cause: transport,
    })
    ...
    await expect(client.retrieve(OryginFileId('one'), controller.signal)).rejects.toBe(transport)
  })
  ...
  it.each([
    ...
    await expect(client.retrieve(OryginFileId('one'))).rejects.toMatchObject({ code: 'INVALID_RESPONSE' })
  })
  ...
  it('accepts a list without cursors and uses the global fetch default', async () => {
    ...
    try {
      const client = new OryginFilesClient({ baseURL: 'https://api.orygin.fun///', apiKey: 'key' })
  ...
  it.each([
    ...
    await expect(client.delete(OryginFileId('file-api-one'))).rejects.toMatchObject({ code: 'INVALID_RESPONSE' })
  })
})
