 ...
 * settings-file, credentials-local, and llm-orygin boot from a test-only
...
import { getOrCreateAnonymousUserId } from '@orygin-ai/dsh-anonymous-user-id'
...
async function loadComposition(
  ...
  await writeFile(configPath, [
    ...
    ...options.withDynamic
      ? [
        '- id: settings',
        "  name: '@orygin-ai/dsh-settings-file'",
        '  config:',
        `    path: ${JSON.stringify(settingsPath)}`,
        '    debounceMs: 10',
        '- id: credentials',
        "  name: '@orygin-ai/dsh-credentials-local'",
        '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@orygin-ai/dsh-settings-file', FileSettingsProvider],
    ['@orygin-ai/dsh-credentials-local', LocalCredentialProvider],
    ['@orygin-ai/dsh-llm-orygin', LlmOrygin],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('llm-orygin real dynamic composition', () => {
  ...
  it('keeps a stored key writable and rotatable across a real restart', async () => {
    // No ambient ORYGIN_API_KEY: the shipped surfaces do not hoist
    ...
    await assemble(boot.ctx, { model: 'orygin-v4-flash', messages: [] })
    ...
    await assemble(restarted.ctx, { model: 'orygin-v4-flash', messages: [] })
  ...
  it('boots the same adapter on entry config alone, resolving the reference from the environment', async () => {
    ...
    vi.stubEnv('ORYGIN_API_KEY', 'entry-key')
