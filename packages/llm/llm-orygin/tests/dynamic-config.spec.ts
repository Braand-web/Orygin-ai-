...
import LlmRuntime, { createUserMessage, INVALID_CREDENTIAL_CODE } from '@orygin-ai/dsh-llm'
import AttachmentStore, { AttachmentId, ImageVariantId } from '@orygin-ai/dsh-attachment'
import type {
...
const KEY_REF = credentialRef('ORYGIN_API_KEY')
 ...
 * llm-orygin over one temp harness home. `watch: false` keeps every change
...
async function boot(dir: string, config: object): Promise<Harness> {
  ...
  await ctx.plugin(LlmOrygin, config)
  return { ctx, settingsFiber }
}

function prompt(ctx: Context) {
  return assemble(ctx, { model: 'orygin-v4-flash', messages: [] })
}
...
describe('request-level dynamic configuration', () => {
  ...
  it('advertises a live settings catalog without re-registration', async () => {
    ...
    await expect(ctx.llm.listModels('orygin-official')).resolves.toEqual([
      { provider: 'orygin-official', id: 'settings-model', name: 'From Settings', inputModalities: ['text', 'image'] },
    ])
  })
  ...
  it('applies changed request file limits to the next request', async () => {
    ...
    await assemble(ctx, { model: 'orygin-v4-flash-vision-exp', messages })
  ...
  it('re-registers the route in place when the captured retry policy changes, without an empty-registry window', async () => {
    ...
    expect(observed).toEqual([['orygin-official']])
  })
  ...
  it('keeps the last good options when a settings snapshot fails beyond-schema validation', async () => {
    ...
    await expect(ctx.llm.listModels('orygin-official')).resolves.toEqual([
      { provider: 'orygin-official', id: 'recovered', name: 'recovered', inputModalities: ['text'] },
    ])
  })
  ...
  it('keeps the whole last-good snapshot when a rejected one changed the URL', async () => {
    ...
    vi.stubEnv('ORYGIN_API_KEY', 'good-key')
