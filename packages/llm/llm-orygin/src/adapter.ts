/**
 * `OryginAdapter`: fetch + SSE against a Orygin (OpenAI-compatible)
 * chat-completions endpoint, emitting harness StreamChunks. The adapter is
 * transport-only: connection facts arrive through a thunk resolved once per
 * operation and the bearer token through a per-request resolver, so the
 * registering plugin owns validation, layering, and credential policy.
 *
 * @module dsh-llm-orygin/adapter
 */

import { attributionHeaders, contentHasImage, CONTEXT_WINDOW_EXCEEDED_CODE, isContextWindowExceededError, isQuotaExceededError, LlmAdapter, LlmError, offloadRequestImagesWithPolicy, ProviderRequestId, QUOTA_EXCEEDED_CODE, ReasoningEffortId } from '@orygin-ai/dsh-llm'
import type {
  ContentBlock,
  GenerateOptions,
  LlmModelInfo,
...
} from '@orygin-ai/dsh-attachment'
import type { CredentialRef } from '@orygin-ai/dsh-credentials'
import { deadline, idleWatchdog, timeoutOf } from '@orygin-ai/dsh-timeout'
import type { AnonymousUserId } from '@orygin-ai/dsh-anonymous-user-id'
import { serializeRequest, serializeRequestWithImages } from './serialize.ts'
import type { ImageWireLocation, RequestDefaults } from './serialize.ts'
import { OryginFileStore } from './file-store.ts'
import type { OryginFilePolicy } from './file-store.ts'
import type { OryginFileId } from './file-id.ts'
 ...
 */
export interface OryginConnectionOptions {
  ...
  models: readonly OryginCatalogModel[]
  ...
  filePolicy: OryginFilePolicy
  /** Provider-owned model-request retry policy, already resolved. */
  retryPolicy: ResolvedRetryPolicy
}

/** Constructor options for {@link OryginAdapter}: the operation-local resolution hooks the plugin owns. */
export interface OryginAdapterOptions {
  /** Current validated connection facts; called once per operation. */
  options: () => OryginConnectionOptions
  /**
   ...
   */
  resolveApiKey: (connection: OryginConnectionOptions) => Promise<string>
...
/** Total-pixel budget matching Orygin's normal vision projection. */
...
const FILES_API_TIMEOUT_CODE = 'ORYGIN_FILES_API_TIMEOUT'
...
class FileResolutionFailure extends Error {
  constructor(cause: unknown) {
    super('Orygin Files API could not resolve a request image.', { cause })
...
/**
 * Resolve the request-image budgets owned by one Orygin model route.
 * @param model - Advertised model route and its optional image overrides.
 * @returns Complete pixel and encoded-byte budgets.
 * @internal
 */
export function resolveRequestImagePolicy(model: OryginCatalogModel): ImageRequestPolicy {
...
function detailNamesFileId(detail: string, fileId: OryginFileId): boolean {
...
function normalizedImageDiagnostic(
  ...
  if (target !== undefined) {
    return `Orygin rejected normalized image ${normalizedImageFacts(target)}: ${providerMessage}. `
  ...
  return `Orygin rejected a normalized request image: ${providerMessage}. Candidate images: `
    + `${candidates.map(normalizedImageFacts).join('; ')}. `
    + 'The provider rejected bytes already normalized by the harness; PNG, JPEG, WebP, and GIF remain supported input formats.'
}

function modelInfo(provider: string, model: OryginCatalogModel): LlmModelInfo {
  return {
    provider,
    id: model.id,
...
function requestId(headers: Headers): ReturnType<typeof ProviderRequestId> | undefined {
  const value = headers.get('x-request-id') ?? headers.get('x-orygin-request-id')
 ...
 */
export class OryginAdapter extends LlmAdapter {
  private readonly files: OryginFileStore

  constructor(private readonly config: OryginAdapterOptions) {
    super()
    this.files = config.resolveFiles?.() ?? new OryginFileStore()
  }

  override providerInfo(provider: string): LlmProviderInfo {
    return { id: provider, name: 'Orygin' }
  }
  ...
  private modelInfoFor(
    connection: OryginConnectionOptions,
    provider: string,
    model: string,
  ): LlmResolvedModelInfo {
  ...
  private async * streamWithConnection(
    ...
    if (hasImages) {
      ...
      if (model?.inputModalities?.includes('image') !== true) {
        throw new LlmError(
          `Orygin model "${options.model}" does not accept image input.`,
          'UNSUPPORTED_CONTENT',
        )
      }
      ...
      if (attachments === undefined) {
        throw new LlmError(
          'Orygin image conversion requires the durable attachment service.',
          'UNSUPPORTED_CONTENT',
        )
      }
    }
    ...
    try {
      ...
      if (timeoutOf(watchdog.signal, STREAM_IDLE_TIMEOUT_CODE) !== undefined) {
        throw new LlmError(
          `Orygin stream idle timeout after ${connection.streamIdleTimeoutMs}ms`,
          'TIMEOUT',
          { cause: error },
        )
      }
      if (options.signal?.aborted) {
        throw new LlmError('Orygin request aborted by caller', 'ABORTED', { cause: error })
      }
      if (error instanceof LlmError) throw error
      throw new LlmError(`Orygin API stream from ${connection.baseURL} failed`, 'TRANSPORT', { cause: error })
    } finally {
      consumer.abort('Orygin stream consumer stopped')
  ...
  private async * request(
    ...
    const headers = {
      ...
      ...attributionHeaders(),
      'x-orygin-ai-user-id': String(userId),
      ...options.sessionId !== undefined
        ? { 'x-orygin-ai-session-id': String(options.sessionId) }
        : {},
      ...options.purpose === 'compaction'
        ? { 'x-orygin-ai-compact': '1' }
        : {},
    }
    ...
    while (true) {
          ...
          body = await serializeRequestWithImages(requestOptions, {
            representation: {
              ...
              resolveFileId: async (version, _block, location) => {
                ...
                let resolved: Awaited<ReturnType<OryginFileStore['ensureUploaded']>>
                try {
      ...
      try {
        ...
        if (signal.aborted) throw error
        throw new LlmError(
          `Orygin API request to ${connection.baseURL} failed`,
          'TRANSPORT',
          { cause: error },
        )
      }
      ...
      if (!response.ok) {
        let message = `Orygin API error (HTTP ${response.status})`
        ...
        throw new LlmError(message, httpErrorCode(response.status, providerError), {
          cause: new Error(rawResponse.length > 0 ? rawResponse : `Orygin HTTP ${response.status}`),
      ...
      if (!response.body) {
        throw new LlmError('Orygin API returned no response body', 'EMPTY_RESPONSE')
      }
