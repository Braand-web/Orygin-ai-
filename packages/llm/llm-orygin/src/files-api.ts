/** OpenAI-compatible Orygin Files API transport. @module dsh-llm-orygin/files-api */

import { attributionHeaders, LlmError } from '@orygin-ai/dsh-llm'
import type { ImageMediaType } from '@orygin-ai/dsh-attachment'
import { OryginFileId } from './file-id.ts'
import type { OryginFileId as OryginFileIdType } from './file-id.ts'
...
export interface OryginFileObject {
  id: OryginFileIdType
  bytes: number
  createdAt: number
  filename: string
...
export interface OryginFilePage {
  data: OryginFileObject[]
  firstId?: OryginFileIdType
  lastId?: OryginFileIdType
  hasMore: boolean
}

/** Files API operation failure with its HTTP status retained for recovery policy. */
export class OryginFilesError extends LlmError {
 ...
 */
export function isFilesQuotaError(error: unknown): error is OryginFilesError {
  return error instanceof OryginFilesError
...
function invalidResponse(operation: string): LlmError {
  return new LlmError(`Orygin Files API returned an invalid ${operation} response.`, 'INVALID_RESPONSE')
}

function parseFileObject(value: unknown, operation: string): OryginFileObject {
...
export class OryginFilesClient {
  ...
  private async request(path: string, init: RequestInit, signal?: AbortSignal): Promise<Response> {
    ...
    try {
      ...
      throw new LlmError(`Orygin Files API request to ${this.baseURL} failed`, 'TRANSPORT', { cause: error })
    }
    ...
    throw new OryginFilesError(
      message ?? `Orygin Files API error (HTTP ${response.status})`,
      response.status,
      detail,
    )
  }
  ...
  async upload(input: {
    ...
    signal?: AbortSignal
  }): Promise<OryginFileObject & { expiresAt: number }> {
    if (input.data.byteLength > MAX_FILE_UPLOAD_BYTES) {
      throw new LlmError('Orygin Files API upload exceeds 128 MiB.', 'INVALID_REQUEST')
    }
    if (!Number.isSafeInteger(input.expiresAfterSeconds)
      ...
      throw new LlmError('Orygin file expiry must be between 3600 and 2592000 seconds.', 'INVALID_REQUEST')
    }
   ...
   */
  async list(options: {
    after?: OryginFileIdType
    limit?: number
    order?: 'asc' | 'desc'
    signal?: AbortSignal
  } = {}): Promise<OryginFilePage> {
    ...
    return {
      ...
      ...typeof wire.first_id === 'string' ? { firstId: OryginFileId(wire.first_id) } : {},
      ...typeof wire.last_id === 'string' ? { lastId: OryginFileId(wire.last_id) } : {},
   ...
   */
  async retrieve(fileId: OryginFileIdType, signal?: AbortSignal): Promise<OryginFileObject> {
   ...
   * @param signal - request cancellation.
   */
  async delete(fileId: OryginFileIdType, signal?: AbortSignal): Promise<void> {
