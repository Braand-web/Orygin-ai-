/** Orygin Files API upload reuse, invalidation, and quota recovery. @module dsh-llm-orygin/file-store */

import type { RequestImageAttachment } from '@orygin-ai/dsh-attachment'
import { LlmError } from '@orygin-ai/dsh-llm'
import { OryginFilesClient, isFilesQuotaError } from './files-api.ts'
import type { OryginFileId } from './file-id.ts'
import { oryginFileScope, OryginUploadIndex } from './upload-index.ts'
import type { OryginUploadRecord } from './upload-index.ts'

/** Orygin chat accepts at most 32 MiB per image even when it is referenced by file id. */
...
export interface OryginFilePolicy {
...
export interface OryginFileConnection {
  baseURL: string
  apiKey: string
}

/** Result of one file-id resolution. */
export interface OryginFileReference {
  record: OryginUploadRecord
  uploaded: boolean
}

interface FileStoreOptions {
  index?: OryginUploadIndex
  now?: () => number
  fetch?: typeof fetch
}
...
interface SharedUpload {
  ...
  promise: Promise<OryginFileReference>
  settled: boolean
  waiters: number
}
...
function abortReason(signal: AbortSignal): Error {
  ...
  return reason instanceof Error
    ? reason
    : new Error('Orygin file upload cancelled with a non-Error reason.', { cause: reason })
}

function uploadFailure(error: unknown): Error {
  return error instanceof Error
    ? error
    : new Error('Orygin file upload failed with a non-Error reason.', { cause: error })
}

function waitForUpload(operation: SharedUpload, signal: AbortSignal | undefined): Promise<OryginFileReference> {
  ...
  return new Promise<OryginFileReference>((resolve, reject) => {
    const abort = (): void => {
...
/** User-scoped durable file-id reuse for the Orygin route. */
export class OryginFileStore {
  private readonly index: OryginUploadIndex
  ...
  constructor(options: FileStoreOptions = {}) {
    this.index = options.index ?? new OryginUploadIndex()
    this.now = options.now ?? Date.now
    this.fetchImpl = options.fetch
  }

  private client(connection: OryginFileConnection): OryginFilesClient {
    return new OryginFilesClient({
  ...
  ensureUploaded(
    ...
    connection: OryginFileConnection,
    policy: OryginFilePolicy,
    signal?: AbortSignal,
  ): Promise<OryginFileReference> {
    signal?.throwIfAborted()
    const scope = oryginFileScope(connection.baseURL, connection.apiKey)
  ...
  private async ensureUploadedOnce(
    ...
    if (version.bytes > MAX_CHAT_IMAGE_BYTES) {
      throw new LlmError('Orygin chat image exceeds the 32 MiB per-image limit.', 'INVALID_REQUEST')
    }
    const scope = oryginFileScope(connection.baseURL, connection.apiKey)
    const now = this.now()
    ...
    const upload = async (): Promise<OryginUploadRecord> => {
      ...
      if (remote.bytes !== version.data.byteLength) {
        throw new LlmError('Orygin Files API upload response does not match the submitted image.', 'INVALID_RESPONSE')
      }
      return {
        scope,
    ...
    let candidate: OryginUploadRecord
    try {
  ...
  async invalidate(
    ...
    await this.index.remove(
      oryginFileScope(connection.baseURL, connection.apiKey),
      version.variantId,
      fileId,
    )
  }
  ...
  async reclaimOldestOwned(
    ...
    const client = this.client(connection)
    let after: OryginFileId | undefined
    const owned: OryginFileId[] = []
   ...
   */
  async releaseAll(connection: OryginFileConnection, signal?: AbortSignal): Promise<number> {
    let total = 0
    for (;;) {
    ...
    await this.index.clear(oryginFileScope(connection.baseURL, connection.apiKey))
    return total
  }
}
