/** Durable Orygin attachment-to-file-id index. @module dsh-llm-orygin/upload-index */

import { createHash } from 'node:crypto'
import { readFile, mkdir } from 'node:fs/promises'
import { dirname, join } from 'node:path'
import { withFileLock, writeFileAtomic } from '@orygin-ai/dsh-atomic-write'
import { resolveDshHome } from '@orygin-ai/dsh-home-paths'
import { ImageVariantId } from '@orygin-ai/dsh-attachment'
import type { AttachmentId, ImageVariantId as ImageVariantIdType } from '@orygin-ai/dsh-attachment'
import { OryginFileId, OryginFileScope } from './file-id.ts'
import type { OryginFileId as OryginFileIdType, OryginFileScope as OryginFileScopeType } from './file-id.ts'

/** One durable remote upload mapping. Unix times are milliseconds. */
export interface OryginUploadRecord {
  scope: OryginFileScopeType
 ...
 */
export function oryginFileScope(baseURL: string, apiKey: string): OryginFileScopeType {
  const digest = createHash('sha256')
    .update(baseURL.replace(/\/+$/u, ''))
    .update('\0')
    .update(apiKey)
    .digest('hex')
  return OryginFileScope(digest)
}
...
function parseRecord(value: unknown): OryginUploadRecord {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new InvalidUploadIndexError('llm-orygin: upload index contains a non-object record')
  }
  ...
  if (typeof record.scope !== 'string' || !/^[0-9a-f]{64}$/u.test(record.scope)
    ...
    throw new InvalidUploadIndexError('llm-orygin: upload index contains an invalid record')
  }
  return {
    scope: OryginFileScope(record.scope),
    attachmentId: record.attachmentId as AttachmentId,
    variantId: ImageVariantId(record.variantId),
    fileId: OryginFileId(record.fileId),
...
function parseIndex(text: string): StoredIndex {
  ...
  try {
    ...
    throw new InvalidUploadIndexError('llm-orygin: upload index is not valid JSON', { cause: error })
  }
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new InvalidUploadIndexError('llm-orygin: upload index is not an object')
  }
  const index = value as { formatVersion?: unknown; records?: unknown }
  if (index.formatVersion !== 3 || !Array.isArray(index.records)) {
    throw new InvalidUploadIndexError('llm-orygin: unsupported upload index format')
  }
  const records = index.records.map(parseRecord)
  const keys = new Set<string>()
  for (const record of records) {
    const key = `${record.scope}\0${record.variantId}`
    if (keys.has(key)) throw new InvalidUploadIndexError('llm-orygin: upload index contains duplicate mappings')
    keys.add(key)
  }
  return { formatVersion: 3, records }
}

function reusable(record: OryginUploadRecord, now: number, refreshMarginMs: number): boolean {
  return record.expiresAt - now > refreshMarginMs
}

/** Atomic local index shared by every Orygin session in this ORYGIN home. */
export class OryginUploadIndex {
  /** Absolute owner-private JSON index path. */
  readonly path: string

  /**
   * @param path - explicit test path; omission uses `ORYGIN_HOME/llm-orygin/files-v3.json`.
   */
  constructor(path = join(resolveDshHome(), 'llm-orygin', 'files-v3.json')) {
    this.path = path
  }
  ...
  ): Promise<OryginUploadRecord | undefined> {
   ...
   * @param scope - endpoint/API-key namespace.
   */
  async clear(scope: OryginFileScopeType): Promise<void> {
    await mkdir(dirname(this.path), { recursive: true, mode: 0o700 })
    await withFileLock(this.path, async () => {
      const index = await this.load()
