...
import LlmRuntime, { createUserMessage, LlmAdapter, LlmError, resolveRetryPolicy  } from '@orygin-ai/dsh-llm'
import type { GenerateOptions, ResolvedRetryPolicy, StreamChunk } from '@orygin-ai/dsh-llm'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@orygin-ai/dsh-llm-retry', retry],
    ['@orygin-ai/dsh-agent-loop', AgentLoop],
  ])
...
describe('real Loader composition', () => {
  ...
  it('loads provider-supplied policy and records recovery through the shipping loop', { timeout: 60_000 }, async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@orygin-ai/dsh-llm-retry'",
      "- name: '@orygin-ai/dsh-agent-loop'",
    ])
