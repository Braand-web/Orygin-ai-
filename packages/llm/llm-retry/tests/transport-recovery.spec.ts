...
import type { MockLlmBehavior, MockLlmServer } from '@orygin-ai/dsh-llm-mock-server'
...
async function harness(
  ...
  vi.stubEnv('ORYGIN_API_KEY', 'mock-key')
...
describe('bounded retry through the real Orygin HTTP/SSE adapter', () => {
  ...
  it('stops after the configured transport retry budget is exhausted', async () => {
    ...
    if (end?.type === 'turn/end' && end.data.reason.kind === 'error') {
      expect(end.data.reason.error.message).toContain('Orygin API request to')
    }
  })
})
