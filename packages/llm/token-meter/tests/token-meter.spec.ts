...
import { createUserMessage, CallId, createMessage } from '@orygin-ai/dsh-llm'
import type { ContentBlock, Message, TokenUsage } from '@orygin-ai/dsh-llm'
import SessionStore, { Session, SessionId, canonicalHeader } from '@orygin-ai/dsh-session'
import type { EpochHeader, SessionEvent } from '@orygin-ai/dsh-session'
import SessionProjectionRegistry from '@orygin-ai/dsh-session-projection'
import TokenMeter from '@orygin-ai/dsh-token-meter'
import type { TokenMeasurement, TokenMeterConfig } from '@orygin-ai/dsh-token-meter'
...
describe('TokenMeter pricing', () => {
  ...
  it('prices header, tools, and surface when no reusable usage exists', () => {
    ...
    appendHeader(session, header('orygin-v4-flash', {
      system: 'system',
...
describe('replay anchors and surface folds', () => {
  ...
  it('uses disjoint provider usage and signed durable-output rewrites', () => {
    ...
    appendSuccessfulCall(session, header('orygin-v4-flash'), {
      providerText: 'short',
  ...
  it('selects a heuristic anchor when provider usage would undercut its scale', () => {
    ...
    const requestHeader = header('orygin-v4-flash', { system })
  ...
  it('uses an estimated anchor when provider usage is absent', () => {
    ...
    appendSuccessfulCall(session, header('orygin-v4-flash', { system: 's' }), {
  ...
  it('distinguishes an explicit empty source-event list from an absent legacy list', () => {
    ...
    appendSuccessfulCall(explicit, header('orygin-v4-flash'), {
      durableText: 'listener injected text',
      providerText: '',
      usage: USAGE,
      provenance: 'empty',
    })
    appendSuccessfulCall(legacy, header('orygin-v4-flash'), {
      durableText: 'listener injected text',
      providerText: '',
      usage: USAGE,
      provenance: 'absent',
  ...
  it('invalidates usage for any canonical envelope change or explicit override', () => {
    ...
    const anchoredHeader = header('orygin-v4-flash', { system: 'one' })
    ...
    expect(service.measure(session, header('orygin-v4-flash', { system: 'two' })).baseline.kind)
      .toBe('estimated')
    expect(service.measure(session, header('orygin-v4-pro', { system: 'one' })).baseline.kind)
      .toBe('estimated')
  ...
  it('replays seeded append and replace operations with signed deltas', () => {
    ...
    appendSuccessfulCall(original, header('orygin-v4-flash'), {
      usage: USAGE,
...
describe('malformed replay and listener lifecycle', () => {
  ...
  it('rejects an assistant without its step boundary transactionally', () => {
    ...
    session.append('assistant/message', {
      ...
      message: createMessage({
        ...
        source: {
          kind: 'model',
          ...{ provider: 'mock', model: 'orygin-v4-flash' },
        },
      }),
  ...
  it('clears completed step boundaries and rejects overlapping or late step events', () => {
    ...
    appendHeader(late, header('orygin-v4-flash'))
  ...
  it('rejects repeated and non-earlier assistant source-event references', () => {
    ...
    appendHeader(duplicate, header('orygin-v4-flash'))
    ...
    appendHeader(future, header('orygin-v4-flash'))
