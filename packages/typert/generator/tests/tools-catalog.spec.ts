...
import type { TypertContribution } from '@orygin-ai/dsh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@orygin-ai/dsh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    ...
    const artifact = new FaceModelEmitter(host).emit('@orygin-ai/dsh-tools')
    ...
    const record = ctx.typert.getPackage('@orygin-ai/dsh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@orygin-ai/dsh-tools', 'host')).toBeUndefined()
  })
})
