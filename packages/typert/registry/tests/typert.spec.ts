...
} from '@orygin-ai/dsh-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@orygin-ai/dsh-typert-protocol'
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@orygin-ai/dsh-tools', 'ToolInput')).toBe('@orygin-ai/dsh-tools#ToolInput')
    expect(typertPackageKey('@orygin-ai/dsh-tools', 'host')).toBe('@orygin-ai/dsh-tools#host')
    expect(ctx.typert.get('@orygin-ai/dsh-tools#ToolInput')).toMatchObject({
      package: '@orygin-ai/dsh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@orygin-ai/dsh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@orygin-ai/dsh-tools', 'host')).toMatchObject({
      key: '@orygin-ai/dsh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@orygin-ai/dsh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@orygin-ai/dsh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@orygin-ai/dsh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@orygin-ai/dsh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@orygin-ai/dsh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@orygin-ai/dsh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@orygin-ai/dsh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@orygin-ai/dsh-tools#ToolInput')).not.toBe(projected)
  })
