...
|---|---|---|---|
| `file` | string | `$ORYGIN_SNAPSHOT_FILE` | Path to the primary (parent) `session.jsonl` fixture. Required (config or env). |
| `overrideFile` | string | `$ORYGIN_SNAPSHOT_OVERRIDE` | Optional `ReplayOverrideDoc` sidecar for the primary session: a bare `ReplayEntry[]` replaces its derived script, while `{ patches }` augments it by call index. |
| `childFiles` | string[] | `$ORYGIN_SNAPSHOT_CHILD_FILES` (path-delimited) | Recorded subagent child-session logs for a nested scenario; empty for a single-session scenario. |
    ...
    providers:
      - id: orygin-official
        ...
        models:
          ...
          - id: orygin-v4-pro
  # file/overrideFile/childFiles default to $ORYGIN_SNAPSHOT_FILE /
  # $ORYGIN_SNAPSHOT_OVERRIDE / $ORYGIN_SNAPSHOT_CHILD_FILES, set by the snapshot
