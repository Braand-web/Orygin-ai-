 ...
 * @module @orygin-ai/dsh-llm-replay
 */

import { existsSync, readFileSync, writeFileSync } from 'node:fs'
import { delimiter as pathDelimiter } from 'node:path'
import type { Context } from '@orygin-ai/cordis'
import type {} from '@orygin-ai/dsh-compaction'
import { decodeStorageRecord, type SessionEvent } from '@orygin-ai/dsh-session'
import type {
  ContentBlock,
  GenerateOptions,
  LlmModelInfo,
...
import { LlmAdapter, LlmError, ReasoningEffortId, assertNever, resolveRetryPolicy } from '@orygin-ai/dsh-llm'
...
export function apply(ctx: Context, config: Config = {}): void {
  const file = config.file ?? process.env.ORYGIN_SNAPSHOT_FILE
  if (file === undefined || file.length === 0) {
    throw new Error('llm-replay: a fixture path is required (Config.file or $ORYGIN_SNAPSHOT_FILE)')
  }
  validateConfiguredModalities(config.providers)
  const overrideFile = config.overrideFile ?? process.env.ORYGIN_SNAPSHOT_OVERRIDE
  const childEnv = process.env.ORYGIN_SNAPSHOT_CHILD_FILES
