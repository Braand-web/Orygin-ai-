 ...
 *
 * @module @orygin-ai/dsh-acp-snapshot/harness
 */
...
export interface RunOptions {
   ...
   * them to `dsh-llm-replay` via `$ORYGIN_SNAPSHOT_CHILD_FILES` so each child
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const env: NodeJS.ProcessEnv = {
      ...opts.env,
      ORYGIN_SNAPSHOT: opts.mode,
      ORYGIN_SNAPSHOT_FILE: opts.fixtureFile,
      ORYGIN_SNAPSHOT_SESSIONS_ROOT: sessionsRoot,
      ORYGIN_SNAPSHOT_SPILL_ROOT: spillRoot,
      ORYGIN_HOME: join(cwd, '.dsh'),
      ORYGIN_AGENTS_HOME: join(cwd, '.agents'),
      ...opts.overrideFile !== undefined ? { ORYGIN_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { ORYGIN_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
