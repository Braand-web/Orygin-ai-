 ...
 * zero-build dev path, resolving `@orygin-ai/dsh-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @orygin-ai/dsh-loader-smoke
 */
...
export const EXAMPLE_MODE_ENV = 'ORYGIN_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.ORYGIN_EXAMPLE_MODE`.
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated ORYGIN homes. */
...
export async function runLoaderSmoke(options: LoaderSmokeOptions): Promise<LoaderSmokeResult> {
  ...
  try {
    ...
    const launch = resolveExampleLaunch({
      ...
      env: { ORYGIN_HOME: join(cwd, '.dsh'), ORYGIN_AGENTS_HOME: join(cwd, '.agents'), ...options.env },
    })
