...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@orygin-ai/dsh-llm', LlmRuntime],
    ['@orygin-ai/dsh-session', SessionStore],
    ['@orygin-ai/dsh-token-meter', TokenMeter],
    ['@orygin-ai/dsh-compaction-tool-result-pruner', ToolResultPruner],
    ['@orygin-ai/dsh-compaction-basic', BasicCompactionEngine],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped token-meter, pruning, and compaction-basic YAML order', async () => {
    const loaded = await loadYaml([
      "- name: '@orygin-ai/dsh-llm'",
      "- name: '@orygin-ai/dsh-session'",
      "- name: '@orygin-ai/dsh-token-meter'",
      "- name: '@orygin-ai/dsh-compaction-tool-result-pruner'",
      '  config:',
      '    thresholdChars: 100',
      '    headChars: 20',
      '    tailChars: 10',
