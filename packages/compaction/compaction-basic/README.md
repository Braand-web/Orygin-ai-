...
```yaml
- name: '@orygin-ai/dsh-compaction-basic'
  config:
    thresholdRatio: 0.8
    retainRatio: 0.16
    modelPolicies:
...
- **Overflow classification is adapter-maintained** — provider wording can change; both Orygin adapters normalize currently recognized context-limit failures to `CONTEXT_WINDOW_EXCEEDED`.
