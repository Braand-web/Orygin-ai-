 ...
 * the REAL `dsh-jsonrpc-agent` runtime (per `ORYGIN_EXAMPLE_MODE`) through the
 * REAL `@orygin-ai/dsh-sdk-client`, drives one turn over stdio JSON-RPC,
 * and pins the SDK `RunResult`, the complete notification stream, and the
 * persisted session logs. Replay serves recorded model
 * responses via `llm-replay` (`cordis.snapshot.yml`); `ORYGIN_SNAPSHOT=record`
 * re-records against the live API; `ORYGIN_SNAPSHOT=refresh` replays committed
...
import { resolveExampleLaunch } from '@orygin-ai/dsh-loader-smoke'
import { OryginHarness, type HarnessNotification, type RunResult } from '@orygin-ai/dsh-sdk-client'
...
const mode = process.env.ORYGIN_SNAPSHOT ?? 'replay'
...
const SCENARIOS: SdkScenario[] = [
  ...
  },
  {
    ...
    environment: { ORYGIN_SYSTEM_PROMPT: MINIMAL_SYSTEM_PROMPT },
...
async function runScenario(scenario: SdkScenario): Promise<{
  ...
  const env: Record<string, string> = {
    ...
    ORYGIN_CORDIS_CONFIG: recording
      ? scenario.configs?.live ?? liveConfig
      : scenario.configs?.replay ?? replayConfig,
    ORYGIN_SESSION_ROOT: sessionsRoot,
    ORYGIN_CWD: cwd,
    ORYGIN_SNAPSHOT: mode,
    NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
    ...parentFixture === undefined ? {} : {
      ORYGIN_SNAPSHOT_FILE: parentFixture,
      ...childFixtures.length > 0 ? { ORYGIN_SNAPSHOT_CHILD_FILES: childFixtures.join(delimiter) } : {},
    },
    ...scenario.environment,
  }

  const harness = new OryginHarness({
    launch: {
