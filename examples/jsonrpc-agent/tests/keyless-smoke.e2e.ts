...
describe('jsonrpc-agent keyless smoke', () => {
  it.each([
    ...
    const child = execa(process.execPath, [
      ...
      cwd: repoRoot,
      env: {
        ORYGIN_API_KEY: 'keyless-smoke-no-call',
        ORYGIN_BASE_URL: `http://127.0.0.1:${address.port}`,
        ORYGIN_CWD: root,
        ORYGIN_SESSION_ROOT: join(root, '.sessions'),
        ...(envValue === undefined ? {} : { ORYGIN_MAX_TOKENS_AS_SUCCESS: envValue }),
      },
      timeout: 35_000,
      killSignal: 'SIGKILL',
      reject: false,
    ...
    try {
      child.stdin.write(`${JSON.stringify({
        ...
        id: 1,
        method: 'initialize',
        params: { cwd: root, provider: 'orygin-official', model: 'orygin-v4-pro', maxTokens: 1234 },
      })}\n`)
  ...
  it('rejects an invalid max-token success env value', async () => {
    const { exitCode, stdout, stderr } = await execa(process.execPath, [
      ...
      env: {
        ORYGIN_API_KEY: 'keyless-smoke-no-call',
        ORYGIN_MAX_TOKENS_AS_SUCCESS: 'sometimes',
      },
      stdin: 'ignore',
      timeout: 25_000,
      killSignal: 'SIGKILL',
    ...
    expect(stderr).toContain('failed to apply loader entry sdk-jsonrpc-server (@orygin-ai/dsh-sdk-jsonrpc-server)')
