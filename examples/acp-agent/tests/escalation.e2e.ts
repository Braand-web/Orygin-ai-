 ...
 *
 * With-key escalation flow (self-skips without ORYGIN_API_KEY or a usable
...
function launchExampleAcpAgent(
  ...
  const launched = launchAcpTestAgent({
    ...
    env: {
      ORYGIN_API_KEY: process.env.ORYGIN_API_KEY ?? 'sk-dummy-for-boot',
      ORYGIN_PERMISSION_MODE: sandboxMode,
    },
...
describe.skipIf(!process.env.ORYGIN_API_KEY || !hasRunner)('default sandbox composition e2e: the live approval loop', () => {
