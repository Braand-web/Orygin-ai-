...
import { resolvePwshPath } from '@orygin-ai/dsh-pwsh-local'
import { parseSessionLog } from '@orygin-ai/dsh-llm-replay'
import { OFFLOADED_IMAGE_TEXT } from '@orygin-ai/dsh-llm'
...
function snapshotModeFromEnv(value: string | undefined): SnapshotSuiteOptions['mode'] {
  switch (value) {
    ...
    case 'refresh':
      return 'refresh'
    default:
      throw new Error(`unknown ORYGIN_SNAPSHOT mode: ${value}`)
  }
}
...
const SCENARIOS: Scenario[] = [
  ...
  // `false` command exits 1, rather than becoming SANDBOX_UNAVAILABLE.
  {
    ...
    env: { ORYGIN_PERMISSION_MODE: 'read-only' },
    posixOnly: true,
  },
  ...
  // foreground error and background job marker without a platform runner.
  {
    ...
    configPath: PARTIAL_LANDLOCK_CONFIG,
    env: {
      ORYGIN_PERMISSION_MODE: 'read-only',
      ORYGIN_SNAPSHOT_MISSING_SANDBOX_RUNNER: '1',
    },
    posixOnly: true,
  },
  ...
  // published-handle disposal failure.
  {
    ...
    env: { ORYGIN_SUBAGENT_PUBLISHED_FAILURE: '1' },
    hasModelTurn: true,
    recorded: false,
    overridden: true,
    configPath: SUBAGENT_DURABILITY_FAILURE_CONFIG,
  ...
  // no session-scoped UI picker on the automation protocol.
  {
    ...
    systemPromptSource: 'text-turn',
    toolSchemasSource: 'text-turn',
    env: { ORYGIN_PERMISSION_MODE: 'workspace-write' },
  },
  {
...
defineAcpSnapshotSuite({
  ...
  scenarios: SCENARIOS,
  mode: snapshotModeFromEnv(process.env.ORYGIN_SNAPSHOT),
  hasPwsh,
})

it('pins native Orygin Files offload and inline fallback in assembled requests', async () => {
  ...
  try {
    const result = await runScenario(input, {
      ...
      env: {
        ORYGIN_SNAPSHOT_API_KEY: 'snapshot-key',
        ORYGIN_SNAPSHOT_BASE_URL: `http://127.0.0.1:${address.port}`,
      },
    })
    ...
    const fallback = await runScenario(input, {
      ...
      workspaceDir: join(SNAPSHOTS_DIR, 'read-image', 'workspace'),
      env: {
        ORYGIN_SNAPSHOT_API_KEY: 'snapshot-fallback-key',
        ORYGIN_SNAPSHOT_BASE_URL: `http://127.0.0.1:${address.port}`,
      },
    })
