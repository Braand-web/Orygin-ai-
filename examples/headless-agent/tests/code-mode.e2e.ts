...
import LlmRuntime, { createUserMessage, CallId, HarnessError  } from '@orygin-ai/dsh-llm'
...
import ToolRuntime, { RUN_CODE_NAME, defineTool } from '@orygin-ai/dsh-tools'
import type { ToolExecutionResult } from '@orygin-ai/dsh-tools'
...
import { WorkerThreadCodeRuntime } from '@orygin-ai/dsh-code-runtime-worker-thread'
...
async function codeModeHarness(cwd: string): Promise<Context> {
  ...
  await harness.plugin(LlmOrygin)
...
async function workspaceCodeModeHarness(): Promise<Context> {
  ...
  await harness.plugin(AgentLoop, { agents: [] })
  await harness.plugin(LlmOrygin, { models: [{ id: 'orygin-v4-flash' }] })
  await harness.plugin(WorkerThreadCodeRuntime, {})
  return harness
}
...
describe.skipIf(!process.env.ORYGIN_API_KEY)('Code Mode: real model writes a program over real tools', () => {
  it('collapses the wire tool list to [run_code], bridges sub-calls, and returns curated output', async () => {
    ...
    const agent = ctx.agentLoop.create(SessionId('e2e-code-mode'), { provider: 'orygin-official', model: 'orygin-v4-flash' })
  ...
  it('projects nested workspace instructions discovered by an fs sub-call', async () => {
    ...
    const handle = await ctx.agents.create({
      ...
      meta: { cwd: workdir },
      agentOptions: { provider: 'orygin-official', model: 'orygin-v4-flash' },
    })
