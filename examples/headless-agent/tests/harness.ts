...
import ToolResultPruner from '@orygin-ai/dsh-compaction-tool-result-pruner'
import JsonlSessionPersistence from '@orygin-ai/dsh-session-persistence-jsonl'
import * as SessionCheckpointPolicy from '@orygin-ai/dsh-session-checkpoint-policy'
import { BasicCompactionEngine } from '@orygin-ai/dsh-compaction-basic'
import type { BasicCompactionConfig } from '@orygin-ai/dsh-compaction-basic'

/**
 * Shared harness for the headless-agent e2e suites: the full plugin stack
 * with the real Orygin adapter and the real bash + todo_write tools. Lives
...
export interface CodingHarnessOptions {
  ...
  /** Test-only context capacity advertised for `orygin-v4-flash`. */
...
export async function codingHarness(workdir: string, options: CodingHarnessOptions = {}): Promise<Context> {
  ...
  await ctx.plugin(LlmOrygin, options.modelContextWindow === undefined ? {} : {
    models: [{ id: 'orygin-v4-flash', contextWindow: options.modelContextWindow }],
  })
