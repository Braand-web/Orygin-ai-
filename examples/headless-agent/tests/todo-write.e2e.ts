...
describe.skipIf(!process.env.ORYGIN_API_KEY)('todo_write: real model records a plan', () => {
  it('appends a todo/write event with the model-produced task list', async () => {
    ...
    const agent = ctx.agentLoop.create(SessionId('e2e-todo'), { provider: 'orygin-official', model: 'orygin-v4-flash' })
