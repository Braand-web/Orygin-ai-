...
describe.skipIf(!process.env.ORYGIN_API_KEY)('coding task: fix a failing test via bash', () => {
  it('repairs add.js so node add.test.js passes', async () => {
    ...
    const agent = ctx.agentLoop.create(SessionId('e2e-task'), { provider: 'orygin-official', model: 'orygin-v4-flash' })
