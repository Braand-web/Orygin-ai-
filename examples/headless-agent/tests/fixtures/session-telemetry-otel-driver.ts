...
import { boot, resolveConfigPath } from '@orygin-ai/dsh-app-boot'
import { recordFeedback } from '@orygin-ai/dsh-command-feedback'
import { runFixtureTurn } from '@orygin-ai/dsh-loader-smoke'
...
process.env.ORYGIN_TELEMETRY_E2E_URL = `http://127.0.0.1:${address.port}/v1/logs`
...
try {
  ...
  const mode = process.env.ORYGIN_TELEMETRY_E2E_MODE ?? 'FULL'
  if (mode !== 'FULL') {
