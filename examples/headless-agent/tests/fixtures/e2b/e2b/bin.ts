...
import { boot } from '@orygin-ai/dsh-app-boot'
...
import type {} from '@orygin-ai/dsh-fs-e2b'
import type {} from '@orygin-ai/dsh-bash-local'
import type {} from '@orygin-ai/dsh-lsp-stdio'
import type {} from '@orygin-ai/dsh-terminal-bash'
...
try {
  ...
  const sleeping = ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "printf 'ORYGIN_SLEEP_%s\\n' READY; sleep 30",
    submit: true,
  })
  let sleepReadyOutput = ''
  const sleepReadyDeadline = Date.now() + 5_000
  while (!sleepReadyOutput.includes('ORYGIN_SLEEP_READY\n')) {
    sleepReadyOutput += sleeping.readOutput().delta
    if (sleepReadyOutput.includes('ORYGIN_SLEEP_READY\n')) break
  ...
  const stubborn = await ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "bash -c 'trap \"\" TERM; exec sleep 30' & printf 'ORYGIN_STUBBORN_PID=%s\\n' \"$!\"",
    submit: true,
  }).done
  const stubbornMatch = /ORYGIN_STUBBORN_PID=([1-9][0-9]*)/.exec(stubborn.viewport)
