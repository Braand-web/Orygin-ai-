...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, runLoaderSmoke } from '@orygin-ai/dsh-loader-smoke'
import {
  decompressZstdFrame,
  scanZstdFrames,
} from '@orygin-ai/dsh-session-persistence-jsonl/src/zstd.ts'
...
const oryginDefaultsConfigPath = fileURLToPath(new URL('./fixtures/orygin-defaults.cordis.yml', import.meta.url))
...
const refreshing = process.env.ORYGIN_SNAPSHOT === 'refresh'
...
interface OryginDefaultsServer {
  readonly url: string
  readonly requests: JsonObject[]
  close(): Promise<void>
}

/** Serve one deterministic Orygin-compatible response while retaining its request body. */
async function oryginDefaultsServer(): Promise<OryginDefaultsServer> {
  ...
  if (address === null || typeof address === 'string') throw new Error('Orygin defaults snapshot server has no port')
  return {
...
describe('headless stream-json snapshots', () => {
  it('runs one task through the product headless profile command', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tsconfigPath,
      env: {
        ORYGIN_PERMISSION_MODE: 'danger-full-access',
        ORYGIN_TELEMETRY_DISABLED: '1',
  ...
  it('recovers from context overflow through an assembled compaction', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ORYGIN_SNAPSHOT: 'replay',
        ORYGIN_SNAPSHOT_FILE: compactionSessionFixture,
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
  ...
  it('logs actionable missing-credential guidance through the one-shot app', async () => {
    ...
    expect(normalized).toContain(
      'store ORYGIN_API_KEY through the credentials service (the web Models page writes it),',
    )
    expect(normalized).toContain('or export ORYGIN_API_KEY in the launching environment')
  ...
  it('logs actionable invalid-credential guidance through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ...
        ORYGIN_API_KEY: 'sk-\u{1F600}pasted-from-a-chat-window',
        ORYGIN_BASE_URL: '',
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
    ...
    // page at all.
    expect(normalized).toContain('the API key resolved from ORYGIN_API_KEY contains characters')
  ...
  it('keeps provider comments alive and sends Orygin defaults through the one-shot app', async () => {
    const server = await oryginDefaultsServer()
    try {
      const result = await runLoaderSmoke({
        label: 'Orygin adapter defaults headless stream-json snapshot',
        tempDirPrefix: 'headless-snapshot-orygin-defaults-',
        binScript,
        libBinScript: binScript,
        configPath: oryginDefaultsConfigPath,
        binArgs: [
          oryginDefaultsConfigPath,
        ...
        env: {
          ...
          ORYGIN_API_KEY: 'snapshot-key',
          ORYGIN_SNAPSHOT_BASE_URL: server.url,
          NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
        },
      })
  ...
  it('replays the advanced toolchain through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ORYGIN_SNAPSHOT: 'replay',
        ORYGIN_SNAPSHOT_FILE: advancedSessionFixture,
        ORYGIN_SNAPSHOT_CHILD_FILES: [
          join(advancedScenarioDir, 'session.1.jsonl'),
          join(advancedScenarioDir, 'session.2.jsonl'),
        ].join(delimiter),
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
  ...
  it('replays persisted goal tools through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ORYGIN_SNAPSHOT: 'replay',
        ORYGIN_SNAPSHOT_FILE: join(goalScenarioDir, 'session.jsonl'),
        ORYGIN_SNAPSHOT_OVERRIDE: join(goalScenarioDir, 'replay.override.json'),
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
  ...
  it('replays two fresh Ralph rounds through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ORYGIN_SNAPSHOT: 'replay',
        ORYGIN_SNAPSHOT_FILE: join(ralphScenarioDir, 'session.jsonl'),
        ORYGIN_SNAPSHOT_OVERRIDE: join(ralphScenarioDir, 'replay.override.json'),
        ORYGIN_SNAPSHOT_CHILD_FILES: [
  ...
  it('delivers a continuable child result without parent polling', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ...
        ORYGIN_SNAPSHOT_FILE: parentReplay,
        ORYGIN_SNAPSHOT_OVERRIDE: parentOverride,
        ORYGIN_SNAPSHOT_CHILD_FILES: childReplay,
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
  ...
  it('replays persistent PTY tools through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ORYGIN_SNAPSHOT: 'replay',
        ORYGIN_SNAPSHOT_FILE: ptySessionFixture,
        NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
      },
      prepare: (cwd) => { runCwd = cwd },
      inspect: async (cwd) => {
