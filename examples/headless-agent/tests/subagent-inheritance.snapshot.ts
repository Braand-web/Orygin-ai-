...
import { normalizeSessionSnapshot, type NormalizeContext } from '@orygin-ai/dsh-acp-snapshot'
...
import SessionStore, { SESSION_FORMAT_VERSION, SessionId, type SessionEvent, type SessionHeader } from '@orygin-ai/dsh-session'
  ...
  it('confines a delegated child through the assembled headless app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      env: {
        ...
        ORYGIN_SNAPSHOT_FILE: replayOverride,
        ORYGIN_SNAPSHOT_OVERRIDE: replayOverride,
      ...
      inspect: async (runCwd) => {
        ...
        const runtimeContexts = (content: string): string[] => content.trimEnd().split('\n').flatMap((line) => {
          ...
          if (record.type !== 'user/message'
            ...
            || record.data.source.plugin !== '@orygin-ai/dsh-system-prompt') return []
        ...
        for (const context of policyContexts) {
          expect(context).toContain('Any available operation enforced by the ORYGIN file sandbox cannot modify files in the standing mode.')
