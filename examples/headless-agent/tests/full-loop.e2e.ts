...
describe.skipIf(!process.env.ORYGIN_API_KEY)('full loop: real model + real bash tool', () => {
  it('runs a bash command on request and reports its output', async () => {
    ...
    const agent = ctx.agentLoop.create(SessionId('e2e-loop'), { provider: 'orygin-official', model: 'orygin-v4-flash' })
