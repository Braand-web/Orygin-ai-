...
The public package family belongs to the `@orygin-ai` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@orygin-ai/node-addon-landlock-run
@orygin-ai/node-addon-landlock-run-<platform>
```
