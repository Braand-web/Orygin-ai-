...
const STANDALONE_ERROR = 'apps/web is not a standalone application: bare Vite cannot inject window.__ORYGIN_BOOT__. '
  + 'From a repository checkout, run `pnpm dsh web`; an installed package uses `dsh web`. '
  + 'For client-plugin HMR, run `pnpm dsh web` together with `pnpm run dev:web`.'
const DEFAULT_CLIENT_TITLE = 'ORYGIN Local Build'
...
function clientDocumentTitle(): Plugin {
  const title = escapeHtmlText(process.env.ORYGIN_CLIENT_TITLE ?? DEFAULT_CLIENT_TITLE)
  return {
    name: 'dsh-client-document-title',
    transformIndexHtml(html) {
      return html.replace('<title>ORYGIN Local Build</title>', `<title>${title}</title>`)
    },
  }
}
