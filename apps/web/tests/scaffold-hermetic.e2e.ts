...
import type {} from '@orygin-ai/dsh-agent-presets'
...
it('isolates replay skill discovery from every ambient host root', async () => {
  ...
  const originalDshHome = process.env.ORYGIN_HOME
  const originalAgentsHome = process.env.ORYGIN_AGENTS_HOME
  const originalBundled = process.env.ORYGIN_BUNDLED_SKILL_DIR
  process.env.ORYGIN_HOME = dshHome
  process.env.ORYGIN_AGENTS_HOME = agentsHome
  ...
  try {
    ...
    try {
      ...
      if (originalDshHome === undefined) delete process.env.ORYGIN_HOME
      else process.env.ORYGIN_HOME = originalDshHome
      if (originalAgentsHome === undefined) delete process.env.ORYGIN_AGENTS_HOME
      else process.env.ORYGIN_AGENTS_HOME = originalAgentsHome
      if (originalBundled === undefined) delete process.env.ORYGIN_BUNDLED_SKILL_DIR
      else process.env.ORYGIN_BUNDLED_SKILL_DIR = originalBundled
