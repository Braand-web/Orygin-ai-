...
// figma comparison pass. Self-skips without ORYGIN_API_KEY (repo e2e
...
describe('dsh web keyless CLI smoke', () => {
  it('listens on 127.0.0.1 by default', async () => {
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...process.env,
          ORYGIN_API_KEY: 'keyless-web-no-call',
          ORYGIN_HOME: join(sessionsDir, '.dsh'),
          ORYGIN_AGENTS_HOME: join(sessionsDir, '.agents'),
  ...
  it('routes web runtime context and workspace instructions through the real CLI request', async () => {
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...process.env,
          ORYGIN_API_KEY: 'keyless-web-workspace',
          ORYGIN_BASE_URL: `http://127.0.0.1:${address.port}`,
          ORYGIN_HOME: join(workspace, '.dsh'),
          ORYGIN_AGENTS_HOME: join(workspace, '.agents'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
        stdio: ['ignore', 'pipe', 'pipe'],
      },
  ...
  it('retries a partial transport failure through the shipped Web composition', async () => {
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...process.env,
          ORYGIN_API_KEY: 'keyless-web-retry',
          ORYGIN_BASE_URL: `http://127.0.0.1:${address.port}`,
          ORYGIN_HOME: join(workspace, '.dsh'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
  ...
  it('ORYGIN_TOOLS_MODE=code collapses the provider wire tools to run_code with the SDK prompt section', async () => {
    requireDist()
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...process.env,
          ORYGIN_API_KEY: 'keyless-web-code-mode',
          ORYGIN_BASE_URL: `http://127.0.0.1:${address.port}`,
          ORYGIN_TOOLS_MODE: 'code',
          ORYGIN_HOME: join(workspace, '.dsh'),
          ORYGIN_AGENTS_HOME: join(workspace, '.agents'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
...
describe.skipIf(!process.env.ORYGIN_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    // This scenario spawns its own server against a fresh $ORYGIN_HOME with the
    // Orygin credential inherited from the environment, so no onboarding
