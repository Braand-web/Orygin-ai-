...
import { bootInjections, orderByModuleGraph } from '@orygin-ai/dsh-client-modules'
import type { ClientModuleLoaderTarget, WebBootEntry } from '@orygin-ai/dsh-client-modules/client'
...
const appBoot = await import(pathToFileURL(webBundleResolver.resolve('@orygin-ai/dsh-app-boot')).href) as unknown as BootComposition
...
interface FixtureWindow extends Window {
  __ORYGIN_BOOT__?: { rev: string; entries: WebBootEntry[] }
...
export function mountAssembledApp(search = '?fixture'): void {
  ...
  win.__ORYGIN_BOOT__ = { rev: 'fx', entries: PLUGINS.map(({ bundlePath: _bundlePath, ...plugin }) => plugin) }
  const [facadeRow] = bootInjections(win.__ORYGIN_BOOT__)
  ...
  for (const id of ['@orygin-ai/dsh-client-modules', '@orygin-ai/dsh-client-runtime']) {
 ...
 * the snapshot gate's `ORYGIN_SNAPSHOT` mode (`record` re-runs the scenarios from
 * scratch, `refresh` re-derives the expected text from the existing ones).
 */
export const REFRESHING_GOLDEN = process.env.ORYGIN_SNAPSHOT === 'record' || process.env.ORYGIN_SNAPSHOT === 'refresh'
