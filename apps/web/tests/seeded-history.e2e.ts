...
// fixture under the same record discipline as every other: ORYGIN_SNAPSHOT=record drives the turn
...
import type { ContentBlock, Message } from '@orygin-ai/dsh-llm'
import { deriveEventMessage, SessionId } from '@orygin-ai/dsh-session'
...
describe('web e2e: seeded history renders through cold resume', () => {
  ...
  it.skipIf(MODE === 'record')('reports full feedback correlation ids in an expandable two-line row', async () => {
    ...
    process.env.ORYGIN_HOME = scaffold.harnessHome
    try {
      const input = page.locator('textarea').first()
