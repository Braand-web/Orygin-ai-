...
// downlink, api-gateway, agent loop, tools, and persistence. Modes ride $ORYGIN_SNAPSHOT:
// replay (default, keyless: normally disables the llm-orygin row and
...
// built dist; ordinary keyless modes disable llm-orygin and fill the open
...
import Include, { type PatchOptions } from '@orygin-ai/cordis-plugin-include'
import Group from '@orygin-ai/cordis-plugin-group'
import {
  scrubRequestHeaders,
  scrubSessionSnapshot,
  stabilizeFixtureMessageIds,
} from '@orygin-ai/dsh-acp-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
  healProfilesModuleFallback,
  loadOverlayPatches,
} from '@orygin-ai/dsh-app-boot'
import { dshHomePath } from '@orygin-ai/dsh-home-paths'
import { settingsNamespace } from '@orygin-ai/dsh-settings'
import { LlmAdapter } from '@orygin-ai/dsh-llm'
import type {
  LlmModelInfo, LlmProviderInfo, LlmResolvedModelInfo, RetryPolicyConfig, StreamChunk,
} from '@orygin-ai/dsh-llm'
import type { ReplayHandle } from '@orygin-ai/dsh-llm-replay'
import { installLlmReplay, parseSessionLog } from '@orygin-ai/dsh-llm-replay'
import SessionStore, {
  packChunkRuns,
  ...
  type SessionEvent,
  type SessionHeader,
} from '@orygin-ai/dsh-session'
import JsonlSessionPersistence from '@orygin-ai/dsh-session-persistence-jsonl'
// Empty type imports carry the webServer/agents/sessionPersistence Context merges.
import type {} from '@orygin-ai/dsh-host-webserver'
import type {} from '@orygin-ai/dsh-agent'
import { provideCmdline } from '@orygin-ai/dsh-cmdline'
...
// } from '@orygin-ai/dsh-client-ui-settings-models'
...
export const WELCOME_NOTICE_COPY = {
  zh: {
    title: '内测声明',
    body: 'Orygin 目前的 0.1 版本仍处在面向 Harness 开发者进行测试的阶段，还有许多地方需要持续改进和打磨，希望听取广大开发者的反馈建议。预计 Orygin 的核心插件以及基础 API 都会在接下来的一段时间内快速迭代、持续演化。\n\n我们期待与全球开发者一起，在开源、开放、可复用、可组合的基础设施之上，共同探索智能上限。欢迎全球 Harness 开发者加入 ORYGIN 插件生态。',
    continueLabel: '继续',
  },
} as const

/** Snapshot mode for the lane, from $ORYGIN_SNAPSHOT (same vocabulary as the other snapshot suites). */
...
export function webSnapshotMode(): WebSnapshotMode {
  const value = process.env.ORYGIN_SNAPSHOT
  if (value === undefined || value === '' || value === 'replay') return 'replay'
  if (value === 'record' || value === 'refresh') return value
  throw new Error(`ORYGIN_SNAPSHOT must be replay, record, or refresh; got ${JSON.stringify(value)}`)
}
...
// mode, never catch-all: with llm-orygin disabled no adapter exists, so a
...
const REPLAY_PROVIDERS = [{
  id: 'orygin-official',
  name: 'Orygin',
  models: [{ id: 'orygin-v4-flash', name: 'Orygin-V4-Flash', contextWindow: 128_000 }],
}]
...
export interface WebScaffold {
  ...
  /** Isolated harness home the settings/credentials rows write ($ORYGIN_HOME double). */
  harnessHome: string
...
export interface LaunchOptions {
   ...
   * fails loud with NO_ADAPTER (llm-orygin is disabled and no replay row
  ...
  cordisTools?: boolean
  /**
   * Keep the shipped Orygin adapter mounted while masking the process
   * environment's ORYGIN_API_KEY for this scaffold lifetime. This is the
   * keyless first-run configuration lane; the default disables the adapter.
   */
  oryginMissingCredential?: boolean
  /** Leave the current welcome notice pending; ordinary scenarios pre-acknowledge it before browser boot. */
  welcomeNoticePending?: boolean
  /**
   * Patch the shipped Orygin search row to a deterministic endpoint and
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  if (mode === 'record') {
    ...
    if (process.env.ORYGIN_API_KEY === undefined || process.env.ORYGIN_API_KEY.length === 0) {
      throw new Error('web e2e record mode needs ORYGIN_API_KEY (env or repo-root .env)')
    }
  }
  if (mode === 'record' && options.oryginMissingCredential === true) {
    throw new Error('oryginMissingCredential is a keyless replay/refresh option')
  }
  const maskOryginCredential = mode !== 'record' && options.oryginMissingCredential === true
  const originalOryginCredential = process.env.ORYGIN_API_KEY
  let credentialEnvironmentRestored = false
  const restoreCredentialEnvironment = (): void => {
    if (credentialEnvironmentRestored || !maskOryginCredential) return
    credentialEnvironmentRestored = true
    if (originalOryginCredential === undefined) {
      Reflect.deleteProperty(process.env, 'ORYGIN_API_KEY')
    } else {
      process.env.ORYGIN_API_KEY = originalOryginCredential
    }
  }
  const workspaceCwd = await realpath(await mkdtemp(join(tmpdir(), 'dsh-web-e2e-ws-')))
  // Isolated harness home: the settings/credentials rows resolve $ORYGIN_HOME
  ...
  // enters replay requests and goldens while CI sees none. `ORYGIN_HOME` follows
  ...
  const skillRootEnvironment = {
    ORYGIN_HOME: harnessHome,
    ORYGIN_AGENTS_HOME: join(workspaceCwd, '.agents-home'),
    ORYGIN_BUNDLED_SKILL_DIR: join(workspaceCwd, '.bundled-skills'),
  }
  ...
  if (maskOryginCredential) Reflect.deleteProperty(process.env, 'ORYGIN_API_KEY')
  ...
  const patches: PatchOptions[] = [
    ...
    // storage-json's yml root is anchored to the real $ORYGIN_HOME; pin the row
    ...
    // to the production OTLP endpoint (or whatever ORYGIN_TELEMETRY_OTLP_URL
    ...
    // (apps/web IS @orygin-ai/dsh-web-frontend); native browser opening and the
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@orygin-ai/dsh-host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@orygin-ai/dsh-client-ui-directory-picker-browse' },
    ] },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@orygin-ai/dsh-tool-cordis' },
      ] }]
      : [],
    ...options.oryginSearch === undefined
      ? []
      : [{
        id: 'web-search-orygin',
        config: {
          apiKeyEnv: options.oryginSearch.apiKeyEnv,
          baseURL: options.oryginSearch.baseURL,
        },
      }],
    ...mode === 'record' || options.oryginMissingCredential === true
      ? []
      : [{ id: 'llm-orygin', disabled: true }],
  ]
  ...
  try {
    ...
    // `@orygin-ai/cordis-plugin-group` by name.
    ...
    // disable llm-orygin; the first-run lane keeps it mounted but has no
    ...
    } else if (mode !== 'record' && options.oryginMissingCredential !== true) {
...
export async function compareOrRefreshGolden(goldenPath: string, actual: string, mode: WebSnapshotMode): Promise<void> {
  ...
  if (!existsSync(goldenPath)) {
    throw new Error(`missing golden ${goldenPath} — run ORYGIN_SNAPSHOT=refresh pnpm run test:web to generate it`)
  }
