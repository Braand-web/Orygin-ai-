...
import type { ReplayEntry, ReplayOverrideDoc } from '@orygin-ai/dsh-llm-replay'
...
import type {} from '@orygin-ai/dsh-session-title'
import {
  launchWebScaffold,
  seedSession,
  watchConsole,
...
function appendRequestHeader(session: Session, turn: number, step: number): void {
  session.append('request/header', {
    header: {
      config: { provider: 'orygin-official', model: 'orygin-v4-flash' },
...
function appendAssistant(
  ...
  session.append('assistant/message', {
    ...
    message: createAssistantMessage({
      content: text(body),
      source: { provider: 'orygin-official', model: 'orygin-v4-flash' },
    }),
    usage: {
