...
// for the official Orygin key again, and the first-run setup card is a card
// they can close. The shipped Orygin adapter stays mounted without a
...
describe.skipIf(MODE === 'record')('web e2e: another usable provider ends first-run onboarding', () => {
  ...
  it('closes the setup card without discarding the add card beside it', async () => {
    ...
    await settings.getByRole('button', { name: '编辑 Orygin (orygin-official)' }).waitFor({ timeout: 10_000 })
  ...
  it('stops prompting for Orygin once the other provider can serve requests', async () => {
    ...
    // Only minimax-cn is reachable; Orygin still holds no credential.
    ...
    expect(credentials).not.toContain('ORYGIN_API_KEY')
    ...
    // The Models page agrees: Orygin stays a row rather than reopening its
