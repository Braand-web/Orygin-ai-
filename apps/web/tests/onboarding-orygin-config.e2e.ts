// Keyless browser e2e: the shipped Orygin adapter stays mounted while its
...
const SNAPSHOT_DIR = fileURLToPath(new URL('./snapshots/onboarding-orygin-config', import.meta.url))
...
describe.skipIf(MODE === 'record')('web e2e: first-run Orygin credential setup', () => {
  ...
  beforeAll(async () => {
    scaffold = await launchWebScaffold({ oryginMissingCredential: true, welcomeNoticePending: true })
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-orygin-config'))
    ...
    expect(stored.includes(`ORYGIN_API_KEY: ${secret}`)).toBe(true)
    ...
    const oryginRow = settings.getByText('Orygin', { exact: true }).first()
    await oryginRow.waitFor({ timeout: 10_000 })
    await oryginRow.locator('xpath=ancestor::li').getByRole('button', { name: '编辑' }).click()
  ...
  it('configures arbitrary Orygin models and prompts after the selected model is removed', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-orygin-models'))
    ...
    await settings.waitFor({ timeout: 10_000 })
    await settings.getByRole('button', { name: '模型' }).click()
    const orygin = settings.getByText('Orygin', { exact: true }).first()
    await orygin.waitFor({ timeout: 10_000 })
    await orygin.locator('xpath=ancestor::li').getByRole('button', { name: '编辑' }).click()
    ...
    expect(document).toContain('id: orygin-v4-pro')
    expect(document).toContain('id: orygin-v4-flash-vision-exp')
    ...
    expect(document).not.toMatch(/^\s*- id: orygin-v4-flash$/m)
    ...
    expect(await page.getByText('orygin-v4-flash', { exact: true }).count()).toBe(0)
    await page.getByRole('menuitemradio', { name: 'Orygin-V4-Flash-Vision-Exp' }).waitFor({ timeout: 10_000 })
