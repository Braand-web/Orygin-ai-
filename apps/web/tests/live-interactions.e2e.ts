...
import type { RetryPolicyConfig } from '@orygin-ai/dsh-llm'
import { deriveReplayScript, parseSessionLog } from '@orygin-ai/dsh-llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  it.skipIf(MODE === 'record')('cancels a hung stream deterministically via the readyFile marker', async () => {
    ...
    await expect.poll(
      () => page.getByRole('status').filter({ hasText: 'Orygin diving...' }).isVisible(),
      { timeout: 10_000 },
    ).toBe(true)
