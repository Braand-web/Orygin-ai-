...
import { startMockLlmServer } from '@orygin-ai/dsh-llm-mock-server'
...
function createProfileLifecycleFixture(): ProfileLifecycleFixture {
  ...
  writeFileSync(join(bundleDir, 'plugin.mjs'), [
    ...
    "  writeFileSync(join(process.env.ORYGIN_HOME, 'config-echo'), String(config.generation ?? 'bundle-default'))",
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    ...
    private: true,
    dependencies: {},
    dsh: { profile: { bundles: ['@orygin-ai/dsh-base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@orygin-ai/dsh-cmdline` and `commander` through the profile module
...
function createStartupFixture(): StartupFixture {
  ...
  writeFileSync(join(bundleDir, 'startup.mjs'), [
    ...
    "import { parseCmdline } from '@orygin-ai/dsh-cmdline'",
  ...
  writeFileSync(join(bundleDir, 'witness.mjs'), [
    ...
    'export function apply(ctx, config = {}) {',
    "  writeFileSync(join(process.env.ORYGIN_HOME, 'witness'), String(config.generation ?? 'bundle-default'))",
    '}',
    '',
  ].join('\n'))
  writeFileSync(join(bundleDir, 'cordis.patch.yml'), [
...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    ...
    try {
      ...
      expect(wildcardHost.stderr).toContain('--host 0.0.0.0 is intentionally not supported without Supabase authentication')
  ...
  it('runs the headless profile through its app-owned task positional', async () => {
    ...
    try {
      const result = await runBuiltBin(['--profile', 'headless', 'answer', 'from', 'the', 'published', 'entry'], {
        ORYGIN_HOME: home,
        ORYGIN_TELEMETRY_DISABLED: '1',
        ORYGIN_API_KEY: apiKey,
        ORYGIN_BASE_URL: server.baseURL,
      })
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    ...
    try {
      const result = await runBuiltBin(['--profile', 'nope'], { ORYGIN_HOME: home })
      expect(result.code).toBe(1)
  ...
  it('reports a patch-overlay boot failure without hanging', async () => {
    ...
    try {
      const result = await runBuiltBin(['--profile', 'web', '--patch', invalidProvider], {
        ORYGIN_HOME: home,
        ORYGIN_API_KEY: 'keyless-invalid-config',
        ORYGIN_TELEMETRY_DISABLED: '1',
      })
      expect(result.code).toBe(1)
  ...
  it('fully settles a custom profile, hot-reloads its patch layer with removal reverting, and disposes on a signal', async () => {
    ...
    try {
      ...
      // The home-level user layer ($ORYGIN_HOME/cordis.patch.yml) is live too
  ...
  it('activates a dependency that gained dsh.bundle in a later update', async () => {
    ...
    try {
      ...
      const first = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { ORYGIN_HOME: home })
      expect(first.code).toBe(0)
      let manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { dsh: { profile: { bundles: string[] } } }
      expect(manifest.dsh.profile.bundles).toEqual(['@orygin-ai/dsh-base'])
      ...
      const second = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { ORYGIN_HOME: home })
      expect(second.code).toBe(0)
      manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { dsh: { profile: { bundles: string[] } } }
      expect(manifest.dsh.profile.bundles).toEqual(['@orygin-ai/dsh-base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  describe('config dump', () => {
    ...
    it('prints the web profile bundle layers without a user layer', async () => {
      const { stdout, code, stderr } = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { ORYGIN_HOME: home })
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@orygin-ai/dsh-agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @orygin-ai/dsh-base')
      expect(stdout).toContain("name: '@orygin-ai/dsh-host-webserver'")
    }, 30_000)
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@orygin-ai/dsh-headless'")
      expect(stdout).not.toMatch(/name: '@orygin-ai\/dsh-host-/)
      expect(stdout).not.toContain("name: '@orygin-ai/dsh-web-app'")
      expect(stdout).not.toMatch(/name: '@orygin-ai\/dsh-client-/)
    }, 30_000)

    it('composes the profile user layer and a --patch overlay in order', async () => {
      // Auto-init the web profile first, then write its user layer.
      const init = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { ORYGIN_HOME: home })
