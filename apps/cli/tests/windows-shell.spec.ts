...
import { entryListSchema } from '@orygin-ai/cordis-plugin-include'
import { evaluate } from '@orygin-ai/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@orygin-ai/dsh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@orygin-ai/dsh-base', '@orygin-ai/dsh-web-app'])
    ...
    for (const name of ['@orygin-ai/dsh-pwsh-sandbox', '@orygin-ai/dsh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@orygin-ai/dsh-base'])
