...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, resolveExampleLaunch } from '@orygin-ai/dsh-loader-smoke'
...
async function runHeadlessPtySmoke(): Promise<string> {
  ...
  try {
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      ...
      private: true,
      dependencies: {},
      dsh: { profile: { bundles: ['@orygin-ai/dsh-base', '@orygin-ai/dsh-headless'] } },
    }, undefined, 2))
    ...
    const launch = resolveExampleLaunch({
      ...
      env: {
        ...
        ORYGIN_API_KEY: 'keyless-shutdown-no-call',
        ORYGIN_TELEMETRY_DISABLED: '1',
        ORYGIN_TEST_SHUTDOWN_ARM_FILE: join(cwd, 'shutdown-armed'),
      },
    })
