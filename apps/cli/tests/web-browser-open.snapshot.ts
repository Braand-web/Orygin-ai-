...
if (process.env.ORYGIN_EXAMPLE_MODE === 'lib' && !builtArtifactsExist) {
...
describe.skipIf(!builtArtifactsExist)('dsh web browser-open assembled snapshot', () => {
  it('hands the reachable page to the default browser after the shipped tree settles', async () => {
    ...
    const result = await execa(process.execPath, [
      ...
      env: {
        ...process.env,
        ORYGIN_API_KEY: 'keyless-browser-open-no-call',
        ORYGIN_AGENTS_HOME: join(root, '.agents'),
        ORYGIN_HOME: join(root, '.dsh'),
  ...
  it('prints the launcher reason and manual URL after the Web app is ready', async () => {
    ...
    const result = await execa(process.execPath, [
      ...
      env: {
        ...
        ORYGIN_API_KEY: 'keyless-browser-open-no-call',
        ORYGIN_AGENTS_HOME: join(root, '.agents'),
        ORYGIN_BROWSER_OPEN_TEST_EXIT_ON_FAILURE: '1',
        ORYGIN_HOME: join(root, '.dsh'),
        ORYGIN_TELEMETRY_DISABLED: '1',
        NODE_NO_WARNINGS: '1',
        SSH_CONNECTION: '',
  ...
  it('prints the host URL without launching a browser in a VS Code Remote SSH session', async () => {
    ...
    const result = await execa(process.execPath, [
      ...
      env: {
        ...
        ORYGIN_API_KEY: 'keyless-browser-open-no-call',
        ORYGIN_AGENTS_HOME: join(root, '.agents'),
        ORYGIN_BROWSER_OPEN_TEST_EXIT_ON_READY: '1',
        ORYGIN_HOME: join(root, '.dsh'),
        ORYGIN_TELEMETRY_DISABLED: '1',
        NODE_NO_WARNINGS: '1',
