...
import { boot, healProfilesModuleFallback, loadOverlayPatches, loadProfile } from '@orygin-ai/dsh-app-boot'
...
import type { PatchOptions } from '@orygin-ai/cordis-plugin-include'
import { afterAll, beforeAll, describe, expect, it, vi } from 'vitest'
import { settingsNamespace } from '@orygin-ai/dsh-settings'
import { resolveSessionPreset, SETTINGS_NAMESPACE } from '@orygin-ai/dsh-agent-presets'
import { applyChildComposition, childSessionMeta } from '@orygin-ai/dsh-subagent'
import { CallId } from '@orygin-ai/dsh-llm'
import type {} from '@orygin-ai/dsh-compaction-basic'
...
import type {} from '@orygin-ai/dsh-token-meter'
...
async function bootWeb(
  ...
  const overrides: PatchOptions[] = [
    // The settings row defaults to `$ORYGIN_HOME/settings.yaml`. Left alone it
    ...
    // storage-json's root is anchored to the real $ORYGIN_HOME. Unpinned, this
...
describe('the shipped Web composition', () => {
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    ...
    const handle = await ctx.agents.create({
      // Unique per run: the composition persists into the ambient ORYGIN home,
...
describe('a launcher that configures no writable root', () => {
  ...
  // roster derives `<dshHome>/.agent-presets` itself. `$ORYGIN_HOME` is pointed
  ...
  afterAll(async () => {
    if (previousHome === undefined) delete process.env.ORYGIN_HOME
    else process.env.ORYGIN_HOME = previousHome
