 ...
 * @module @orygin-ai/dsh/profile-boot
 */

import { writeFileSync } from 'node:fs'
import { join, resolve } from 'node:path'
import { fileURLToPath } from 'node:url'
import { FiberState, type Context } from '@orygin-ai/cordis'
import type { PatchOptions } from '@orygin-ai/cordis-plugin-include'
import type { EntryOptions } from '@orygin-ai/cordis-plugin-loader'
import {
  boot,
  composeEntries,
...
import { ORYGIN_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@orygin-ai/dsh-launch-environment'
...
/**
 * The home-level user patch layer (`$ORYGIN_HOME/cordis.patch.yml`), applied
 * over every profile's own layer. Resolved per call, not at module load:
 * `$ORYGIN_HOME` may be set by the test or launcher after import.
...
/** The session-telemetry row id the ORYGIN_TELEMETRY_DISABLED switch targets. */
 ...
 * switch set.
 * @param disabledEnv - the raw `ORYGIN_TELEMETRY_DISABLED` value (`undefined` when unset).
...
interface ComposedProfile {
  ...
  /** The home-level user layer (`$ORYGIN_HOME/cordis.patch.yml`), applied after the profile's own. */
 ...
 * layer (`$ORYGIN_HOME/cordis.patch.yml` — machine-local preferences that apply
...
function composeProfile(
  ...
  const telemetryPatch = resolveTelemetryPatch(process.env.ORYGIN_TELEMETRY_DISABLED, rows.has(TELEMETRY_ROW_ID))
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  const ctx = await boot(NAME, rootConfig, structuredClone(allPatches(composed)), (hostCtx) => {
    ...
    hostCtx.provide(ORYGIN_LAUNCH_ENVIRONMENT_KEY, options.environment)
    ...
    try {
      ...
      if (ctx.get('hmr') === undefined) {
        if (ctx.get('timer') === undefined) {
          await ctx.loader.create({ name: '@orygin-ai/cordis-plugin-timer' })
        }
        await ctx.loader.create({ name: '@orygin-ai/cordis-plugin-hmr', config: { root: [] } })
      }
