# Agent Note: experimental subcommands gate behind `--experimental` or `ORYGIN_EXPERIMENTAL=1`
...
**Accept any non-empty `ORYGIN_EXPERIMENTAL`.** Rejected: the telemetry switch prefers off-by-mistake for a privacy control, but an experimental gate is an acknowledgement — `ORYGIN_EXPERIMENTAL=0` must not enable the commands it names.

## Consequences

Daily invocations shorten to `dsh meta --experimental` and `dsh upgrade --experimental`, and a developer who sets `ORYGIN_EXPERIMENTAL=1` in their environment gets the bare `dsh meta`/`dsh upgrade`. `dsh --help` marks both commands `(experimental)`. The gate costs one extra flag or env var until a command stabilizes, at which point the gate is deleted and the name is already final.
