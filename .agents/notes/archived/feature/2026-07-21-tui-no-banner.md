...
- Anything that anchored on banner text (`ORYGIN`, box corners) re-anchors on the footer model name; `main-session-` no longer appears in boot output.
...
`packages/ui/tui/tests/tui.spec.ts` pins: no box corners/product title and an empty transcript when `welcome` is unset, with the model in the footer; a configured welcome as the first transcript line without a banner; and the welcome surviving a palette-swap transcript rebuild. The PTY smoke boots on the footer model name and asserts `ORYGIN HARNESS` is absent. Snapshots verify the full frames.
