...
1. `@orygin-ai/dsh-web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@orygin-ai/dsh-web-search-exa`, `@orygin-ai/dsh-web-search-perplexity`, `@orygin-ai/dsh-web-search-orygin`, and `@orygin-ai/dsh-web-fetch-http`.
3. `@orygin-ai/dsh-tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@orygin-ai/dsh-tool-web  --depends on-->  @orygin-ai/dsh-web  <--depends on--  @orygin-ai/dsh-web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @orygin-ai/dsh-web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @orygin-ai/dsh-web-search-orygin
                                                                                  implementation
                                                                 <--depends on--  @orygin-ai/dsh-web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@orygin-ai/dsh-web-search-exa"] -->|registerSearchProvider| web["@orygin-ai/dsh-web / ctx.web"]
  perplexity["@orygin-ai/dsh-web-search-perplexity"] -->|registerSearchProvider| web
  orygin["@orygin-ai/dsh-web-search-orygin"] -->|registerSearchProvider| web
  fetchLocal["@orygin-ai/dsh-web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@orygin-ai/dsh-tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@orygin-ai/dsh-web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@orygin-ai/dsh-tool-web` depends on `@orygin-ai/dsh-web`, `@orygin-ai/dsh-tools`, `@orygin-ai/dsh-system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@orygin-ai/dsh-web'
...
```yaml
- id: web
  name: '@orygin-ai/dsh-web'
  config:
    searchProvider: exa
    fetchProvider: http

- id: web-search-exa
  name: '@orygin-ai/dsh-web-search-exa'

- id: web-search-perplexity
  name: '@orygin-ai/dsh-web-search-perplexity'

- id: web-search-orygin
  name: '@orygin-ai/dsh-web-search-orygin'

- id: web-fetch-http
  name: '@orygin-ai/dsh-web-fetch-http'

- id: tool-web
  name: '@orygin-ai/dsh-tool-web'
```

Operational overrides feed the same explicit selection path: `ORYGIN_WEB_SEARCH_PROVIDER=perplexity` is equivalent to config `searchProvider: perplexity`, not a hidden priority chain inside `dsh-tool-web`.
