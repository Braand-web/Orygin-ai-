...
- The served rendering of the manifest global changed from `window.__ORYGIN_BOOT__ =` to `globalThis["__ORYGIN_BOOT__"] =`; no committed snapshot expectation carries that text, so none needed re-recording.
