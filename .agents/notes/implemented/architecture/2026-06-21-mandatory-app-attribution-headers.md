...
LLM provider requests should identify the product making them. That is useful for provider-side support, abuse investigation, compatibility debugging, and traffic analytics. Before this Agent Note the harness only partially did this: the hand-rolled Orygin adapter sent a hand-copied `User-Agent` constant (`packages/llm/llm-orygin/src/adapter.ts`), while the pi-ai-backed twin sent no harness-owned headers at all (`packages/llm/llm-pi-ai/src/adapter.ts`). New adapters could therefore omit attribution silently, and a library-backed adapter could drift from the hand-rolled adapter even though [the twin-adapter Agent Note](2026-06-13-twin-llm-adapters.md) exists to keep the provider contract honest across both implementations.

The immediate prompt came from OpenRouter's [App Attribution](https://openrouter.ai/docs/app-attribution) docs. OpenRouter creates app pages and rankings from `HTTP-Referer` plus display/category headers. That is valuable, but it is not the HTTP standard for application identity. The risk is adopting OpenRouter's exact header set as if it were universal, then leaking provider-specific headers to direct Orygin requests, future OpenAI/Anthropic/Vertex adapters, test servers, or proxies that log unknown fields indefinitely.
...
- product token for `User-Agent`: `orygin-ai` (continuity with the pre-Agent Note wire value and the repo/org identity)
- version: read from the owning package's manifest via `createRequire`, never a hand-copied constant
- app URL: `https://github.com/Braand-web/Orygin-ai-` - the repository home
...
| Direct Orygin endpoint | `User-Agent` for app attribution; `x-orygin-ai-user-id` and conditional `x-orygin-ai-session-id` are separate request identity under the Orygin-specific decision. Do not send OpenRouter-only headers unless Orygin documents an equivalent contract. |
...
- `dsh-llm-orygin` sends the shared `User-Agent` on every request and its mock-server suite asserts the exact value.
...
**SDK-named token (`orygin-ai-sdk`).** Considered for the `User-Agent` token because the supported runtime client stack uses the SDK name. `orygin-ai` won because it names the Orygin product, matches the org/repo identity and package scope, and keeps wire attribution stable without calling the complete product an SDK.
