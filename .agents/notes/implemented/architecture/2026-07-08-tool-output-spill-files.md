...
| Package | Role |
|---|---|
| `@orygin-ai/dsh-spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@orygin-ai/dsh-spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@orygin-ai/dsh-spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |
...
- id: spill-local
  name: '@orygin-ai/dsh-spill-local'

- id: spill-policy
  name: '@orygin-ai/dsh-spill-policy'
  config:
...
- `@orygin-ai/dsh-output-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@orygin-ai/dsh-spill` owns saving final text and returning a locator plus retrieval hint.
- `@orygin-ai/dsh-spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
