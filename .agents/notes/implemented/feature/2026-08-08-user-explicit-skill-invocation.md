...
Peer-product survey (Pi, OpenCode, Claude Code, Kimi Code, Codex, Orygin-Reasonix — local checkouts) was unanimous that user-explicit triggering is programmatic injection with zero model participation; the final shape is closest to Codex's core-side `$name` mention scanning, which likewise frees every entry point from implementing recognition.
