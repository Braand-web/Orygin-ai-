...
| Package | Role |
|---|---|
| `@orygin-ai/dsh-subagent` | interface: `SubagentRuntime` (`ctx.subagents`), `SubagentProvider`, `SubagentRun`, the request/result/capability vocabulary, the `subagent/*` events |
| `@orygin-ai/dsh-subagent-spawn-in-process` | implementation: a fresh in-process child via `ctx.agents.create` |
| `@orygin-ai/dsh-subagent-fork-in-process` | implementation: an in-process child seeded with a snapshot of the parent's log |
| `@orygin-ai/dsh-subagent-acp` | implementation: an ACP client driving a configured child process |
| `@orygin-ai/dsh-subagent-codex` | implementation: a one-shot official Codex app-server process |
| `@orygin-ai/dsh-subagent-claude-code` | implementation: a one-shot official Claude Code process through the Agent SDK |
| `@orygin-ai/dsh-tool-subagent` | consumer: the model-facing `subagent` tool over `ctx.subagents` |
