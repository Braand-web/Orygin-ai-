...
`ORYGIN_SYSTEM_PROMPT` selects the standalone persona. `ORYGIN_MODEL` names the Orygin provider catalog entry, and `ORYGIN_CONTEXT_WINDOW` supplies that entry's capacity. Because the SDK client owns the JSON-RPC `initialize` request, [`minimal.py`](../../../../examples/jsonrpc-agent/minimal.py) also uses `ORYGIN_MODEL` as its default `model` argument; an explicit `--model` remains authoritative. Endpoint and credential variables stay owned by the Orygin adapter's existing environment-resolution path.
...
**Read `ORYGIN_MODEL` only inside Cordis.** Rejected because Cordis configures the provider catalog but does not own the SDK client's JSON-RPC `initialize` request. The launcher must pass the same model to the client request for the environment value to select the routed model.
