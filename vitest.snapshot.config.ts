...
const snapshotMaxConcurrency = positiveIntFromEnv(
  'ORYGIN_SNAPSHOT_MAX_CONCURRENCY',
...
if (process.env.ORYGIN_SNAPSHOT === 'record') {
  try {
...
export default defineConfig({
  ...
  test: {
    ...
    include: [
      ...
      ...(process.env.ORYGIN_EXAMPLE_MODE === 'lib' ? ['apps/web/tests/**/*.snapshot.ts'] : []),
    ...
    testTimeout: 120_000,
    hookTimeout: 30_000,
    fileParallelism: (process.env.ORYGIN_SNAPSHOT || 'replay') === 'replay' && snapshotMaxConcurrency > 1,
